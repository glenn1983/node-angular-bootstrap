app.directive('regionDirective',function(){
var region = {
    provinces2: {
        region_id: "2",
        region_name: "北京",
        city: {
            cities52: {
                region_id: "52",
                region_name: "北京",
                county: {
                    counties500: {
                        region_id: "500",
                        region_name: "东城区"
                    },
                    counties501: {
                        region_id: "501",
                        region_name: "西城区"
                    },
                    counties502: {
                        region_id: "502",
                        region_name: "海淀区"
                    },
                    counties503: {
                        region_id: "503",
                        region_name: "朝阳区"
                    },
                    counties504: {
                        region_id: "504",
                        region_name: "崇文区"
                    },
                    counties505: {
                        region_id: "505",
                        region_name: "宣武区"
                    },
                    counties506: {
                        region_id: "506",
                        region_name: "丰台区"
                    },
                    counties507: {
                        region_id: "507",
                        region_name: "石景山区"
                    },
                    counties508: {
                        region_id: "508",
                        region_name: "房山区"
                    },
                    counties509: {
                        region_id: "509",
                        region_name: "门头沟区"
                    },
                    counties510: {
                        region_id: "510",
                        region_name: "通州区"
                    },
                    counties511: {
                        region_id: "511",
                        region_name: "顺义区"
                    },
                    counties512: {
                        region_id: "512",
                        region_name: "昌平区"
                    },
                    counties513: {
                        region_id: "513",
                        region_name: "怀柔区"
                    },
                    counties514: {
                        region_id: "514",
                        region_name: "平谷区"
                    },
                    counties515: {
                        region_id: "515",
                        region_name: "大兴区"
                    },
                    counties516: {
                        region_id: "516",
                        region_name: "密云县"
                    },
                    counties517: {
                        region_id: "517",
                        region_name: "延庆县"
                    }
                }
            }
        }
    },
    provinces3: {
        region_id: "3",
        region_name: "安徽",
        city: {
            cities36: {
                region_id: "36",
                region_name: "安庆",
                county: {
                    counties398: {
                        region_id: "398",
                        region_name: "迎江区"
                    },
                    counties399: {
                        region_id: "399",
                        region_name: "大观区"
                    },
                    counties400: {
                        region_id: "400",
                        region_name: "宜秀区"
                    },
                    counties401: {
                        region_id: "401",
                        region_name: "桐城市"
                    },
                    counties402: {
                        region_id: "402",
                        region_name: "怀宁县"
                    },
                    counties403: {
                        region_id: "403",
                        region_name: "枞阳县"
                    },
                    counties404: {
                        region_id: "404",
                        region_name: "潜山县"
                    },
                    counties405: {
                        region_id: "405",
                        region_name: "太湖县"
                    },
                    counties406: {
                        region_id: "406",
                        region_name: "宿松县"
                    },
                    counties407: {
                        region_id: "407",
                        region_name: "望江县"
                    },
                    counties408: {
                        region_id: "408",
                        region_name: "岳西县"
                    }
                }
            },
            cities37: {
                region_id: "37",
                region_name: "蚌埠",
                county: {
                    counties409: {
                        region_id: "409",
                        region_name: "中市区"
                    },
                    counties410: {
                        region_id: "410",
                        region_name: "东市区"
                    },
                    counties411: {
                        region_id: "411",
                        region_name: "西市区"
                    },
                    counties412: {
                        region_id: "412",
                        region_name: "郊区"
                    },
                    counties413: {
                        region_id: "413",
                        region_name: "怀远县"
                    },
                    counties414: {
                        region_id: "414",
                        region_name: "五河县"
                    },
                    counties415: {
                        region_id: "415",
                        region_name: "固镇县"
                    }
                }
            },
            cities38: {
                region_id: "38",
                region_name: "巢湖",
                county: {
                    counties416: {
                        region_id: "416",
                        region_name: "居巢区"
                    },
                    counties417: {
                        region_id: "417",
                        region_name: "庐江县"
                    },
                    counties418: {
                        region_id: "418",
                        region_name: "无为县"
                    },
                    counties419: {
                        region_id: "419",
                        region_name: "含山县"
                    },
                    counties420: {
                        region_id: "420",
                        region_name: "和县"
                    }
                }
            },
            cities39: {
                region_id: "39",
                region_name: "池州",
                county: {
                    counties421: {
                        region_id: "421",
                        region_name: "贵池区"
                    },
                    counties422: {
                        region_id: "422",
                        region_name: "东至县"
                    },
                    counties423: {
                        region_id: "423",
                        region_name: "石台县"
                    },
                    counties424: {
                        region_id: "424",
                        region_name: "青阳县"
                    }
                }
            },
            cities40: {
                region_id: "40",
                region_name: "滁州",
                county: {
                    counties425: {
                        region_id: "425",
                        region_name: "琅琊区"
                    },
                    counties426: {
                        region_id: "426",
                        region_name: "南谯区"
                    },
                    counties427: {
                        region_id: "427",
                        region_name: "天长市"
                    },
                    counties428: {
                        region_id: "428",
                        region_name: "明光市"
                    },
                    counties429: {
                        region_id: "429",
                        region_name: "来安县"
                    },
                    counties430: {
                        region_id: "430",
                        region_name: "全椒县"
                    },
                    counties431: {
                        region_id: "431",
                        region_name: "定远县"
                    },
                    counties432: {
                        region_id: "432",
                        region_name: "凤阳县"
                    }
                }
            },
            cities41: {
                region_id: "41",
                region_name: "阜阳",
                county: {
                    counties433: {
                        region_id: "433",
                        region_name: "蚌山区"
                    },
                    counties434: {
                        region_id: "434",
                        region_name: "龙子湖区"
                    },
                    counties435: {
                        region_id: "435",
                        region_name: "禹会区"
                    },
                    counties436: {
                        region_id: "436",
                        region_name: "淮上区"
                    },
                    counties437: {
                        region_id: "437",
                        region_name: "颍州区"
                    },
                    counties438: {
                        region_id: "438",
                        region_name: "颍东区"
                    },
                    counties439: {
                        region_id: "439",
                        region_name: "颍泉区"
                    },
                    counties440: {
                        region_id: "440",
                        region_name: "界首市"
                    },
                    counties441: {
                        region_id: "441",
                        region_name: "临泉县"
                    },
                    counties442: {
                        region_id: "442",
                        region_name: "太和县"
                    },
                    counties443: {
                        region_id: "443",
                        region_name: "阜南县"
                    },
                    counties444: {
                        region_id: "444",
                        region_name: "颖上县"
                    }
                }
            },
            cities42: {
                region_id: "42",
                region_name: "淮北",
                county: {
                    counties445: {
                        region_id: "445",
                        region_name: "相山区"
                    },
                    counties446: {
                        region_id: "446",
                        region_name: "杜集区"
                    },
                    counties447: {
                        region_id: "447",
                        region_name: "烈山区"
                    },
                    counties448: {
                        region_id: "448",
                        region_name: "濉溪县"
                    }
                }
            },
            cities43: {
                region_id: "43",
                region_name: "淮南",
                county: {
                    counties449: {
                        region_id: "449",
                        region_name: "田家庵区"
                    },
                    counties450: {
                        region_id: "450",
                        region_name: "大通区"
                    },
                    counties451: {
                        region_id: "451",
                        region_name: "谢家集区"
                    },
                    counties452: {
                        region_id: "452",
                        region_name: "八公山区"
                    },
                    counties453: {
                        region_id: "453",
                        region_name: "潘集区"
                    },
                    counties454: {
                        region_id: "454",
                        region_name: "凤台县"
                    }
                }
            },
            cities44: {
                region_id: "44",
                region_name: "黄山",
                county: {
                    counties455: {
                        region_id: "455",
                        region_name: "屯溪区"
                    },
                    counties456: {
                        region_id: "456",
                        region_name: "黄山区"
                    },
                    counties457: {
                        region_id: "457",
                        region_name: "徽州区"
                    },
                    counties458: {
                        region_id: "458",
                        region_name: "歙县"
                    },
                    counties459: {
                        region_id: "459",
                        region_name: "休宁县"
                    },
                    counties460: {
                        region_id: "460",
                        region_name: "黟县"
                    },
                    counties461: {
                        region_id: "461",
                        region_name: "祁门县"
                    }
                }
            },
            cities45: {
                region_id: "45",
                region_name: "六安",
                county: {
                    counties462: {
                        region_id: "462",
                        region_name: "金安区"
                    },
                    counties463: {
                        region_id: "463",
                        region_name: "裕安区"
                    },
                    counties464: {
                        region_id: "464",
                        region_name: "寿县"
                    },
                    counties465: {
                        region_id: "465",
                        region_name: "霍邱县"
                    },
                    counties466: {
                        region_id: "466",
                        region_name: "舒城县"
                    },
                    counties467: {
                        region_id: "467",
                        region_name: "金寨县"
                    },
                    counties468: {
                        region_id: "468",
                        region_name: "霍山县"
                    }
                }
            },
            cities46: {
                region_id: "46",
                region_name: "马鞍山",
                county: {
                    counties469: {
                        region_id: "469",
                        region_name: "雨山区"
                    },
                    counties470: {
                        region_id: "470",
                        region_name: "花山区"
                    },
                    counties471: {
                        region_id: "471",
                        region_name: "金家庄区"
                    },
                    counties472: {
                        region_id: "472",
                        region_name: "当涂县"
                    }
                }
            },
            cities47: {
                region_id: "47",
                region_name: "宿州",
                county: {
                    counties473: {
                        region_id: "473",
                        region_name: "埇桥区"
                    },
                    counties474: {
                        region_id: "474",
                        region_name: "砀山县"
                    },
                    counties475: {
                        region_id: "475",
                        region_name: "萧县"
                    },
                    counties476: {
                        region_id: "476",
                        region_name: "灵璧县"
                    },
                    counties477: {
                        region_id: "477",
                        region_name: "泗县"
                    }
                }
            },
            cities48: {
                region_id: "48",
                region_name: "铜陵",
                county: {
                    counties478: {
                        region_id: "478",
                        region_name: "铜官山区"
                    },
                    counties479: {
                        region_id: "479",
                        region_name: "狮子山区"
                    },
                    counties480: {
                        region_id: "480",
                        region_name: "郊区"
                    },
                    counties481: {
                        region_id: "481",
                        region_name: "铜陵县"
                    }
                }
            },
            cities49: {
                region_id: "49",
                region_name: "芜湖",
                county: {
                    counties482: {
                        region_id: "482",
                        region_name: "镜湖区"
                    },
                    counties483: {
                        region_id: "483",
                        region_name: "弋江区"
                    },
                    counties484: {
                        region_id: "484",
                        region_name: "鸠江区"
                    },
                    counties485: {
                        region_id: "485",
                        region_name: "三山区"
                    },
                    counties486: {
                        region_id: "486",
                        region_name: "芜湖县"
                    },
                    counties487: {
                        region_id: "487",
                        region_name: "繁昌县"
                    },
                    counties488: {
                        region_id: "488",
                        region_name: "南陵县"
                    }
                }
            },
            cities50: {
                region_id: "50",
                region_name: "宣城",
                county: {
                    counties489: {
                        region_id: "489",
                        region_name: "宣州区"
                    },
                    counties490: {
                        region_id: "490",
                        region_name: "宁国市"
                    },
                    counties491: {
                        region_id: "491",
                        region_name: "郎溪县"
                    },
                    counties492: {
                        region_id: "492",
                        region_name: "广德县"
                    },
                    counties493: {
                        region_id: "493",
                        region_name: "泾县"
                    },
                    counties494: {
                        region_id: "494",
                        region_name: "绩溪县"
                    },
                    counties495: {
                        region_id: "495",
                        region_name: "旌德县"
                    }
                }
            },
            cities3401: {
                region_id: "3401",
                region_name: "合肥",
                county: {
                    counties3402: {
                        region_id: "3402",
                        region_name: "庐阳区"
                    },
                    counties3403: {
                        region_id: "3403",
                        region_name: "瑶海区"
                    },
                    counties3404: {
                        region_id: "3404",
                        region_name: "蜀山区"
                    },
                    counties3405: {
                        region_id: "3405",
                        region_name: "包河区"
                    },
                    counties3406: {
                        region_id: "3406",
                        region_name: "长丰县"
                    },
                    counties3407: {
                        region_id: "3407",
                        region_name: "肥东县"
                    },
                    counties3408: {
                        region_id: "3408",
                        region_name: "肥西县"
                    }
                }
            }
        }
    },
    provinces4: {
        region_id: "4",
        region_name: "福建",
        city: {
            cities53: {
                region_id: "53",
                region_name: "福州",
                county: {
                    counties518: {
                        region_id: "518",
                        region_name: "鼓楼区"
                    },
                    counties519: {
                        region_id: "519",
                        region_name: "台江区"
                    },
                    counties520: {
                        region_id: "520",
                        region_name: "仓山区"
                    },
                    counties521: {
                        region_id: "521",
                        region_name: "马尾区"
                    },
                    counties522: {
                        region_id: "522",
                        region_name: "晋安区"
                    },
                    counties523: {
                        region_id: "523",
                        region_name: "福清市"
                    },
                    counties524: {
                        region_id: "524",
                        region_name: "长乐市"
                    },
                    counties525: {
                        region_id: "525",
                        region_name: "闽侯县"
                    },
                    counties526: {
                        region_id: "526",
                        region_name: "连江县"
                    },
                    counties527: {
                        region_id: "527",
                        region_name: "罗源县"
                    },
                    counties528: {
                        region_id: "528",
                        region_name: "闽清县"
                    },
                    counties529: {
                        region_id: "529",
                        region_name: "永泰县"
                    },
                    counties530: {
                        region_id: "530",
                        region_name: "平潭县"
                    }
                }
            },
            cities54: {
                region_id: "54",
                region_name: "龙岩",
                county: {
                    counties531: {
                        region_id: "531",
                        region_name: "新罗区"
                    },
                    counties532: {
                        region_id: "532",
                        region_name: "漳平市"
                    },
                    counties533: {
                        region_id: "533",
                        region_name: "长汀县"
                    },
                    counties534: {
                        region_id: "534",
                        region_name: "永定县"
                    },
                    counties535: {
                        region_id: "535",
                        region_name: "上杭县"
                    },
                    counties536: {
                        region_id: "536",
                        region_name: "武平县"
                    },
                    counties537: {
                        region_id: "537",
                        region_name: "连城县"
                    }
                }
            },
            cities55: {
                region_id: "55",
                region_name: "南平",
                county: {
                    counties538: {
                        region_id: "538",
                        region_name: "延平区"
                    },
                    counties539: {
                        region_id: "539",
                        region_name: "邵武市"
                    },
                    counties540: {
                        region_id: "540",
                        region_name: "武夷山市"
                    },
                    counties541: {
                        region_id: "541",
                        region_name: "建瓯市"
                    },
                    counties542: {
                        region_id: "542",
                        region_name: "建阳市"
                    },
                    counties543: {
                        region_id: "543",
                        region_name: "顺昌县"
                    },
                    counties544: {
                        region_id: "544",
                        region_name: "浦城县"
                    },
                    counties545: {
                        region_id: "545",
                        region_name: "光泽县"
                    },
                    counties546: {
                        region_id: "546",
                        region_name: "松溪县"
                    },
                    counties547: {
                        region_id: "547",
                        region_name: "政和县"
                    }
                }
            },
            cities56: {
                region_id: "56",
                region_name: "宁德",
                county: {
                    counties548: {
                        region_id: "548",
                        region_name: "蕉城区"
                    },
                    counties549: {
                        region_id: "549",
                        region_name: "福安市"
                    },
                    counties550: {
                        region_id: "550",
                        region_name: "福鼎市"
                    },
                    counties551: {
                        region_id: "551",
                        region_name: "霞浦县"
                    },
                    counties552: {
                        region_id: "552",
                        region_name: "古田县"
                    },
                    counties553: {
                        region_id: "553",
                        region_name: "屏南县"
                    },
                    counties554: {
                        region_id: "554",
                        region_name: "寿宁县"
                    },
                    counties555: {
                        region_id: "555",
                        region_name: "周宁县"
                    },
                    counties556: {
                        region_id: "556",
                        region_name: "柘荣县"
                    }
                }
            },
            cities57: {
                region_id: "57",
                region_name: "莆田",
                county: {
                    counties557: {
                        region_id: "557",
                        region_name: "城厢区"
                    },
                    counties558: {
                        region_id: "558",
                        region_name: "涵江区"
                    },
                    counties559: {
                        region_id: "559",
                        region_name: "荔城区"
                    },
                    counties560: {
                        region_id: "560",
                        region_name: "秀屿区"
                    },
                    counties561: {
                        region_id: "561",
                        region_name: "仙游县"
                    }
                }
            },
            cities58: {
                region_id: "58",
                region_name: "泉州",
                county: {
                    counties562: {
                        region_id: "562",
                        region_name: "鲤城区"
                    },
                    counties563: {
                        region_id: "563",
                        region_name: "丰泽区"
                    },
                    counties564: {
                        region_id: "564",
                        region_name: "洛江区"
                    },
                    counties565: {
                        region_id: "565",
                        region_name: "清濛开发区"
                    },
                    counties566: {
                        region_id: "566",
                        region_name: "泉港区"
                    },
                    counties567: {
                        region_id: "567",
                        region_name: "石狮市"
                    },
                    counties568: {
                        region_id: "568",
                        region_name: "晋江市"
                    },
                    counties569: {
                        region_id: "569",
                        region_name: "南安市"
                    },
                    counties570: {
                        region_id: "570",
                        region_name: "惠安县"
                    },
                    counties571: {
                        region_id: "571",
                        region_name: "安溪县"
                    },
                    counties572: {
                        region_id: "572",
                        region_name: "永春县"
                    },
                    counties573: {
                        region_id: "573",
                        region_name: "德化县"
                    },
                    counties574: {
                        region_id: "574",
                        region_name: "金门县"
                    }
                }
            },
            cities59: {
                region_id: "59",
                region_name: "三明",
                county: {
                    counties575: {
                        region_id: "575",
                        region_name: "梅列区"
                    },
                    counties576: {
                        region_id: "576",
                        region_name: "三元区"
                    },
                    counties577: {
                        region_id: "577",
                        region_name: "永安市"
                    },
                    counties578: {
                        region_id: "578",
                        region_name: "明溪县"
                    },
                    counties579: {
                        region_id: "579",
                        region_name: "清流县"
                    },
                    counties580: {
                        region_id: "580",
                        region_name: "宁化县"
                    },
                    counties581: {
                        region_id: "581",
                        region_name: "大田县"
                    },
                    counties582: {
                        region_id: "582",
                        region_name: "尤溪县"
                    },
                    counties583: {
                        region_id: "583",
                        region_name: "沙县"
                    },
                    counties584: {
                        region_id: "584",
                        region_name: "将乐县"
                    },
                    counties585: {
                        region_id: "585",
                        region_name: "泰宁县"
                    },
                    counties586: {
                        region_id: "586",
                        region_name: "建宁县"
                    }
                }
            },
            cities60: {
                region_id: "60",
                region_name: "厦门",
                county: {
                    counties587: {
                        region_id: "587",
                        region_name: "思明区"
                    },
                    counties588: {
                        region_id: "588",
                        region_name: "海沧区"
                    },
                    counties589: {
                        region_id: "589",
                        region_name: "湖里区"
                    },
                    counties590: {
                        region_id: "590",
                        region_name: "集美区"
                    },
                    counties591: {
                        region_id: "591",
                        region_name: "同安区"
                    },
                    counties592: {
                        region_id: "592",
                        region_name: "翔安区"
                    }
                }
            },
            cities61: {
                region_id: "61",
                region_name: "漳州",
                county: {
                    counties593: {
                        region_id: "593",
                        region_name: "芗城区"
                    },
                    counties594: {
                        region_id: "594",
                        region_name: "龙文区"
                    },
                    counties595: {
                        region_id: "595",
                        region_name: "龙海市"
                    },
                    counties596: {
                        region_id: "596",
                        region_name: "云霄县"
                    },
                    counties597: {
                        region_id: "597",
                        region_name: "漳浦县"
                    },
                    counties598: {
                        region_id: "598",
                        region_name: "诏安县"
                    },
                    counties599: {
                        region_id: "599",
                        region_name: "长泰县"
                    },
                    counties600: {
                        region_id: "600",
                        region_name: "东山县"
                    },
                    counties601: {
                        region_id: "601",
                        region_name: "南靖县"
                    },
                    counties602: {
                        region_id: "602",
                        region_name: "平和县"
                    },
                    counties603: {
                        region_id: "603",
                        region_name: "华安县"
                    }
                }
            }
        }
    },
    provinces5: {
        region_id: "5",
        region_name: "甘肃",
        city: {
            cities62: {
                region_id: "62",
                region_name: "兰州",
                county: {
                    counties604: {
                        region_id: "604",
                        region_name: "皋兰县"
                    },
                    counties605: {
                        region_id: "605",
                        region_name: "城关区"
                    },
                    counties606: {
                        region_id: "606",
                        region_name: "七里河区"
                    },
                    counties607: {
                        region_id: "607",
                        region_name: "西固区"
                    },
                    counties608: {
                        region_id: "608",
                        region_name: "安宁区"
                    },
                    counties609: {
                        region_id: "609",
                        region_name: "红古区"
                    },
                    counties610: {
                        region_id: "610",
                        region_name: "永登县"
                    },
                    counties611: {
                        region_id: "611",
                        region_name: "榆中县"
                    }
                }
            },
            cities63: {
                region_id: "63",
                region_name: "白银",
                county: {
                    counties612: {
                        region_id: "612",
                        region_name: "白银区"
                    },
                    counties613: {
                        region_id: "613",
                        region_name: "平川区"
                    },
                    counties614: {
                        region_id: "614",
                        region_name: "会宁县"
                    },
                    counties615: {
                        region_id: "615",
                        region_name: "景泰县"
                    },
                    counties616: {
                        region_id: "616",
                        region_name: "靖远县"
                    }
                }
            },
            cities64: {
                region_id: "64",
                region_name: "定西",
                county: {
                    counties617: {
                        region_id: "617",
                        region_name: "临洮县"
                    },
                    counties618: {
                        region_id: "618",
                        region_name: "陇西县"
                    },
                    counties619: {
                        region_id: "619",
                        region_name: "通渭县"
                    },
                    counties620: {
                        region_id: "620",
                        region_name: "渭源县"
                    },
                    counties621: {
                        region_id: "621",
                        region_name: "漳县"
                    },
                    counties622: {
                        region_id: "622",
                        region_name: "岷县"
                    },
                    counties623: {
                        region_id: "623",
                        region_name: "安定区"
                    },
                    counties624: {
                        region_id: "624",
                        region_name: "安定区"
                    }
                }
            },
            cities65: {
                region_id: "65",
                region_name: "甘南",
                county: {
                    counties625: {
                        region_id: "625",
                        region_name: "合作市"
                    },
                    counties626: {
                        region_id: "626",
                        region_name: "临潭县"
                    },
                    counties627: {
                        region_id: "627",
                        region_name: "卓尼县"
                    },
                    counties628: {
                        region_id: "628",
                        region_name: "舟曲县"
                    },
                    counties629: {
                        region_id: "629",
                        region_name: "迭部县"
                    },
                    counties630: {
                        region_id: "630",
                        region_name: "玛曲县"
                    },
                    counties631: {
                        region_id: "631",
                        region_name: "碌曲县"
                    },
                    counties632: {
                        region_id: "632",
                        region_name: "夏河县"
                    }
                }
            },
            cities66: {
                region_id: "66",
                region_name: "嘉峪关",
                county: {
                    counties633: {
                        region_id: "633",
                        region_name: "嘉峪关市"
                    }
                }
            },
            cities67: {
                region_id: "67",
                region_name: "金昌",
                county: {
                    counties634: {
                        region_id: "634",
                        region_name: "金川区"
                    },
                    counties635: {
                        region_id: "635",
                        region_name: "永昌县"
                    }
                }
            },
            cities68: {
                region_id: "68",
                region_name: "酒泉",
                county: {
                    counties636: {
                        region_id: "636",
                        region_name: "肃州区"
                    },
                    counties637: {
                        region_id: "637",
                        region_name: "玉门市"
                    },
                    counties638: {
                        region_id: "638",
                        region_name: "敦煌市"
                    },
                    counties639: {
                        region_id: "639",
                        region_name: "金塔县"
                    },
                    counties640: {
                        region_id: "640",
                        region_name: "瓜州县"
                    },
                    counties641: {
                        region_id: "641",
                        region_name: "肃北"
                    },
                    counties642: {
                        region_id: "642",
                        region_name: "阿克塞"
                    }
                }
            },
            cities69: {
                region_id: "69",
                region_name: "临夏",
                county: {
                    counties643: {
                        region_id: "643",
                        region_name: "临夏市"
                    },
                    counties644: {
                        region_id: "644",
                        region_name: "临夏县"
                    },
                    counties645: {
                        region_id: "645",
                        region_name: "康乐县"
                    },
                    counties646: {
                        region_id: "646",
                        region_name: "永靖县"
                    },
                    counties647: {
                        region_id: "647",
                        region_name: "广河县"
                    },
                    counties648: {
                        region_id: "648",
                        region_name: "和政县"
                    },
                    counties649: {
                        region_id: "649",
                        region_name: "东乡族自治县"
                    },
                    counties650: {
                        region_id: "650",
                        region_name: "积石山"
                    }
                }
            },
            cities70: {
                region_id: "70",
                region_name: "陇南",
                county: {
                    counties651: {
                        region_id: "651",
                        region_name: "成县"
                    },
                    counties652: {
                        region_id: "652",
                        region_name: "徽县"
                    },
                    counties653: {
                        region_id: "653",
                        region_name: "康县"
                    },
                    counties654: {
                        region_id: "654",
                        region_name: "礼县"
                    },
                    counties655: {
                        region_id: "655",
                        region_name: "两当县"
                    },
                    counties656: {
                        region_id: "656",
                        region_name: "文县"
                    },
                    counties657: {
                        region_id: "657",
                        region_name: "西和县"
                    },
                    counties658: {
                        region_id: "658",
                        region_name: "宕昌县"
                    },
                    counties659: {
                        region_id: "659",
                        region_name: "武都区"
                    }
                }
            },
            cities71: {
                region_id: "71",
                region_name: "平凉",
                county: {
                    counties660: {
                        region_id: "660",
                        region_name: "崇信县"
                    },
                    counties661: {
                        region_id: "661",
                        region_name: "华亭县"
                    },
                    counties662: {
                        region_id: "662",
                        region_name: "静宁县"
                    },
                    counties663: {
                        region_id: "663",
                        region_name: "灵台县"
                    },
                    counties664: {
                        region_id: "664",
                        region_name: "崆峒区"
                    },
                    counties665: {
                        region_id: "665",
                        region_name: "庄浪县"
                    },
                    counties666: {
                        region_id: "666",
                        region_name: "泾川县"
                    }
                }
            },
            cities72: {
                region_id: "72",
                region_name: "庆阳",
                county: {
                    counties667: {
                        region_id: "667",
                        region_name: "合水县"
                    },
                    counties668: {
                        region_id: "668",
                        region_name: "华池县"
                    },
                    counties669: {
                        region_id: "669",
                        region_name: "环县"
                    },
                    counties670: {
                        region_id: "670",
                        region_name: "宁县"
                    },
                    counties671: {
                        region_id: "671",
                        region_name: "庆城县"
                    },
                    counties672: {
                        region_id: "672",
                        region_name: "西峰区"
                    },
                    counties673: {
                        region_id: "673",
                        region_name: "镇原县"
                    },
                    counties674: {
                        region_id: "674",
                        region_name: "正宁县"
                    }
                }
            },
            cities73: {
                region_id: "73",
                region_name: "天水",
                county: {
                    counties675: {
                        region_id: "675",
                        region_name: "甘谷县"
                    },
                    counties676: {
                        region_id: "676",
                        region_name: "秦安县"
                    },
                    counties677: {
                        region_id: "677",
                        region_name: "清水县"
                    },
                    counties678: {
                        region_id: "678",
                        region_name: "秦州区"
                    },
                    counties679: {
                        region_id: "679",
                        region_name: "麦积区"
                    },
                    counties680: {
                        region_id: "680",
                        region_name: "武山县"
                    },
                    counties681: {
                        region_id: "681",
                        region_name: "张家川"
                    }
                }
            },
            cities74: {
                region_id: "74",
                region_name: "武威",
                county: {
                    counties682: {
                        region_id: "682",
                        region_name: "古浪县"
                    },
                    counties683: {
                        region_id: "683",
                        region_name: "民勤县"
                    },
                    counties684: {
                        region_id: "684",
                        region_name: "天祝"
                    },
                    counties685: {
                        region_id: "685",
                        region_name: "凉州区"
                    }
                }
            },
            cities75: {
                region_id: "75",
                region_name: "张掖",
                county: {
                    counties686: {
                        region_id: "686",
                        region_name: "高台县"
                    },
                    counties687: {
                        region_id: "687",
                        region_name: "临泽县"
                    },
                    counties688: {
                        region_id: "688",
                        region_name: "民乐县"
                    },
                    counties689: {
                        region_id: "689",
                        region_name: "山丹县"
                    },
                    counties690: {
                        region_id: "690",
                        region_name: "肃南"
                    },
                    counties691: {
                        region_id: "691",
                        region_name: "甘州区"
                    }
                }
            }
        }
    },
    provinces6: {
        region_id: "6",
        region_name: "广东",
        city: {
            cities76: {
                region_id: "76",
                region_name: "广州",
                county: {
                    counties692: {
                        region_id: "692",
                        region_name: "从化市"
                    },
                    counties693: {
                        region_id: "693",
                        region_name: "天河区"
                    },
                    counties694: {
                        region_id: "694",
                        region_name: "东山区"
                    },
                    counties695: {
                        region_id: "695",
                        region_name: "白云区"
                    },
                    counties696: {
                        region_id: "696",
                        region_name: "海珠区"
                    },
                    counties697: {
                        region_id: "697",
                        region_name: "荔湾区"
                    },
                    counties698: {
                        region_id: "698",
                        region_name: "越秀区"
                    },
                    counties699: {
                        region_id: "699",
                        region_name: "黄埔区"
                    },
                    counties700: {
                        region_id: "700",
                        region_name: "番禺区"
                    },
                    counties701: {
                        region_id: "701",
                        region_name: "花都区"
                    },
                    counties702: {
                        region_id: "702",
                        region_name: "增城区"
                    },
                    counties703: {
                        region_id: "703",
                        region_name: "从化区"
                    },
                    counties704: {
                        region_id: "704",
                        region_name: "市郊"
                    }
                }
            },
            cities77: {
                region_id: "77",
                region_name: "深圳",
                county: {
                    counties705: {
                        region_id: "705",
                        region_name: "福田区"
                    },
                    counties706: {
                        region_id: "706",
                        region_name: "罗湖区"
                    },
                    counties707: {
                        region_id: "707",
                        region_name: "南山区"
                    },
                    counties708: {
                        region_id: "708",
                        region_name: "宝安区"
                    },
                    counties709: {
                        region_id: "709",
                        region_name: "龙岗区"
                    },
                    counties710: {
                        region_id: "710",
                        region_name: "盐田区"
                    }
                }
            },
            cities78: {
                region_id: "78",
                region_name: "潮州",
                county: {
                    counties711: {
                        region_id: "711",
                        region_name: "湘桥区"
                    },
                    counties712: {
                        region_id: "712",
                        region_name: "潮安县"
                    },
                    counties713: {
                        region_id: "713",
                        region_name: "饶平县"
                    }
                }
            },
            cities79: {
                region_id: "79",
                region_name: "东莞",
                county: {
                    counties714: {
                        region_id: "714",
                        region_name: "南城区"
                    },
                    counties715: {
                        region_id: "715",
                        region_name: "东城区"
                    },
                    counties716: {
                        region_id: "716",
                        region_name: "万江区"
                    },
                    counties717: {
                        region_id: "717",
                        region_name: "莞城区"
                    },
                    counties718: {
                        region_id: "718",
                        region_name: "石龙镇"
                    },
                    counties719: {
                        region_id: "719",
                        region_name: "虎门镇"
                    },
                    counties720: {
                        region_id: "720",
                        region_name: "麻涌镇"
                    },
                    counties721: {
                        region_id: "721",
                        region_name: "道滘镇"
                    },
                    counties722: {
                        region_id: "722",
                        region_name: "石碣镇"
                    },
                    counties723: {
                        region_id: "723",
                        region_name: "沙田镇"
                    },
                    counties724: {
                        region_id: "724",
                        region_name: "望牛墩镇"
                    },
                    counties725: {
                        region_id: "725",
                        region_name: "洪梅镇"
                    },
                    counties726: {
                        region_id: "726",
                        region_name: "茶山镇"
                    },
                    counties727: {
                        region_id: "727",
                        region_name: "寮步镇"
                    },
                    counties728: {
                        region_id: "728",
                        region_name: "大岭山镇"
                    },
                    counties729: {
                        region_id: "729",
                        region_name: "大朗镇"
                    },
                    counties730: {
                        region_id: "730",
                        region_name: "黄江镇"
                    },
                    counties731: {
                        region_id: "731",
                        region_name: "樟木头"
                    },
                    counties732: {
                        region_id: "732",
                        region_name: "凤岗镇"
                    },
                    counties733: {
                        region_id: "733",
                        region_name: "塘厦镇"
                    },
                    counties734: {
                        region_id: "734",
                        region_name: "谢岗镇"
                    },
                    counties735: {
                        region_id: "735",
                        region_name: "厚街镇"
                    },
                    counties736: {
                        region_id: "736",
                        region_name: "清溪镇"
                    },
                    counties737: {
                        region_id: "737",
                        region_name: "常平镇"
                    },
                    counties738: {
                        region_id: "738",
                        region_name: "桥头镇"
                    },
                    counties739: {
                        region_id: "739",
                        region_name: "横沥镇"
                    },
                    counties740: {
                        region_id: "740",
                        region_name: "东坑镇"
                    },
                    counties741: {
                        region_id: "741",
                        region_name: "企石镇"
                    },
                    counties742: {
                        region_id: "742",
                        region_name: "石排镇"
                    },
                    counties743: {
                        region_id: "743",
                        region_name: "长安镇"
                    },
                    counties744: {
                        region_id: "744",
                        region_name: "中堂镇"
                    },
                    counties745: {
                        region_id: "745",
                        region_name: "高埗镇"
                    }
                }
            },
            cities80: {
                region_id: "80",
                region_name: "佛山",
                county: {
                    counties746: {
                        region_id: "746",
                        region_name: "禅城区"
                    },
                    counties747: {
                        region_id: "747",
                        region_name: "南海区"
                    },
                    counties748: {
                        region_id: "748",
                        region_name: "顺德区"
                    },
                    counties749: {
                        region_id: "749",
                        region_name: "三水区"
                    },
                    counties750: {
                        region_id: "750",
                        region_name: "高明区"
                    }
                }
            },
            cities81: {
                region_id: "81",
                region_name: "河源",
                county: {
                    counties751: {
                        region_id: "751",
                        region_name: "东源县"
                    },
                    counties752: {
                        region_id: "752",
                        region_name: "和平县"
                    },
                    counties753: {
                        region_id: "753",
                        region_name: "源城区"
                    },
                    counties754: {
                        region_id: "754",
                        region_name: "连平县"
                    },
                    counties755: {
                        region_id: "755",
                        region_name: "龙川县"
                    },
                    counties756: {
                        region_id: "756",
                        region_name: "紫金县"
                    }
                }
            },
            cities82: {
                region_id: "82",
                region_name: "惠州",
                county: {
                    counties757: {
                        region_id: "757",
                        region_name: "惠阳区"
                    },
                    counties758: {
                        region_id: "758",
                        region_name: "惠城区"
                    },
                    counties759: {
                        region_id: "759",
                        region_name: "大亚湾"
                    },
                    counties760: {
                        region_id: "760",
                        region_name: "博罗县"
                    },
                    counties761: {
                        region_id: "761",
                        region_name: "惠东县"
                    },
                    counties762: {
                        region_id: "762",
                        region_name: "龙门县"
                    }
                }
            },
            cities83: {
                region_id: "83",
                region_name: "江门",
                county: {
                    counties763: {
                        region_id: "763",
                        region_name: "江海区"
                    },
                    counties764: {
                        region_id: "764",
                        region_name: "蓬江区"
                    },
                    counties765: {
                        region_id: "765",
                        region_name: "新会区"
                    },
                    counties766: {
                        region_id: "766",
                        region_name: "台山市"
                    },
                    counties767: {
                        region_id: "767",
                        region_name: "开平市"
                    },
                    counties768: {
                        region_id: "768",
                        region_name: "鹤山市"
                    },
                    counties769: {
                        region_id: "769",
                        region_name: "恩平市"
                    }
                }
            },
            cities84: {
                region_id: "84",
                region_name: "揭阳",
                county: {
                    counties770: {
                        region_id: "770",
                        region_name: "榕城区"
                    },
                    counties771: {
                        region_id: "771",
                        region_name: "普宁市"
                    },
                    counties772: {
                        region_id: "772",
                        region_name: "揭东县"
                    },
                    counties773: {
                        region_id: "773",
                        region_name: "揭西县"
                    },
                    counties774: {
                        region_id: "774",
                        region_name: "惠来县"
                    }
                }
            },
            cities85: {
                region_id: "85",
                region_name: "茂名",
                county: {
                    counties775: {
                        region_id: "775",
                        region_name: "茂南区"
                    },
                    counties776: {
                        region_id: "776",
                        region_name: "茂港区"
                    },
                    counties777: {
                        region_id: "777",
                        region_name: "高州市"
                    },
                    counties778: {
                        region_id: "778",
                        region_name: "化州市"
                    },
                    counties779: {
                        region_id: "779",
                        region_name: "信宜市"
                    },
                    counties780: {
                        region_id: "780",
                        region_name: "电白县"
                    }
                }
            },
            cities86: {
                region_id: "86",
                region_name: "梅州",
                county: {
                    counties781: {
                        region_id: "781",
                        region_name: "梅县"
                    },
                    counties782: {
                        region_id: "782",
                        region_name: "梅江区"
                    },
                    counties783: {
                        region_id: "783",
                        region_name: "兴宁市"
                    },
                    counties784: {
                        region_id: "784",
                        region_name: "大埔县"
                    },
                    counties785: {
                        region_id: "785",
                        region_name: "丰顺县"
                    },
                    counties786: {
                        region_id: "786",
                        region_name: "五华县"
                    },
                    counties787: {
                        region_id: "787",
                        region_name: "平远县"
                    },
                    counties788: {
                        region_id: "788",
                        region_name: "蕉岭县"
                    }
                }
            },
            cities87: {
                region_id: "87",
                region_name: "清远",
                county: {
                    counties789: {
                        region_id: "789",
                        region_name: "清城区"
                    },
                    counties790: {
                        region_id: "790",
                        region_name: "英德市"
                    },
                    counties791: {
                        region_id: "791",
                        region_name: "连州市"
                    },
                    counties792: {
                        region_id: "792",
                        region_name: "佛冈县"
                    },
                    counties793: {
                        region_id: "793",
                        region_name: "阳山县"
                    },
                    counties794: {
                        region_id: "794",
                        region_name: "清新县"
                    },
                    counties795: {
                        region_id: "795",
                        region_name: "连山"
                    },
                    counties796: {
                        region_id: "796",
                        region_name: "连南"
                    }
                }
            },
            cities88: {
                region_id: "88",
                region_name: "汕头",
                county: {
                    counties797: {
                        region_id: "797",
                        region_name: "南澳县"
                    },
                    counties798: {
                        region_id: "798",
                        region_name: "潮阳区"
                    },
                    counties799: {
                        region_id: "799",
                        region_name: "澄海区"
                    },
                    counties800: {
                        region_id: "800",
                        region_name: "龙湖区"
                    },
                    counties801: {
                        region_id: "801",
                        region_name: "金平区"
                    },
                    counties802: {
                        region_id: "802",
                        region_name: "濠江区"
                    },
                    counties803: {
                        region_id: "803",
                        region_name: "潮南区"
                    }
                }
            },
            cities89: {
                region_id: "89",
                region_name: "汕尾",
                county: {
                    counties804: {
                        region_id: "804",
                        region_name: "城区"
                    },
                    counties805: {
                        region_id: "805",
                        region_name: "陆丰市"
                    },
                    counties806: {
                        region_id: "806",
                        region_name: "海丰县"
                    },
                    counties807: {
                        region_id: "807",
                        region_name: "陆河县"
                    }
                }
            },
            cities90: {
                region_id: "90",
                region_name: "韶关",
                county: {
                    counties808: {
                        region_id: "808",
                        region_name: "曲江县"
                    },
                    counties809: {
                        region_id: "809",
                        region_name: "浈江区"
                    },
                    counties810: {
                        region_id: "810",
                        region_name: "武江区"
                    },
                    counties811: {
                        region_id: "811",
                        region_name: "曲江区"
                    },
                    counties812: {
                        region_id: "812",
                        region_name: "乐昌市"
                    },
                    counties813: {
                        region_id: "813",
                        region_name: "南雄市"
                    },
                    counties814: {
                        region_id: "814",
                        region_name: "始兴县"
                    },
                    counties815: {
                        region_id: "815",
                        region_name: "仁化县"
                    },
                    counties816: {
                        region_id: "816",
                        region_name: "翁源县"
                    },
                    counties817: {
                        region_id: "817",
                        region_name: "新丰县"
                    },
                    counties818: {
                        region_id: "818",
                        region_name: "乳源"
                    }
                }
            },
            cities91: {
                region_id: "91",
                region_name: "阳江",
                county: {
                    counties819: {
                        region_id: "819",
                        region_name: "江城区"
                    },
                    counties820: {
                        region_id: "820",
                        region_name: "阳春市"
                    },
                    counties821: {
                        region_id: "821",
                        region_name: "阳西县"
                    },
                    counties822: {
                        region_id: "822",
                        region_name: "阳东县"
                    }
                }
            },
            cities92: {
                region_id: "92",
                region_name: "云浮",
                county: {
                    counties823: {
                        region_id: "823",
                        region_name: "云城区"
                    },
                    counties824: {
                        region_id: "824",
                        region_name: "罗定市"
                    },
                    counties825: {
                        region_id: "825",
                        region_name: "新兴县"
                    },
                    counties826: {
                        region_id: "826",
                        region_name: "郁南县"
                    },
                    counties827: {
                        region_id: "827",
                        region_name: "云安县"
                    }
                }
            },
            cities93: {
                region_id: "93",
                region_name: "湛江",
                county: {
                    counties828: {
                        region_id: "828",
                        region_name: "赤坎区"
                    },
                    counties829: {
                        region_id: "829",
                        region_name: "霞山区"
                    },
                    counties830: {
                        region_id: "830",
                        region_name: "坡头区"
                    },
                    counties831: {
                        region_id: "831",
                        region_name: "麻章区"
                    },
                    counties832: {
                        region_id: "832",
                        region_name: "廉江市"
                    },
                    counties833: {
                        region_id: "833",
                        region_name: "雷州市"
                    },
                    counties834: {
                        region_id: "834",
                        region_name: "吴川市"
                    },
                    counties835: {
                        region_id: "835",
                        region_name: "遂溪县"
                    },
                    counties836: {
                        region_id: "836",
                        region_name: "徐闻县"
                    }
                }
            },
            cities94: {
                region_id: "94",
                region_name: "肇庆",
                county: {
                    counties837: {
                        region_id: "837",
                        region_name: "肇庆市"
                    },
                    counties838: {
                        region_id: "838",
                        region_name: "高要市"
                    },
                    counties839: {
                        region_id: "839",
                        region_name: "四会市"
                    },
                    counties840: {
                        region_id: "840",
                        region_name: "广宁县"
                    },
                    counties841: {
                        region_id: "841",
                        region_name: "怀集县"
                    },
                    counties842: {
                        region_id: "842",
                        region_name: "封开县"
                    },
                    counties843: {
                        region_id: "843",
                        region_name: "德庆县"
                    }
                }
            },
            cities95: {
                region_id: "95",
                region_name: "中山",
                county: {
                    counties844: {
                        region_id: "844",
                        region_name: "石岐街道"
                    },
                    counties845: {
                        region_id: "845",
                        region_name: "东区街道"
                    },
                    counties846: {
                        region_id: "846",
                        region_name: "西区街道"
                    },
                    counties847: {
                        region_id: "847",
                        region_name: "环城街道"
                    },
                    counties848: {
                        region_id: "848",
                        region_name: "中山港街道"
                    },
                    counties849: {
                        region_id: "849",
                        region_name: "五桂山街道"
                    }
                }
            },
            cities96: {
                region_id: "96",
                region_name: "珠海",
                county: {
                    counties850: {
                        region_id: "850",
                        region_name: "香洲区"
                    },
                    counties851: {
                        region_id: "851",
                        region_name: "斗门区"
                    },
                    counties852: {
                        region_id: "852",
                        region_name: "金湾区"
                    }
                }
            }
        }
    },
    provinces7: {
        region_id: "7",
        region_name: "广西",
        city: {
            cities97: {
                region_id: "97",
                region_name: "南宁",
                county: {
                    counties853: {
                        region_id: "853",
                        region_name: "邕宁区"
                    },
                    counties854: {
                        region_id: "854",
                        region_name: "青秀区"
                    },
                    counties855: {
                        region_id: "855",
                        region_name: "兴宁区"
                    },
                    counties856: {
                        region_id: "856",
                        region_name: "良庆区"
                    },
                    counties857: {
                        region_id: "857",
                        region_name: "西乡塘区"
                    },
                    counties858: {
                        region_id: "858",
                        region_name: "江南区"
                    },
                    counties859: {
                        region_id: "859",
                        region_name: "武鸣县"
                    },
                    counties860: {
                        region_id: "860",
                        region_name: "隆安县"
                    },
                    counties861: {
                        region_id: "861",
                        region_name: "马山县"
                    },
                    counties862: {
                        region_id: "862",
                        region_name: "上林县"
                    },
                    counties863: {
                        region_id: "863",
                        region_name: "宾阳县"
                    },
                    counties864: {
                        region_id: "864",
                        region_name: "横县"
                    }
                }
            },
            cities98: {
                region_id: "98",
                region_name: "桂林",
                county: {
                    counties865: {
                        region_id: "865",
                        region_name: "秀峰区"
                    },
                    counties866: {
                        region_id: "866",
                        region_name: "叠彩区"
                    },
                    counties867: {
                        region_id: "867",
                        region_name: "象山区"
                    },
                    counties868: {
                        region_id: "868",
                        region_name: "七星区"
                    },
                    counties869: {
                        region_id: "869",
                        region_name: "雁山区"
                    },
                    counties870: {
                        region_id: "870",
                        region_name: "阳朔县"
                    },
                    counties871: {
                        region_id: "871",
                        region_name: "临桂县"
                    },
                    counties872: {
                        region_id: "872",
                        region_name: "灵川县"
                    },
                    counties873: {
                        region_id: "873",
                        region_name: "全州县"
                    },
                    counties874: {
                        region_id: "874",
                        region_name: "平乐县"
                    },
                    counties875: {
                        region_id: "875",
                        region_name: "兴安县"
                    },
                    counties876: {
                        region_id: "876",
                        region_name: "灌阳县"
                    },
                    counties877: {
                        region_id: "877",
                        region_name: "荔浦县"
                    },
                    counties878: {
                        region_id: "878",
                        region_name: "资源县"
                    },
                    counties879: {
                        region_id: "879",
                        region_name: "永福县"
                    },
                    counties880: {
                        region_id: "880",
                        region_name: "龙胜"
                    },
                    counties881: {
                        region_id: "881",
                        region_name: "恭城"
                    }
                }
            },
            cities99: {
                region_id: "99",
                region_name: "百色",
                county: {
                    counties882: {
                        region_id: "882",
                        region_name: "右江区"
                    },
                    counties883: {
                        region_id: "883",
                        region_name: "凌云县"
                    },
                    counties884: {
                        region_id: "884",
                        region_name: "平果县"
                    },
                    counties885: {
                        region_id: "885",
                        region_name: "西林县"
                    },
                    counties886: {
                        region_id: "886",
                        region_name: "乐业县"
                    },
                    counties887: {
                        region_id: "887",
                        region_name: "德保县"
                    },
                    counties888: {
                        region_id: "888",
                        region_name: "田林县"
                    },
                    counties889: {
                        region_id: "889",
                        region_name: "田阳县"
                    },
                    counties890: {
                        region_id: "890",
                        region_name: "靖西县"
                    },
                    counties891: {
                        region_id: "891",
                        region_name: "田东县"
                    },
                    counties892: {
                        region_id: "892",
                        region_name: "那坡县"
                    },
                    counties893: {
                        region_id: "893",
                        region_name: "隆林"
                    }
                }
            },
            cities100: {
                region_id: "100",
                region_name: "北海",
                county: {
                    counties894: {
                        region_id: "894",
                        region_name: "海城区"
                    },
                    counties895: {
                        region_id: "895",
                        region_name: "银海区"
                    },
                    counties896: {
                        region_id: "896",
                        region_name: "铁山港区"
                    },
                    counties897: {
                        region_id: "897",
                        region_name: "合浦县"
                    }
                }
            },
            cities101: {
                region_id: "101",
                region_name: "崇左",
                county: {
                    counties898: {
                        region_id: "898",
                        region_name: "江州区"
                    },
                    counties899: {
                        region_id: "899",
                        region_name: "凭祥市"
                    },
                    counties900: {
                        region_id: "900",
                        region_name: "宁明县"
                    },
                    counties901: {
                        region_id: "901",
                        region_name: "扶绥县"
                    },
                    counties902: {
                        region_id: "902",
                        region_name: "龙州县"
                    },
                    counties903: {
                        region_id: "903",
                        region_name: "大新县"
                    },
                    counties904: {
                        region_id: "904",
                        region_name: "天等县"
                    }
                }
            },
            cities102: {
                region_id: "102",
                region_name: "防城港",
                county: {
                    counties905: {
                        region_id: "905",
                        region_name: "港口区"
                    },
                    counties906: {
                        region_id: "906",
                        region_name: "防城区"
                    },
                    counties907: {
                        region_id: "907",
                        region_name: "东兴市"
                    },
                    counties908: {
                        region_id: "908",
                        region_name: "上思县"
                    }
                }
            },
            cities103: {
                region_id: "103",
                region_name: "贵港",
                county: {
                    counties909: {
                        region_id: "909",
                        region_name: "港北区"
                    },
                    counties910: {
                        region_id: "910",
                        region_name: "港南区"
                    },
                    counties911: {
                        region_id: "911",
                        region_name: "覃塘区"
                    },
                    counties912: {
                        region_id: "912",
                        region_name: "桂平市"
                    },
                    counties913: {
                        region_id: "913",
                        region_name: "平南县"
                    }
                }
            },
            cities104: {
                region_id: "104",
                region_name: "河池",
                county: {
                    counties914: {
                        region_id: "914",
                        region_name: "金城江区"
                    },
                    counties915: {
                        region_id: "915",
                        region_name: "宜州市"
                    },
                    counties916: {
                        region_id: "916",
                        region_name: "天峨县"
                    },
                    counties917: {
                        region_id: "917",
                        region_name: "凤山县"
                    },
                    counties918: {
                        region_id: "918",
                        region_name: "南丹县"
                    },
                    counties919: {
                        region_id: "919",
                        region_name: "东兰县"
                    },
                    counties920: {
                        region_id: "920",
                        region_name: "都安"
                    },
                    counties921: {
                        region_id: "921",
                        region_name: "罗城"
                    },
                    counties922: {
                        region_id: "922",
                        region_name: "巴马"
                    },
                    counties923: {
                        region_id: "923",
                        region_name: "环江"
                    },
                    counties924: {
                        region_id: "924",
                        region_name: "大化"
                    }
                }
            },
            cities105: {
                region_id: "105",
                region_name: "贺州",
                county: {
                    counties925: {
                        region_id: "925",
                        region_name: "八步区"
                    },
                    counties926: {
                        region_id: "926",
                        region_name: "钟山县"
                    },
                    counties927: {
                        region_id: "927",
                        region_name: "昭平县"
                    },
                    counties928: {
                        region_id: "928",
                        region_name: "富川"
                    }
                }
            },
            cities106: {
                region_id: "106",
                region_name: "来宾",
                county: {
                    counties929: {
                        region_id: "929",
                        region_name: "兴宾区"
                    },
                    counties930: {
                        region_id: "930",
                        region_name: "合山市"
                    },
                    counties931: {
                        region_id: "931",
                        region_name: "象州县"
                    },
                    counties932: {
                        region_id: "932",
                        region_name: "武宣县"
                    },
                    counties933: {
                        region_id: "933",
                        region_name: "忻城县"
                    },
                    counties934: {
                        region_id: "934",
                        region_name: "金秀"
                    }
                }
            },
            cities107: {
                region_id: "107",
                region_name: "柳州",
                county: {
                    counties935: {
                        region_id: "935",
                        region_name: "城中区"
                    },
                    counties936: {
                        region_id: "936",
                        region_name: "鱼峰区"
                    },
                    counties937: {
                        region_id: "937",
                        region_name: "柳北区"
                    },
                    counties938: {
                        region_id: "938",
                        region_name: "柳南区"
                    },
                    counties939: {
                        region_id: "939",
                        region_name: "柳江县"
                    },
                    counties940: {
                        region_id: "940",
                        region_name: "柳城县"
                    },
                    counties941: {
                        region_id: "941",
                        region_name: "鹿寨县"
                    },
                    counties942: {
                        region_id: "942",
                        region_name: "融安县"
                    },
                    counties943: {
                        region_id: "943",
                        region_name: "融水"
                    },
                    counties944: {
                        region_id: "944",
                        region_name: "三江"
                    }
                }
            },
            cities108: {
                region_id: "108",
                region_name: "钦州",
                county: {
                    counties945: {
                        region_id: "945",
                        region_name: "钦南区"
                    },
                    counties946: {
                        region_id: "946",
                        region_name: "钦北区"
                    },
                    counties947: {
                        region_id: "947",
                        region_name: "灵山县"
                    },
                    counties948: {
                        region_id: "948",
                        region_name: "浦北县"
                    }
                }
            },
            cities109: {
                region_id: "109",
                region_name: "梧州",
                county: {
                    counties949: {
                        region_id: "949",
                        region_name: "万秀区"
                    },
                    counties950: {
                        region_id: "950",
                        region_name: "蝶山区"
                    },
                    counties951: {
                        region_id: "951",
                        region_name: "长洲区"
                    },
                    counties952: {
                        region_id: "952",
                        region_name: "岑溪市"
                    },
                    counties953: {
                        region_id: "953",
                        region_name: "苍梧县"
                    },
                    counties954: {
                        region_id: "954",
                        region_name: "藤县"
                    },
                    counties955: {
                        region_id: "955",
                        region_name: "蒙山县"
                    }
                }
            },
            cities110: {
                region_id: "110",
                region_name: "玉林",
                county: {
                    counties956: {
                        region_id: "956",
                        region_name: "玉州区"
                    },
                    counties957: {
                        region_id: "957",
                        region_name: "北流市"
                    },
                    counties958: {
                        region_id: "958",
                        region_name: "容县"
                    },
                    counties959: {
                        region_id: "959",
                        region_name: "陆川县"
                    },
                    counties960: {
                        region_id: "960",
                        region_name: "博白县"
                    },
                    counties961: {
                        region_id: "961",
                        region_name: "兴业县"
                    }
                }
            }
        }
    },
    provinces8: {
        region_id: "8",
        region_name: "贵州",
        city: {
            cities111: {
                region_id: "111",
                region_name: "贵阳",
                county: {
                    counties962: {
                        region_id: "962",
                        region_name: "南明区"
                    },
                    counties963: {
                        region_id: "963",
                        region_name: "云岩区"
                    },
                    counties964: {
                        region_id: "964",
                        region_name: "花溪区"
                    },
                    counties965: {
                        region_id: "965",
                        region_name: "乌当区"
                    },
                    counties966: {
                        region_id: "966",
                        region_name: "白云区"
                    },
                    counties967: {
                        region_id: "967",
                        region_name: "小河区"
                    },
                    counties968: {
                        region_id: "968",
                        region_name: "金阳新区"
                    },
                    counties969: {
                        region_id: "969",
                        region_name: "新天园区"
                    },
                    counties970: {
                        region_id: "970",
                        region_name: "清镇市"
                    },
                    counties971: {
                        region_id: "971",
                        region_name: "开阳县"
                    },
                    counties972: {
                        region_id: "972",
                        region_name: "修文县"
                    },
                    counties973: {
                        region_id: "973",
                        region_name: "息烽县"
                    }
                }
            },
            cities112: {
                region_id: "112",
                region_name: "安顺",
                county: {
                    counties974: {
                        region_id: "974",
                        region_name: "西秀区"
                    },
                    counties975: {
                        region_id: "975",
                        region_name: "关岭"
                    },
                    counties976: {
                        region_id: "976",
                        region_name: "镇宁"
                    },
                    counties977: {
                        region_id: "977",
                        region_name: "紫云"
                    },
                    counties978: {
                        region_id: "978",
                        region_name: "平坝县"
                    },
                    counties979: {
                        region_id: "979",
                        region_name: "普定县"
                    }
                }
            },
            cities113: {
                region_id: "113",
                region_name: "毕节",
                county: {
                    counties980: {
                        region_id: "980",
                        region_name: "毕节市"
                    },
                    counties981: {
                        region_id: "981",
                        region_name: "大方县"
                    },
                    counties982: {
                        region_id: "982",
                        region_name: "黔西县"
                    },
                    counties983: {
                        region_id: "983",
                        region_name: "金沙县"
                    },
                    counties984: {
                        region_id: "984",
                        region_name: "织金县"
                    },
                    counties985: {
                        region_id: "985",
                        region_name: "纳雍县"
                    },
                    counties986: {
                        region_id: "986",
                        region_name: "赫章县"
                    },
                    counties987: {
                        region_id: "987",
                        region_name: "威宁"
                    }
                }
            },
            cities114: {
                region_id: "114",
                region_name: "六盘水",
                county: {
                    counties988: {
                        region_id: "988",
                        region_name: "钟山区"
                    },
                    counties989: {
                        region_id: "989",
                        region_name: "六枝特区"
                    },
                    counties990: {
                        region_id: "990",
                        region_name: "水城县"
                    },
                    counties991: {
                        region_id: "991",
                        region_name: "盘县"
                    }
                }
            },
            cities115: {
                region_id: "115",
                region_name: "黔东南",
                county: {
                    counties992: {
                        region_id: "992",
                        region_name: "凯里市"
                    },
                    counties993: {
                        region_id: "993",
                        region_name: "黄平县"
                    },
                    counties994: {
                        region_id: "994",
                        region_name: "施秉县"
                    },
                    counties995: {
                        region_id: "995",
                        region_name: "三穗县"
                    },
                    counties996: {
                        region_id: "996",
                        region_name: "镇远县"
                    },
                    counties997: {
                        region_id: "997",
                        region_name: "岑巩县"
                    },
                    counties998: {
                        region_id: "998",
                        region_name: "天柱县"
                    },
                    counties999: {
                        region_id: "999",
                        region_name: "锦屏县"
                    },
                    counties1000: {
                        region_id: "1000",
                        region_name: "剑河县"
                    },
                    counties1001: {
                        region_id: "1001",
                        region_name: "台江县"
                    },
                    counties1002: {
                        region_id: "1002",
                        region_name: "黎平县"
                    },
                    counties1003: {
                        region_id: "1003",
                        region_name: "榕江县"
                    },
                    counties1004: {
                        region_id: "1004",
                        region_name: "从江县"
                    },
                    counties1005: {
                        region_id: "1005",
                        region_name: "雷山县"
                    },
                    counties1006: {
                        region_id: "1006",
                        region_name: "麻江县"
                    },
                    counties1007: {
                        region_id: "1007",
                        region_name: "丹寨县"
                    }
                }
            },
            cities116: {
                region_id: "116",
                region_name: "黔南",
                county: {
                    counties1008: {
                        region_id: "1008",
                        region_name: "都匀市"
                    },
                    counties1009: {
                        region_id: "1009",
                        region_name: "福泉市"
                    },
                    counties1010: {
                        region_id: "1010",
                        region_name: "荔波县"
                    },
                    counties1011: {
                        region_id: "1011",
                        region_name: "贵定县"
                    },
                    counties1012: {
                        region_id: "1012",
                        region_name: "瓮安县"
                    },
                    counties1013: {
                        region_id: "1013",
                        region_name: "独山县"
                    },
                    counties1014: {
                        region_id: "1014",
                        region_name: "平塘县"
                    },
                    counties1015: {
                        region_id: "1015",
                        region_name: "罗甸县"
                    },
                    counties1016: {
                        region_id: "1016",
                        region_name: "长顺县"
                    },
                    counties1017: {
                        region_id: "1017",
                        region_name: "龙里县"
                    },
                    counties1018: {
                        region_id: "1018",
                        region_name: "惠水县"
                    },
                    counties1019: {
                        region_id: "1019",
                        region_name: "三都"
                    }
                }
            },
            cities117: {
                region_id: "117",
                region_name: "黔西南",
                county: {
                    counties1020: {
                        region_id: "1020",
                        region_name: "兴义市"
                    },
                    counties1021: {
                        region_id: "1021",
                        region_name: "兴仁县"
                    },
                    counties1022: {
                        region_id: "1022",
                        region_name: "普安县"
                    },
                    counties1023: {
                        region_id: "1023",
                        region_name: "晴隆县"
                    },
                    counties1024: {
                        region_id: "1024",
                        region_name: "贞丰县"
                    },
                    counties1025: {
                        region_id: "1025",
                        region_name: "望谟县"
                    },
                    counties1026: {
                        region_id: "1026",
                        region_name: "册亨县"
                    },
                    counties1027: {
                        region_id: "1027",
                        region_name: "安龙县"
                    }
                }
            },
            cities118: {
                region_id: "118",
                region_name: "铜仁",
                county: {
                    counties1028: {
                        region_id: "1028",
                        region_name: "铜仁市"
                    },
                    counties1029: {
                        region_id: "1029",
                        region_name: "江口县"
                    },
                    counties1030: {
                        region_id: "1030",
                        region_name: "石阡县"
                    },
                    counties1031: {
                        region_id: "1031",
                        region_name: "思南县"
                    },
                    counties1032: {
                        region_id: "1032",
                        region_name: "德江县"
                    },
                    counties1033: {
                        region_id: "1033",
                        region_name: "玉屏"
                    },
                    counties1034: {
                        region_id: "1034",
                        region_name: "印江"
                    },
                    counties1035: {
                        region_id: "1035",
                        region_name: "沿河"
                    },
                    counties1036: {
                        region_id: "1036",
                        region_name: "松桃"
                    },
                    counties1037: {
                        region_id: "1037",
                        region_name: "万山特区"
                    }
                }
            },
            cities119: {
                region_id: "119",
                region_name: "遵义",
                county: {
                    counties1038: {
                        region_id: "1038",
                        region_name: "红花岗区"
                    },
                    counties1039: {
                        region_id: "1039",
                        region_name: "务川县"
                    },
                    counties1040: {
                        region_id: "1040",
                        region_name: "道真县"
                    },
                    counties1041: {
                        region_id: "1041",
                        region_name: "汇川区"
                    },
                    counties1042: {
                        region_id: "1042",
                        region_name: "赤水市"
                    },
                    counties1043: {
                        region_id: "1043",
                        region_name: "仁怀市"
                    },
                    counties1044: {
                        region_id: "1044",
                        region_name: "遵义县"
                    },
                    counties1045: {
                        region_id: "1045",
                        region_name: "桐梓县"
                    },
                    counties1046: {
                        region_id: "1046",
                        region_name: "绥阳县"
                    },
                    counties1047: {
                        region_id: "1047",
                        region_name: "正安县"
                    },
                    counties1048: {
                        region_id: "1048",
                        region_name: "凤冈县"
                    },
                    counties1049: {
                        region_id: "1049",
                        region_name: "湄潭县"
                    },
                    counties1050: {
                        region_id: "1050",
                        region_name: "余庆县"
                    },
                    counties1051: {
                        region_id: "1051",
                        region_name: "习水县"
                    },
                    counties1052: {
                        region_id: "1052",
                        region_name: "道真"
                    },
                    counties1053: {
                        region_id: "1053",
                        region_name: "务川"
                    }
                }
            }
        }
    },
    provinces9: {
        region_id: "9",
        region_name: "海南",
        city: {
            cities120: {
                region_id: "120",
                region_name: "海口",
                county: {
                    counties1054: {
                        region_id: "1054",
                        region_name: "秀英区"
                    },
                    counties1055: {
                        region_id: "1055",
                        region_name: "龙华区"
                    },
                    counties1056: {
                        region_id: "1056",
                        region_name: "琼山区"
                    },
                    counties1057: {
                        region_id: "1057",
                        region_name: "美兰区"
                    }
                }
            },
            cities121: {
                region_id: "121",
                region_name: "三亚",
                county: {
                    counties3409: {
                        region_id: "3409",
                        region_name: "海棠区"
                    },
                    counties3410: {
                        region_id: "3410",
                        region_name: "吉阳区"
                    },
                    counties3411: {
                        region_id: "3411",
                        region_name: "天涯区"
                    },
                    counties3412: {
                        region_id: "3412",
                        region_name: "崖州区"
                    }
                }
            },
            cities122: {
                region_id: "122",
                region_name: "白沙",
                county: {
                    counties3413: {
                        region_id: "3413",
                        region_name: "白沙"
                    }
                }
            },
            cities123: {
                region_id: "123",
                region_name: "保亭",
                county: {
                    counties3414: {
                        region_id: "3414",
                        region_name: "保亭"
                    }
                }
            },
            cities124: {
                region_id: "124",
                region_name: "昌江",
                county: {
                    counties3415: {
                        region_id: "3415",
                        region_name: "昌江"
                    }
                }
            },
            cities125: {
                region_id: "125",
                region_name: "澄迈县",
                county: {
                    counties3416: {
                        region_id: "3416",
                        region_name: "澄迈县"
                    }
                }
            },
            cities126: {
                region_id: "126",
                region_name: "定安县",
                county: {
                    counties3417: {
                        region_id: "3417",
                        region_name: "定安县"
                    }
                }
            },
            cities127: {
                region_id: "127",
                region_name: "东方",
                county: {
                    counties3418: {
                        region_id: "3418",
                        region_name: "东方"
                    }
                }
            },
            cities128: {
                region_id: "128",
                region_name: "乐东",
                county: {
                    counties3419: {
                        region_id: "3419",
                        region_name: "乐东"
                    }
                }
            },
            cities129: {
                region_id: "129",
                region_name: "临高县",
                county: {
                    counties3420: {
                        region_id: "3420",
                        region_name: "临高县"
                    }
                }
            },
            cities130: {
                region_id: "130",
                region_name: "陵水",
                county: {
                    counties3421: {
                        region_id: "3421",
                        region_name: "陵水"
                    }
                }
            },
            cities131: {
                region_id: "131",
                region_name: "琼海",
                county: {
                    counties3422: {
                        region_id: "3422",
                        region_name: "琼海"
                    }
                }
            },
            cities132: {
                region_id: "132",
                region_name: "琼中",
                county: {
                    counties3423: {
                        region_id: "3423",
                        region_name: "琼中"
                    }
                }
            },
            cities133: {
                region_id: "133",
                region_name: "屯昌县",
                county: {
                    counties3424: {
                        region_id: "3424",
                        region_name: "屯昌县"
                    }
                }
            },
            cities134: {
                region_id: "134",
                region_name: "万宁",
                county: {
                    counties3425: {
                        region_id: "3424",
                        region_name: "万宁"
                    }
                }
            },
            cities135: {
                region_id: "135",
                region_name: "文昌",
                county: {
                    counties3426: {
                        region_id: "3426",
                        region_name: "文昌"
                    }
                }
            },
            cities136: {
                region_id: "136",
                region_name: "五指山",
                county: {
                    counties3427: {
                        region_id: "3427",
                        region_name: "五指山"
                    }
                }
            },
            cities137: {
                region_id: "137",
                region_name: "儋州",
                county: {
                    counties1058: {
                        region_id: "1058",
                        region_name: "市区"
                    },
                    counties1059: {
                        region_id: "1059",
                        region_name: "洋浦开发区"
                    },
                    counties1060: {
                        region_id: "1060",
                        region_name: "那大镇"
                    },
                    counties1061: {
                        region_id: "1061",
                        region_name: "王五镇"
                    },
                    counties1062: {
                        region_id: "1062",
                        region_name: "雅星镇"
                    },
                    counties1063: {
                        region_id: "1063",
                        region_name: "大成镇"
                    },
                    counties1064: {
                        region_id: "1064",
                        region_name: "中和镇"
                    },
                    counties1065: {
                        region_id: "1065",
                        region_name: "峨蔓镇"
                    },
                    counties1066: {
                        region_id: "1066",
                        region_name: "南丰镇"
                    },
                    counties1067: {
                        region_id: "1067",
                        region_name: "白马井镇"
                    },
                    counties1068: {
                        region_id: "1068",
                        region_name: "兰洋镇"
                    },
                    counties1069: {
                        region_id: "1069",
                        region_name: "和庆镇"
                    },
                    counties1070: {
                        region_id: "1070",
                        region_name: "海头镇"
                    },
                    counties1071: {
                        region_id: "1071",
                        region_name: "排浦镇"
                    },
                    counties1072: {
                        region_id: "1072",
                        region_name: "东成镇"
                    },
                    counties1073: {
                        region_id: "1073",
                        region_name: "光村镇"
                    },
                    counties1074: {
                        region_id: "1074",
                        region_name: "木棠镇"
                    },
                    counties1075: {
                        region_id: "1075",
                        region_name: "新州镇"
                    },
                    counties1076: {
                        region_id: "1076",
                        region_name: "三都镇"
                    },
                    counties1077: {
                        region_id: "1077",
                        region_name: "其他"
                    }
                }
            },
            cities3432: {
                region_id: "3432",
                region_name: "西南中沙",
                county : {
                    counties3433: {
                        region_id: "3433",
                        region_name: "西南中沙"
                    }
                }
            }
        }
    },
    provinces10: {
        region_id: "10",
        region_name: "河北",
        city: {
            cities138: {
                region_id: "138",
                region_name: "石家庄",
                county: {
                    counties1078: {
                        region_id: "1078",
                        region_name: "长安区"
                    },
                    counties1079: {
                        region_id: "1079",
                        region_name: "桥东区"
                    },
                    counties1080: {
                        region_id: "1080",
                        region_name: "桥西区"
                    },
                    counties1081: {
                        region_id: "1081",
                        region_name: "新华区"
                    },
                    counties1082: {
                        region_id: "1082",
                        region_name: "裕华区"
                    },
                    counties1083: {
                        region_id: "1083",
                        region_name: "井陉矿区"
                    },
                    counties1084: {
                        region_id: "1084",
                        region_name: "高新区"
                    },
                    counties1085: {
                        region_id: "1085",
                        region_name: "辛集市"
                    },
                    counties1086: {
                        region_id: "1086",
                        region_name: "藁城市"
                    },
                    counties1087: {
                        region_id: "1087",
                        region_name: "晋州市"
                    },
                    counties1088: {
                        region_id: "1088",
                        region_name: "新乐市"
                    },
                    counties1089: {
                        region_id: "1089",
                        region_name: "鹿泉市"
                    },
                    counties1090: {
                        region_id: "1090",
                        region_name: "井陉县"
                    },
                    counties1091: {
                        region_id: "1091",
                        region_name: "正定县"
                    },
                    counties1092: {
                        region_id: "1092",
                        region_name: "栾城县"
                    },
                    counties1093: {
                        region_id: "1093",
                        region_name: "行唐县"
                    },
                    counties1094: {
                        region_id: "1094",
                        region_name: "灵寿县"
                    },
                    counties1095: {
                        region_id: "1095",
                        region_name: "高邑县"
                    },
                    counties1096: {
                        region_id: "1096",
                        region_name: "深泽县"
                    },
                    counties1097: {
                        region_id: "1097",
                        region_name: "赞皇县"
                    },
                    counties1098: {
                        region_id: "1098",
                        region_name: "无极县"
                    },
                    counties1099: {
                        region_id: "1099",
                        region_name: "平山县"
                    },
                    counties1100: {
                        region_id: "1100",
                        region_name: "元氏县"
                    },
                    counties1101: {
                        region_id: "1101",
                        region_name: "赵县"
                    }
                }
            },
            cities139: {
                region_id: "139",
                region_name: "保定",
                county: {
                    counties1102: {
                        region_id: "1102",
                        region_name: "新市区"
                    },
                    counties1103: {
                        region_id: "1103",
                        region_name: "南市区"
                    },
                    counties1104: {
                        region_id: "1104",
                        region_name: "北市区"
                    },
                    counties1105: {
                        region_id: "1105",
                        region_name: "涿州市"
                    },
                    counties1106: {
                        region_id: "1106",
                        region_name: "定州市"
                    },
                    counties1107: {
                        region_id: "1107",
                        region_name: "安国市"
                    },
                    counties1108: {
                        region_id: "1108",
                        region_name: "高碑店市"
                    },
                    counties1109: {
                        region_id: "1109",
                        region_name: "满城县"
                    },
                    counties1110: {
                        region_id: "1110",
                        region_name: "清苑县"
                    },
                    counties1111: {
                        region_id: "1111",
                        region_name: "涞水县"
                    },
                    counties1112: {
                        region_id: "1112",
                        region_name: "阜平县"
                    },
                    counties1113: {
                        region_id: "1113",
                        region_name: "徐水县"
                    },
                    counties1114: {
                        region_id: "1114",
                        region_name: "定兴县"
                    },
                    counties1115: {
                        region_id: "1115",
                        region_name: "唐县"
                    },
                    counties1116: {
                        region_id: "1116",
                        region_name: "高阳县"
                    },
                    counties1117: {
                        region_id: "1117",
                        region_name: "容城县"
                    },
                    counties1118: {
                        region_id: "1118",
                        region_name: "涞源县"
                    },
                    counties1119: {
                        region_id: "1119",
                        region_name: "望都县"
                    },
                    counties1120: {
                        region_id: "1120",
                        region_name: "安新县"
                    },
                    counties1121: {
                        region_id: "1121",
                        region_name: "易县"
                    },
                    counties1122: {
                        region_id: "1122",
                        region_name: "曲阳县"
                    },
                    counties1123: {
                        region_id: "1123",
                        region_name: "蠡县"
                    },
                    counties1124: {
                        region_id: "1124",
                        region_name: "顺平县"
                    },
                    counties1125: {
                        region_id: "1125",
                        region_name: "博野县"
                    },
                    counties1126: {
                        region_id: "1126",
                        region_name: "雄县"
                    }
                }
            },
            cities140: {
                region_id: "140",
                region_name: "沧州",
                county: {
                    counties1127: {
                        region_id: "1127",
                        region_name: "运河区"
                    },
                    counties1128: {
                        region_id: "1128",
                        region_name: "新华区"
                    },
                    counties1129: {
                        region_id: "1129",
                        region_name: "泊头市"
                    },
                    counties1130: {
                        region_id: "1130",
                        region_name: "任丘市"
                    },
                    counties1131: {
                        region_id: "1131",
                        region_name: "黄骅市"
                    },
                    counties1132: {
                        region_id: "1132",
                        region_name: "河间市"
                    },
                    counties1133: {
                        region_id: "1133",
                        region_name: "沧县"
                    },
                    counties1134: {
                        region_id: "1134",
                        region_name: "青县"
                    },
                    counties1135: {
                        region_id: "1135",
                        region_name: "东光县"
                    },
                    counties1136: {
                        region_id: "1136",
                        region_name: "海兴县"
                    },
                    counties1137: {
                        region_id: "1137",
                        region_name: "盐山县"
                    },
                    counties1138: {
                        region_id: "1138",
                        region_name: "肃宁县"
                    },
                    counties1139: {
                        region_id: "1139",
                        region_name: "南皮县"
                    },
                    counties1140: {
                        region_id: "1140",
                        region_name: "吴桥县"
                    },
                    counties1141: {
                        region_id: "1141",
                        region_name: "献县"
                    },
                    counties1142: {
                        region_id: "1142",
                        region_name: "孟村"
                    }
                }
            },
            cities141: {
                region_id: "141",
                region_name: "承德",
                county: {
                    counties1143: {
                        region_id: "1143",
                        region_name: "双桥区"
                    },
                    counties1144: {
                        region_id: "1144",
                        region_name: "双滦区"
                    },
                    counties1145: {
                        region_id: "1145",
                        region_name: "鹰手营子矿区"
                    },
                    counties1146: {
                        region_id: "1146",
                        region_name: "承德县"
                    },
                    counties1147: {
                        region_id: "1147",
                        region_name: "兴隆县"
                    },
                    counties1148: {
                        region_id: "1148",
                        region_name: "平泉县"
                    },
                    counties1149: {
                        region_id: "1149",
                        region_name: "滦平县"
                    },
                    counties1150: {
                        region_id: "1150",
                        region_name: "隆化县"
                    },
                    counties1151: {
                        region_id: "1151",
                        region_name: "丰宁"
                    },
                    counties1152: {
                        region_id: "1152",
                        region_name: "宽城"
                    },
                    counties1153: {
                        region_id: "1153",
                        region_name: "围场"
                    }
                }
            },
            cities142: {
                region_id: "142",
                region_name: "邯郸",
                county: {
                    counties1154: {
                        region_id: "1154",
                        region_name: "从台区"
                    },
                    counties1155: {
                        region_id: "1155",
                        region_name: "复兴区"
                    },
                    counties1156: {
                        region_id: "1156",
                        region_name: "邯山区"
                    },
                    counties1157: {
                        region_id: "1157",
                        region_name: "峰峰矿区"
                    },
                    counties1158: {
                        region_id: "1158",
                        region_name: "武安市"
                    },
                    counties1159: {
                        region_id: "1159",
                        region_name: "邯郸县"
                    },
                    counties1160: {
                        region_id: "1160",
                        region_name: "临漳县"
                    },
                    counties1161: {
                        region_id: "1161",
                        region_name: "成安县"
                    },
                    counties1162: {
                        region_id: "1162",
                        region_name: "大名县"
                    },
                    counties1163: {
                        region_id: "1163",
                        region_name: "涉县"
                    },
                    counties1164: {
                        region_id: "1164",
                        region_name: "磁县"
                    },
                    counties1165: {
                        region_id: "1165",
                        region_name: "肥乡县"
                    },
                    counties1166: {
                        region_id: "1166",
                        region_name: "永年县"
                    },
                    counties1167: {
                        region_id: "1167",
                        region_name: "邱县"
                    },
                    counties1168: {
                        region_id: "1168",
                        region_name: "鸡泽县"
                    },
                    counties1169: {
                        region_id: "1169",
                        region_name: "广平县"
                    },
                    counties1170: {
                        region_id: "1170",
                        region_name: "馆陶县"
                    },
                    counties1171: {
                        region_id: "1171",
                        region_name: "魏县"
                    },
                    counties1172: {
                        region_id: "1172",
                        region_name: "曲周县"
                    }
                }
            },
            cities143: {
                region_id: "143",
                region_name: "衡水",
                county: {
                    counties1173: {
                        region_id: "1173",
                        region_name: "桃城区"
                    },
                    counties1174: {
                        region_id: "1174",
                        region_name: "冀州市"
                    },
                    counties1175: {
                        region_id: "1175",
                        region_name: "深州市"
                    },
                    counties1176: {
                        region_id: "1176",
                        region_name: "枣强县"
                    },
                    counties1177: {
                        region_id: "1177",
                        region_name: "武邑县"
                    },
                    counties1178: {
                        region_id: "1178",
                        region_name: "武强县"
                    },
                    counties1179: {
                        region_id: "1179",
                        region_name: "饶阳县"
                    },
                    counties1180: {
                        region_id: "1180",
                        region_name: "安平县"
                    },
                    counties1181: {
                        region_id: "1181",
                        region_name: "故城县"
                    },
                    counties1182: {
                        region_id: "1182",
                        region_name: "景县"
                    },
                    counties1183: {
                        region_id: "1183",
                        region_name: "阜城县"
                    }
                }
            },
            cities144: {
                region_id: "144",
                region_name: "廊坊",
                county: {
                    counties1184: {
                        region_id: "1184",
                        region_name: "安次区"
                    },
                    counties1185: {
                        region_id: "1185",
                        region_name: "广阳区"
                    },
                    counties1186: {
                        region_id: "1186",
                        region_name: "霸州市"
                    },
                    counties1187: {
                        region_id: "1187",
                        region_name: "三河市"
                    },
                    counties1188: {
                        region_id: "1188",
                        region_name: "固安县"
                    },
                    counties1189: {
                        region_id: "1189",
                        region_name: "永清县"
                    },
                    counties1190: {
                        region_id: "1190",
                        region_name: "香河县"
                    },
                    counties1191: {
                        region_id: "1191",
                        region_name: "大城县"
                    },
                    counties1192: {
                        region_id: "1192",
                        region_name: "文安县"
                    },
                    counties1193: {
                        region_id: "1193",
                        region_name: "大厂"
                    }
                }
            },
            cities145: {
                region_id: "145",
                region_name: "秦皇岛",
                county: {
                    counties1194: {
                        region_id: "1194",
                        region_name: "海港区"
                    },
                    counties1195: {
                        region_id: "1195",
                        region_name: "山海关区"
                    },
                    counties1196: {
                        region_id: "1196",
                        region_name: "北戴河区"
                    },
                    counties1197: {
                        region_id: "1197",
                        region_name: "昌黎县"
                    },
                    counties1198: {
                        region_id: "1198",
                        region_name: "抚宁县"
                    },
                    counties1199: {
                        region_id: "1199",
                        region_name: "卢龙县"
                    },
                    counties1200: {
                        region_id: "1200",
                        region_name: "青龙"
                    }
                }
            },
            cities146: {
                region_id: "146",
                region_name: "唐山",
                county: {
                    counties1201: {
                        region_id: "1201",
                        region_name: "路北区"
                    },
                    counties1202: {
                        region_id: "1202",
                        region_name: "路南区"
                    },
                    counties1203: {
                        region_id: "1203",
                        region_name: "古冶区"
                    },
                    counties1204: {
                        region_id: "1204",
                        region_name: "开平区"
                    },
                    counties1205: {
                        region_id: "1205",
                        region_name: "丰南区"
                    },
                    counties1206: {
                        region_id: "1206",
                        region_name: "丰润区"
                    },
                    counties1207: {
                        region_id: "1207",
                        region_name: "遵化市"
                    },
                    counties1208: {
                        region_id: "1208",
                        region_name: "迁安市"
                    },
                    counties1209: {
                        region_id: "1209",
                        region_name: "滦县"
                    },
                    counties1210: {
                        region_id: "1210",
                        region_name: "滦南县"
                    },
                    counties1211: {
                        region_id: "1211",
                        region_name: "乐亭县"
                    },
                    counties1212: {
                        region_id: "1212",
                        region_name: "迁西县"
                    },
                    counties1213: {
                        region_id: "1213",
                        region_name: "玉田县"
                    },
                    counties1214: {
                        region_id: "1214",
                        region_name: "唐海县"
                    }
                }
            },
            cities147: {
                region_id: "147",
                region_name: "邢台",
                county: {
                    counties1215: {
                        region_id: "1215",
                        region_name: "桥东区"
                    },
                    counties1216: {
                        region_id: "1216",
                        region_name: "桥西区"
                    },
                    counties1217: {
                        region_id: "1217",
                        region_name: "南宫市"
                    },
                    counties1218: {
                        region_id: "1218",
                        region_name: "沙河市"
                    },
                    counties1219: {
                        region_id: "1219",
                        region_name: "邢台县"
                    },
                    counties1220: {
                        region_id: "1220",
                        region_name: "临城县"
                    },
                    counties1221: {
                        region_id: "1221",
                        region_name: "内丘县"
                    },
                    counties1222: {
                        region_id: "1222",
                        region_name: "柏乡县"
                    },
                    counties1223: {
                        region_id: "1223",
                        region_name: "隆尧县"
                    },
                    counties1224: {
                        region_id: "1224",
                        region_name: "任县"
                    },
                    counties1225: {
                        region_id: "1225",
                        region_name: "南和县"
                    },
                    counties1226: {
                        region_id: "1226",
                        region_name: "宁晋县"
                    },
                    counties1227: {
                        region_id: "1227",
                        region_name: "巨鹿县"
                    },
                    counties1228: {
                        region_id: "1228",
                        region_name: "新河县"
                    },
                    counties1229: {
                        region_id: "1229",
                        region_name: "广宗县"
                    },
                    counties1230: {
                        region_id: "1230",
                        region_name: "平乡县"
                    },
                    counties1231: {
                        region_id: "1231",
                        region_name: "威县"
                    },
                    counties1232: {
                        region_id: "1232",
                        region_name: "清河县"
                    },
                    counties1233: {
                        region_id: "1233",
                        region_name: "临西县"
                    }
                }
            },
            cities148: {
                region_id: "148",
                region_name: "张家口",
                county: {
                    counties1234: {
                        region_id: "1234",
                        region_name: "桥西区"
                    },
                    counties1235: {
                        region_id: "1235",
                        region_name: "桥东区"
                    },
                    counties1236: {
                        region_id: "1236",
                        region_name: "宣化区"
                    },
                    counties1237: {
                        region_id: "1237",
                        region_name: "下花园区"
                    },
                    counties1238: {
                        region_id: "1238",
                        region_name: "宣化县"
                    },
                    counties1239: {
                        region_id: "1239",
                        region_name: "张北县"
                    },
                    counties1240: {
                        region_id: "1240",
                        region_name: "康保县"
                    },
                    counties1241: {
                        region_id: "1241",
                        region_name: "沽源县"
                    },
                    counties1242: {
                        region_id: "1242",
                        region_name: "尚义县"
                    },
                    counties1243: {
                        region_id: "1243",
                        region_name: "蔚县"
                    },
                    counties1244: {
                        region_id: "1244",
                        region_name: "阳原县"
                    },
                    counties1245: {
                        region_id: "1245",
                        region_name: "怀安县"
                    },
                    counties1246: {
                        region_id: "1246",
                        region_name: "万全县"
                    },
                    counties1247: {
                        region_id: "1247",
                        region_name: "怀来县"
                    },
                    counties1248: {
                        region_id: "1248",
                        region_name: "涿鹿县"
                    },
                    counties1249: {
                        region_id: "1249",
                        region_name: "赤城县"
                    },
                    counties1250: {
                        region_id: "1250",
                        region_name: "崇礼县"
                    }
                }
            }
        }
    },
    provinces11: {
        region_id: "11",
        region_name: "河南",
        city: {
            cities149: {
                region_id: "149",
                region_name: "郑州",
                county: {
                    counties1251: {
                        region_id: "1251",
                        region_name: "金水区"
                    },
                    counties1252: {
                        region_id: "1252",
                        region_name: "邙山区"
                    },
                    counties1253: {
                        region_id: "1253",
                        region_name: "二七区"
                    },
                    counties1254: {
                        region_id: "1254",
                        region_name: "管城区"
                    },
                    counties1255: {
                        region_id: "1255",
                        region_name: "中原区"
                    },
                    counties1256: {
                        region_id: "1256",
                        region_name: "上街区"
                    },
                    counties1257: {
                        region_id: "1257",
                        region_name: "惠济区"
                    },
                    counties1258: {
                        region_id: "1258",
                        region_name: "郑东新区"
                    },
                    counties1259: {
                        region_id: "1259",
                        region_name: "经济技术开发区"
                    },
                    counties1260: {
                        region_id: "1260",
                        region_name: "高新开发区"
                    },
                    counties1261: {
                        region_id: "1261",
                        region_name: "出口加工区"
                    },
                    counties1262: {
                        region_id: "1262",
                        region_name: "巩义市"
                    },
                    counties1263: {
                        region_id: "1263",
                        region_name: "荥阳市"
                    },
                    counties1264: {
                        region_id: "1264",
                        region_name: "新密市"
                    },
                    counties1265: {
                        region_id: "1265",
                        region_name: "新郑市"
                    },
                    counties1266: {
                        region_id: "1266",
                        region_name: "登封市"
                    },
                    counties1267: {
                        region_id: "1267",
                        region_name: "中牟县"
                    }
                }
            },
            cities150: {
                region_id: "150",
                region_name: "洛阳",
                county: {
                    counties1268: {
                        region_id: "1268",
                        region_name: "西工区"
                    },
                    counties1269: {
                        region_id: "1269",
                        region_name: "老城区"
                    },
                    counties1270: {
                        region_id: "1270",
                        region_name: "涧西区"
                    },
                    counties1271: {
                        region_id: "1271",
                        region_name: "瀍河回族区"
                    },
                    counties1272: {
                        region_id: "1272",
                        region_name: "洛龙区"
                    },
                    counties1273: {
                        region_id: "1273",
                        region_name: "吉利区"
                    },
                    counties1274: {
                        region_id: "1274",
                        region_name: "偃师市"
                    },
                    counties1275: {
                        region_id: "1275",
                        region_name: "孟津县"
                    },
                    counties1276: {
                        region_id: "1276",
                        region_name: "新安县"
                    },
                    counties1277: {
                        region_id: "1277",
                        region_name: "栾川县"
                    },
                    counties1278: {
                        region_id: "1278",
                        region_name: "嵩县"
                    },
                    counties1279: {
                        region_id: "1279",
                        region_name: "汝阳县"
                    },
                    counties1280: {
                        region_id: "1280",
                        region_name: "宜阳县"
                    },
                    counties1281: {
                        region_id: "1281",
                        region_name: "洛宁县"
                    },
                    counties1282: {
                        region_id: "1282",
                        region_name: "伊川县"
                    }
                }
            },
            cities151: {
                region_id: "151",
                region_name: "开封",
                county: {
                    counties1283: {
                        region_id: "1283",
                        region_name: "鼓楼区"
                    },
                    counties1284: {
                        region_id: "1284",
                        region_name: "龙亭区"
                    },
                    counties1285: {
                        region_id: "1285",
                        region_name: "顺河回族区"
                    },
                    counties1286: {
                        region_id: "1286",
                        region_name: "金明区"
                    },
                    counties1287: {
                        region_id: "1287",
                        region_name: "禹王台区"
                    },
                    counties1288: {
                        region_id: "1288",
                        region_name: "杞县"
                    },
                    counties1289: {
                        region_id: "1289",
                        region_name: "通许县"
                    },
                    counties1290: {
                        region_id: "1290",
                        region_name: "尉氏县"
                    },
                    counties1291: {
                        region_id: "1291",
                        region_name: "开封县"
                    },
                    counties1292: {
                        region_id: "1292",
                        region_name: "兰考县"
                    }
                }
            },
            cities152: {
                region_id: "152",
                region_name: "安阳",
                county: {
                    counties1293: {
                        region_id: "1293",
                        region_name: "北关区"
                    },
                    counties1294: {
                        region_id: "1294",
                        region_name: "文峰区"
                    },
                    counties1295: {
                        region_id: "1295",
                        region_name: "殷都区"
                    },
                    counties1296: {
                        region_id: "1296",
                        region_name: "龙安区"
                    },
                    counties1297: {
                        region_id: "1297",
                        region_name: "林州市"
                    },
                    counties1298: {
                        region_id: "1298",
                        region_name: "安阳县"
                    },
                    counties1299: {
                        region_id: "1299",
                        region_name: "汤阴县"
                    },
                    counties1300: {
                        region_id: "1300",
                        region_name: "滑县"
                    },
                    counties1301: {
                        region_id: "1301",
                        region_name: "内黄县"
                    }
                }
            },
            cities153: {
                region_id: "153",
                region_name: "鹤壁",
                county: {
                    counties1302: {
                        region_id: "1302",
                        region_name: "淇滨区"
                    },
                    counties1303: {
                        region_id: "1303",
                        region_name: "山城区"
                    },
                    counties1304: {
                        region_id: "1304",
                        region_name: "鹤山区"
                    },
                    counties1305: {
                        region_id: "1305",
                        region_name: "浚县"
                    },
                    counties1306: {
                        region_id: "1306",
                        region_name: "淇县"
                    }
                }
            },
            cities154: {
                region_id: "154",
                region_name: "济源",
                county: {
                    counties1307: {
                        region_id: "1307",
                        region_name: "济源市"
                    }
                }
            },
            cities155: {
                region_id: "155",
                region_name: "焦作",
                county: {
                    counties1308: {
                        region_id: "1308",
                        region_name: "解放区"
                    },
                    counties1309: {
                        region_id: "1309",
                        region_name: "中站区"
                    },
                    counties1310: {
                        region_id: "1310",
                        region_name: "马村区"
                    },
                    counties1311: {
                        region_id: "1311",
                        region_name: "山阳区"
                    },
                    counties1312: {
                        region_id: "1312",
                        region_name: "沁阳市"
                    },
                    counties1313: {
                        region_id: "1313",
                        region_name: "孟州市"
                    },
                    counties1314: {
                        region_id: "1314",
                        region_name: "修武县"
                    },
                    counties1315: {
                        region_id: "1315",
                        region_name: "博爱县"
                    },
                    counties1316: {
                        region_id: "1316",
                        region_name: "武陟县"
                    },
                    counties1317: {
                        region_id: "1317",
                        region_name: "温县"
                    }
                }
            },
            cities156: {
                region_id: "156",
                region_name: "南阳",
                county: {
                    counties1318: {
                        region_id: "1318",
                        region_name: "卧龙区"
                    },
                    counties1319: {
                        region_id: "1319",
                        region_name: "宛城区"
                    },
                    counties1320: {
                        region_id: "1320",
                        region_name: "邓州市"
                    },
                    counties1321: {
                        region_id: "1321",
                        region_name: "南召县"
                    },
                    counties1322: {
                        region_id: "1322",
                        region_name: "方城县"
                    },
                    counties1323: {
                        region_id: "1323",
                        region_name: "西峡县"
                    },
                    counties1324: {
                        region_id: "1324",
                        region_name: "镇平县"
                    },
                    counties1325: {
                        region_id: "1325",
                        region_name: "内乡县"
                    },
                    counties1326: {
                        region_id: "1326",
                        region_name: "淅川县"
                    },
                    counties1327: {
                        region_id: "1327",
                        region_name: "社旗县"
                    },
                    counties1328: {
                        region_id: "1328",
                        region_name: "唐河县"
                    },
                    counties1329: {
                        region_id: "1329",
                        region_name: "新野县"
                    },
                    counties1330: {
                        region_id: "1330",
                        region_name: "桐柏县"
                    }
                }
            },
            cities157: {
                region_id: "157",
                region_name: "平顶山",
                county: {
                    counties1331: {
                        region_id: "1331",
                        region_name: "新华区"
                    },
                    counties1332: {
                        region_id: "1332",
                        region_name: "卫东区"
                    },
                    counties1333: {
                        region_id: "1333",
                        region_name: "湛河区"
                    },
                    counties1334: {
                        region_id: "1334",
                        region_name: "石龙区"
                    },
                    counties1335: {
                        region_id: "1335",
                        region_name: "舞钢市"
                    },
                    counties1336: {
                        region_id: "1336",
                        region_name: "汝州市"
                    },
                    counties1337: {
                        region_id: "1337",
                        region_name: "宝丰县"
                    },
                    counties1338: {
                        region_id: "1338",
                        region_name: "叶县"
                    },
                    counties1339: {
                        region_id: "1339",
                        region_name: "鲁山县"
                    },
                    counties1340: {
                        region_id: "1340",
                        region_name: "郏县"
                    }
                }
            },
            cities158: {
                region_id: "158",
                region_name: "三门峡",
                county: {
                    counties1341: {
                        region_id: "1341",
                        region_name: "湖滨区"
                    },
                    counties1342: {
                        region_id: "1342",
                        region_name: "义马市"
                    },
                    counties1343: {
                        region_id: "1343",
                        region_name: "灵宝市"
                    },
                    counties1344: {
                        region_id: "1344",
                        region_name: "渑池县"
                    },
                    counties1345: {
                        region_id: "1345",
                        region_name: "陕县"
                    },
                    counties1346: {
                        region_id: "1346",
                        region_name: "卢氏县"
                    }
                }
            },
            cities159: {
                region_id: "159",
                region_name: "商丘",
                county: {
                    counties1347: {
                        region_id: "1347",
                        region_name: "梁园区"
                    },
                    counties1348: {
                        region_id: "1348",
                        region_name: "睢阳区"
                    },
                    counties1349: {
                        region_id: "1349",
                        region_name: "永城市"
                    },
                    counties1350: {
                        region_id: "1350",
                        region_name: "民权县"
                    },
                    counties1351: {
                        region_id: "1351",
                        region_name: "睢县"
                    },
                    counties1352: {
                        region_id: "1352",
                        region_name: "宁陵县"
                    },
                    counties1353: {
                        region_id: "1353",
                        region_name: "虞城县"
                    },
                    counties1354: {
                        region_id: "1354",
                        region_name: "柘城县"
                    },
                    counties1355: {
                        region_id: "1355",
                        region_name: "夏邑县"
                    }
                }
            },
            cities160: {
                region_id: "160",
                region_name: "新乡",
                county: {
                    counties1356: {
                        region_id: "1356",
                        region_name: "卫滨区"
                    },
                    counties1357: {
                        region_id: "1357",
                        region_name: "红旗区"
                    },
                    counties1358: {
                        region_id: "1358",
                        region_name: "凤泉区"
                    },
                    counties1359: {
                        region_id: "1359",
                        region_name: "牧野区"
                    },
                    counties1360: {
                        region_id: "1360",
                        region_name: "卫辉市"
                    },
                    counties1361: {
                        region_id: "1361",
                        region_name: "辉县市"
                    },
                    counties1362: {
                        region_id: "1362",
                        region_name: "新乡县"
                    },
                    counties1363: {
                        region_id: "1363",
                        region_name: "获嘉县"
                    },
                    counties1364: {
                        region_id: "1364",
                        region_name: "原阳县"
                    },
                    counties1365: {
                        region_id: "1365",
                        region_name: "延津县"
                    },
                    counties1366: {
                        region_id: "1366",
                        region_name: "封丘县"
                    },
                    counties1367: {
                        region_id: "1367",
                        region_name: "长垣县"
                    }
                }
            },
            cities161: {
                region_id: "161",
                region_name: "信阳",
                county: {
                    counties1368: {
                        region_id: "1368",
                        region_name: "浉河区"
                    },
                    counties1369: {
                        region_id: "1369",
                        region_name: "平桥区"
                    },
                    counties1370: {
                        region_id: "1370",
                        region_name: "罗山县"
                    },
                    counties1371: {
                        region_id: "1371",
                        region_name: "光山县"
                    },
                    counties1372: {
                        region_id: "1372",
                        region_name: "新县"
                    },
                    counties1373: {
                        region_id: "1373",
                        region_name: "商城县"
                    },
                    counties1374: {
                        region_id: "1374",
                        region_name: "固始县"
                    },
                    counties1375: {
                        region_id: "1375",
                        region_name: "潢川县"
                    },
                    counties1376: {
                        region_id: "1376",
                        region_name: "淮滨县"
                    },
                    counties1377: {
                        region_id: "1377",
                        region_name: "息县"
                    }
                }
            },
            cities162: {
                region_id: "162",
                region_name: "许昌",
                county: {
                    counties1378: {
                        region_id: "1378",
                        region_name: "魏都区"
                    },
                    counties1379: {
                        region_id: "1379",
                        region_name: "禹州市"
                    },
                    counties1380: {
                        region_id: "1380",
                        region_name: "长葛市"
                    },
                    counties1381: {
                        region_id: "1381",
                        region_name: "许昌县"
                    },
                    counties1382: {
                        region_id: "1382",
                        region_name: "鄢陵县"
                    },
                    counties1383: {
                        region_id: "1383",
                        region_name: "襄城县"
                    }
                }
            },
            cities163: {
                region_id: "163",
                region_name: "周口",
                county: {
                    counties1384: {
                        region_id: "1384",
                        region_name: "川汇区"
                    },
                    counties1385: {
                        region_id: "1385",
                        region_name: "项城市"
                    },
                    counties1386: {
                        region_id: "1386",
                        region_name: "扶沟县"
                    },
                    counties1387: {
                        region_id: "1387",
                        region_name: "西华县"
                    },
                    counties1388: {
                        region_id: "1388",
                        region_name: "商水县"
                    },
                    counties1389: {
                        region_id: "1389",
                        region_name: "沈丘县"
                    },
                    counties1390: {
                        region_id: "1390",
                        region_name: "郸城县"
                    },
                    counties1391: {
                        region_id: "1391",
                        region_name: "淮阳县"
                    },
                    counties1392: {
                        region_id: "1392",
                        region_name: "太康县"
                    },
                    counties1393: {
                        region_id: "1393",
                        region_name: "鹿邑县"
                    }
                }
            },
            cities164: {
                region_id: "164",
                region_name: "驻马店",
                county: {
                    counties1394: {
                        region_id: "1394",
                        region_name: "驿城区"
                    },
                    counties1395: {
                        region_id: "1395",
                        region_name: "西平县"
                    },
                    counties1396: {
                        region_id: "1396",
                        region_name: "上蔡县"
                    },
                    counties1397: {
                        region_id: "1397",
                        region_name: "平舆县"
                    },
                    counties1398: {
                        region_id: "1398",
                        region_name: "正阳县"
                    },
                    counties1399: {
                        region_id: "1399",
                        region_name: "确山县"
                    },
                    counties1400: {
                        region_id: "1400",
                        region_name: "泌阳县"
                    },
                    counties1401: {
                        region_id: "1401",
                        region_name: "汝南县"
                    },
                    counties1402: {
                        region_id: "1402",
                        region_name: "遂平县"
                    },
                    counties1403: {
                        region_id: "1403",
                        region_name: "新蔡县"
                    }
                }
            },
            cities165: {
                region_id: "165",
                region_name: "漯河",
                county: {
                    counties1404: {
                        region_id: "1404",
                        region_name: "郾城区"
                    },
                    counties1405: {
                        region_id: "1405",
                        region_name: "源汇区"
                    },
                    counties1406: {
                        region_id: "1406",
                        region_name: "召陵区"
                    },
                    counties1407: {
                        region_id: "1407",
                        region_name: "舞阳县"
                    },
                    counties1408: {
                        region_id: "1408",
                        region_name: "临颍县"
                    }
                }
            },
            cities166: {
                region_id: "166",
                region_name: "濮阳",
                county: {
                    counties1409: {
                        region_id: "1409",
                        region_name: "华龙区"
                    },
                    counties1410: {
                        region_id: "1410",
                        region_name: "清丰县"
                    },
                    counties1411: {
                        region_id: "1411",
                        region_name: "南乐县"
                    },
                    counties1412: {
                        region_id: "1412",
                        region_name: "范县"
                    },
                    counties1413: {
                        region_id: "1413",
                        region_name: "台前县"
                    },
                    counties1414: {
                        region_id: "1414",
                        region_name: "濮阳县"
                    }
                }
            }
        }
    },
    provinces12: {
        region_id: "12",
        region_name: "黑龙江",
        city: {
            cities167: {
                region_id: "167",
                region_name: "哈尔滨",
                county: {
                    counties1415: {
                        region_id: "1415",
                        region_name: "道里区"
                    },
                    counties1416: {
                        region_id: "1416",
                        region_name: "南岗区"
                    },
                    counties1417: {
                        region_id: "1417",
                        region_name: "动力区"
                    },
                    counties1418: {
                        region_id: "1418",
                        region_name: "平房区"
                    },
                    counties1419: {
                        region_id: "1419",
                        region_name: "香坊区"
                    },
                    counties1420: {
                        region_id: "1420",
                        region_name: "太平区"
                    },
                    counties1421: {
                        region_id: "1421",
                        region_name: "道外区"
                    },
                    counties1422: {
                        region_id: "1422",
                        region_name: "阿城区"
                    },
                    counties1423: {
                        region_id: "1423",
                        region_name: "呼兰区"
                    },
                    counties1424: {
                        region_id: "1424",
                        region_name: "松北区"
                    },
                    counties1425: {
                        region_id: "1425",
                        region_name: "尚志市"
                    },
                    counties1426: {
                        region_id: "1426",
                        region_name: "双城市"
                    },
                    counties1427: {
                        region_id: "1427",
                        region_name: "五常市"
                    },
                    counties1428: {
                        region_id: "1428",
                        region_name: "方正县"
                    },
                    counties1429: {
                        region_id: "1429",
                        region_name: "宾县"
                    },
                    counties1430: {
                        region_id: "1430",
                        region_name: "依兰县"
                    },
                    counties1431: {
                        region_id: "1431",
                        region_name: "巴彦县"
                    },
                    counties1432: {
                        region_id: "1432",
                        region_name: "通河县"
                    },
                    counties1433: {
                        region_id: "1433",
                        region_name: "木兰县"
                    },
                    counties1434: {
                        region_id: "1434",
                        region_name: "延寿县"
                    }
                }
            },
            cities168: {
                region_id: "168",
                region_name: "大庆",
                county: {
                    counties1435: {
                        region_id: "1435",
                        region_name: "萨尔图区"
                    },
                    counties1436: {
                        region_id: "1436",
                        region_name: "红岗区"
                    },
                    counties1437: {
                        region_id: "1437",
                        region_name: "龙凤区"
                    },
                    counties1438: {
                        region_id: "1438",
                        region_name: "让胡路区"
                    },
                    counties1439: {
                        region_id: "1439",
                        region_name: "大同区"
                    },
                    counties1440: {
                        region_id: "1440",
                        region_name: "肇州县"
                    },
                    counties1441: {
                        region_id: "1441",
                        region_name: "肇源县"
                    },
                    counties1442: {
                        region_id: "1442",
                        region_name: "林甸县"
                    },
                    counties1443: {
                        region_id: "1443",
                        region_name: "杜尔伯特"
                    }
                }
            },
            cities169: {
                region_id: "169",
                region_name: "大兴安岭",
                county: {
                    counties1444: {
                        region_id: "1444",
                        region_name: "呼玛县"
                    },
                    counties1445: {
                        region_id: "1445",
                        region_name: "漠河县"
                    },
                    counties1446: {
                        region_id: "1446",
                        region_name: "塔河县"
                    }
                }
            },
            cities170: {
                region_id: "170",
                region_name: "鹤岗",
                county: {
                    counties1447: {
                        region_id: "1447",
                        region_name: "兴山区"
                    },
                    counties1448: {
                        region_id: "1448",
                        region_name: "工农区"
                    },
                    counties1449: {
                        region_id: "1449",
                        region_name: "南山区"
                    },
                    counties1450: {
                        region_id: "1450",
                        region_name: "兴安区"
                    },
                    counties1451: {
                        region_id: "1451",
                        region_name: "向阳区"
                    },
                    counties1452: {
                        region_id: "1452",
                        region_name: "东山区"
                    },
                    counties1453: {
                        region_id: "1453",
                        region_name: "萝北县"
                    },
                    counties1454: {
                        region_id: "1454",
                        region_name: "绥滨县"
                    }
                }
            },
            cities171: {
                region_id: "171",
                region_name: "黑河",
                county: {
                    counties1455: {
                        region_id: "1455",
                        region_name: "爱辉区"
                    },
                    counties1456: {
                        region_id: "1456",
                        region_name: "五大连池市"
                    },
                    counties1457: {
                        region_id: "1457",
                        region_name: "北安市"
                    },
                    counties1458: {
                        region_id: "1458",
                        region_name: "嫩江县"
                    },
                    counties1459: {
                        region_id: "1459",
                        region_name: "逊克县"
                    },
                    counties1460: {
                        region_id: "1460",
                        region_name: "孙吴县"
                    }
                }
            },
            cities172: {
                region_id: "172",
                region_name: "鸡西",
                county: {
                    counties1461: {
                        region_id: "1461",
                        region_name: "鸡冠区"
                    },
                    counties1462: {
                        region_id: "1462",
                        region_name: "恒山区"
                    },
                    counties1463: {
                        region_id: "1463",
                        region_name: "城子河区"
                    },
                    counties1464: {
                        region_id: "1464",
                        region_name: "滴道区"
                    },
                    counties1465: {
                        region_id: "1465",
                        region_name: "梨树区"
                    },
                    counties1466: {
                        region_id: "1466",
                        region_name: "虎林市"
                    },
                    counties1467: {
                        region_id: "1467",
                        region_name: "密山市"
                    },
                    counties1468: {
                        region_id: "1468",
                        region_name: "鸡东县"
                    }
                }
            },
            cities173: {
                region_id: "173",
                region_name: "佳木斯",
                county: {
                    counties1469: {
                        region_id: "1469",
                        region_name: "前进区"
                    },
                    counties1470: {
                        region_id: "1470",
                        region_name: "郊区"
                    },
                    counties1471: {
                        region_id: "1471",
                        region_name: "向阳区"
                    },
                    counties1472: {
                        region_id: "1472",
                        region_name: "东风区"
                    },
                    counties1473: {
                        region_id: "1473",
                        region_name: "同江市"
                    },
                    counties1474: {
                        region_id: "1474",
                        region_name: "富锦市"
                    },
                    counties1475: {
                        region_id: "1475",
                        region_name: "桦南县"
                    },
                    counties1476: {
                        region_id: "1476",
                        region_name: "桦川县"
                    },
                    counties1477: {
                        region_id: "1477",
                        region_name: "汤原县"
                    },
                    counties1478: {
                        region_id: "1478",
                        region_name: "抚远县"
                    }
                }
            },
            cities174: {
                region_id: "174",
                region_name: "牡丹江",
                county: {
                    counties1479: {
                        region_id: "1479",
                        region_name: "爱民区"
                    },
                    counties1480: {
                        region_id: "1480",
                        region_name: "东安区"
                    },
                    counties1481: {
                        region_id: "1481",
                        region_name: "阳明区"
                    },
                    counties1482: {
                        region_id: "1482",
                        region_name: "西安区"
                    },
                    counties1483: {
                        region_id: "1483",
                        region_name: "绥芬河市"
                    },
                    counties1484: {
                        region_id: "1484",
                        region_name: "海林市"
                    },
                    counties1485: {
                        region_id: "1485",
                        region_name: "宁安市"
                    },
                    counties1486: {
                        region_id: "1486",
                        region_name: "穆棱市"
                    },
                    counties1487: {
                        region_id: "1487",
                        region_name: "东宁县"
                    },
                    counties1488: {
                        region_id: "1488",
                        region_name: "林口县"
                    }
                }
            },
            cities175: {
                region_id: "175",
                region_name: "七台河",
                county: {
                    counties1489: {
                        region_id: "1489",
                        region_name: "桃山区"
                    },
                    counties1490: {
                        region_id: "1490",
                        region_name: "新兴区"
                    },
                    counties1491: {
                        region_id: "1491",
                        region_name: "茄子河区"
                    },
                    counties1492: {
                        region_id: "1492",
                        region_name: "勃利县"
                    }
                }
            },
            cities176: {
                region_id: "176",
                region_name: "齐齐哈尔",
                county: {
                    counties1493: {
                        region_id: "1493",
                        region_name: "龙沙区"
                    },
                    counties1494: {
                        region_id: "1494",
                        region_name: "昂昂溪区"
                    },
                    counties1495: {
                        region_id: "1495",
                        region_name: "铁峰区"
                    },
                    counties1496: {
                        region_id: "1496",
                        region_name: "建华区"
                    },
                    counties1497: {
                        region_id: "1497",
                        region_name: "富拉尔基区"
                    },
                    counties1498: {
                        region_id: "1498",
                        region_name: "碾子山区"
                    },
                    counties1499: {
                        region_id: "1499",
                        region_name: "梅里斯达斡尔区"
                    },
                    counties1500: {
                        region_id: "1500",
                        region_name: "讷河市"
                    },
                    counties1501: {
                        region_id: "1501",
                        region_name: "龙江县"
                    },
                    counties1502: {
                        region_id: "1502",
                        region_name: "依安县"
                    },
                    counties1503: {
                        region_id: "1503",
                        region_name: "泰来县"
                    },
                    counties1504: {
                        region_id: "1504",
                        region_name: "甘南县"
                    },
                    counties1505: {
                        region_id: "1505",
                        region_name: "富裕县"
                    },
                    counties1506: {
                        region_id: "1506",
                        region_name: "克山县"
                    },
                    counties1507: {
                        region_id: "1507",
                        region_name: "克东县"
                    },
                    counties1508: {
                        region_id: "1508",
                        region_name: "拜泉县"
                    }
                }
            },
            cities177: {
                region_id: "177",
                region_name: "双鸭山",
                county: {
                    counties1509: {
                        region_id: "1509",
                        region_name: "尖山区"
                    },
                    counties1510: {
                        region_id: "1510",
                        region_name: "岭东区"
                    },
                    counties1511: {
                        region_id: "1511",
                        region_name: "四方台区"
                    },
                    counties1512: {
                        region_id: "1512",
                        region_name: "宝山区"
                    },
                    counties1513: {
                        region_id: "1513",
                        region_name: "集贤县"
                    },
                    counties1514: {
                        region_id: "1514",
                        region_name: "友谊县"
                    },
                    counties1515: {
                        region_id: "1515",
                        region_name: "宝清县"
                    },
                    counties1516: {
                        region_id: "1516",
                        region_name: "饶河县"
                    }
                }
            },
            cities178: {
                region_id: "178",
                region_name: "绥化",
                county: {
                    counties1517: {
                        region_id: "1517",
                        region_name: "北林区"
                    },
                    counties1518: {
                        region_id: "1518",
                        region_name: "安达市"
                    },
                    counties1519: {
                        region_id: "1519",
                        region_name: "肇东市"
                    },
                    counties1520: {
                        region_id: "1520",
                        region_name: "海伦市"
                    },
                    counties1521: {
                        region_id: "1521",
                        region_name: "望奎县"
                    },
                    counties1522: {
                        region_id: "1522",
                        region_name: "兰西县"
                    },
                    counties1523: {
                        region_id: "1523",
                        region_name: "青冈县"
                    },
                    counties1524: {
                        region_id: "1524",
                        region_name: "庆安县"
                    },
                    counties1525: {
                        region_id: "1525",
                        region_name: "明水县"
                    },
                    counties1526: {
                        region_id: "1526",
                        region_name: "绥棱县"
                    }
                }
            },
            cities179: {
                region_id: "179",
                region_name: "伊春",
                county: {
                    counties1527: {
                        region_id: "1527",
                        region_name: "伊春区"
                    },
                    counties1528: {
                        region_id: "1528",
                        region_name: "带岭区"
                    },
                    counties1529: {
                        region_id: "1529",
                        region_name: "南岔区"
                    },
                    counties1530: {
                        region_id: "1530",
                        region_name: "金山屯区"
                    },
                    counties1531: {
                        region_id: "1531",
                        region_name: "西林区"
                    },
                    counties1532: {
                        region_id: "1532",
                        region_name: "美溪区"
                    },
                    counties1533: {
                        region_id: "1533",
                        region_name: "乌马河区"
                    },
                    counties1534: {
                        region_id: "1534",
                        region_name: "翠峦区"
                    },
                    counties1535: {
                        region_id: "1535",
                        region_name: "友好区"
                    },
                    counties1536: {
                        region_id: "1536",
                        region_name: "上甘岭区"
                    },
                    counties1537: {
                        region_id: "1537",
                        region_name: "五营区"
                    },
                    counties1538: {
                        region_id: "1538",
                        region_name: "红星区"
                    },
                    counties1539: {
                        region_id: "1539",
                        region_name: "新青区"
                    },
                    counties1540: {
                        region_id: "1540",
                        region_name: "汤旺河区"
                    },
                    counties1541: {
                        region_id: "1541",
                        region_name: "乌伊岭区"
                    },
                    counties1542: {
                        region_id: "1542",
                        region_name: "铁力市"
                    },
                    counties1543: {
                        region_id: "1543",
                        region_name: "嘉荫县"
                    }
                }
            }
        }
    },
    provinces13: {
        region_id: "13",
        region_name: "湖北",
        city: {
            cities180: {
                region_id: "180",
                region_name: "武汉",
                county: {
                    counties1544: {
                        region_id: "1544",
                        region_name: "江岸区"
                    },
                    counties1545: {
                        region_id: "1545",
                        region_name: "武昌区"
                    },
                    counties1546: {
                        region_id: "1546",
                        region_name: "江汉区"
                    },
                    counties1547: {
                        region_id: "1547",
                        region_name: "硚口区"
                    },
                    counties1548: {
                        region_id: "1548",
                        region_name: "汉阳区"
                    },
                    counties1549: {
                        region_id: "1549",
                        region_name: "青山区"
                    },
                    counties1550: {
                        region_id: "1550",
                        region_name: "洪山区"
                    },
                    counties1551: {
                        region_id: "1551",
                        region_name: "东西湖区"
                    },
                    counties1552: {
                        region_id: "1552",
                        region_name: "汉南区"
                    },
                    counties1553: {
                        region_id: "1553",
                        region_name: "蔡甸区"
                    },
                    counties1554: {
                        region_id: "1554",
                        region_name: "江夏区"
                    },
                    counties1555: {
                        region_id: "1555",
                        region_name: "黄陂区"
                    },
                    counties1556: {
                        region_id: "1556",
                        region_name: "新洲区"
                    },
                    counties1557: {
                        region_id: "1557",
                        region_name: "经济开发区"
                    }
                }
            },
            cities181: {
                region_id: "181",
                region_name: "仙桃",
                county: {
                    counties1558: {
                        region_id: "1558",
                        region_name: "仙桃市"
                    }
                }
            },
            cities182: {
                region_id: "182",
                region_name: "鄂州",
                county: {
                    counties1559: {
                        region_id: "1559",
                        region_name: "鄂城区"
                    },
                    counties1560: {
                        region_id: "1560",
                        region_name: "华容区"
                    },
                    counties1561: {
                        region_id: "1561",
                        region_name: "梁子湖区"
                    }
                }
            },
            cities183: {
                region_id: "183",
                region_name: "黄冈",
                county: {
                    counties1562: {
                        region_id: "1562",
                        region_name: "黄州区"
                    },
                    counties1563: {
                        region_id: "1563",
                        region_name: "麻城市"
                    },
                    counties1564: {
                        region_id: "1564",
                        region_name: "武穴市"
                    },
                    counties1565: {
                        region_id: "1565",
                        region_name: "团风县"
                    },
                    counties1566: {
                        region_id: "1566",
                        region_name: "红安县"
                    },
                    counties1567: {
                        region_id: "1567",
                        region_name: "罗田县"
                    },
                    counties1568: {
                        region_id: "1568",
                        region_name: "英山县"
                    },
                    counties1569: {
                        region_id: "1569",
                        region_name: "浠水县"
                    },
                    counties1570: {
                        region_id: "1570",
                        region_name: "蕲春县"
                    },
                    counties1571: {
                        region_id: "1571",
                        region_name: "黄梅县"
                    }
                }
            },
            cities184: {
                region_id: "184",
                region_name: "黄石",
                county: {
                    counties1572: {
                        region_id: "1572",
                        region_name: "黄石港区"
                    },
                    counties1573: {
                        region_id: "1573",
                        region_name: "西塞山区"
                    },
                    counties1574: {
                        region_id: "1574",
                        region_name: "下陆区"
                    },
                    counties1575: {
                        region_id: "1575",
                        region_name: "铁山区"
                    },
                    counties1576: {
                        region_id: "1576",
                        region_name: "大冶市"
                    },
                    counties1577: {
                        region_id: "1577",
                        region_name: "阳新县"
                    }
                }
            },
            cities185: {
                region_id: "185",
                region_name: "荆门",
                county: {
                    counties1578: {
                        region_id: "1578",
                        region_name: "东宝区"
                    },
                    counties1579: {
                        region_id: "1579",
                        region_name: "掇刀区"
                    },
                    counties1580: {
                        region_id: "1580",
                        region_name: "钟祥市"
                    },
                    counties1581: {
                        region_id: "1581",
                        region_name: "京山县"
                    },
                    counties1582: {
                        region_id: "1582",
                        region_name: "沙洋县"
                    }
                }
            },
            cities186: {
                region_id: "186",
                region_name: "荆州",
                county: {
                    counties1583: {
                        region_id: "1583",
                        region_name: "沙市区"
                    },
                    counties1584: {
                        region_id: "1584",
                        region_name: "荆州区"
                    },
                    counties1585: {
                        region_id: "1585",
                        region_name: "石首市"
                    },
                    counties1586: {
                        region_id: "1586",
                        region_name: "洪湖市"
                    },
                    counties1587: {
                        region_id: "1587",
                        region_name: "松滋市"
                    },
                    counties1588: {
                        region_id: "1588",
                        region_name: "公安县"
                    },
                    counties1589: {
                        region_id: "1589",
                        region_name: "监利县"
                    },
                    counties1590: {
                        region_id: "1590",
                        region_name: "江陵县"
                    }
                }
            },
            cities187: {
                region_id: "187",
                region_name: "潜江",
                county: {
                    counties1591: {
                        region_id: "1591",
                        region_name: "潜江市"
                    }
                }
            },
            cities188: {
                region_id: "188",
                region_name: "神农架",
                county: {
                    counties1592: {
                        region_id: "1592",
                        region_name: "神农架"
                    }
                }
            },
            cities189: {
                region_id: "189",
                region_name: "十堰",
                county: {
                    counties1593: {
                        region_id: "1593",
                        region_name: "张湾区"
                    },
                    counties1594: {
                        region_id: "1594",
                        region_name: "茅箭区"
                    },
                    counties1595: {
                        region_id: "1595",
                        region_name: "丹江口市"
                    },
                    counties1596: {
                        region_id: "1596",
                        region_name: "郧县"
                    },
                    counties1597: {
                        region_id: "1597",
                        region_name: "郧西县"
                    },
                    counties1598: {
                        region_id: "1598",
                        region_name: "竹山县"
                    },
                    counties1599: {
                        region_id: "1599",
                        region_name: "竹溪县"
                    },
                    counties1600: {
                        region_id: "1600",
                        region_name: "房县"
                    }
                }
            },
            cities191: {
                region_id: "191",
                region_name: "天门",
                county: {
                    counties1603: {
                        region_id: "1603",
                        region_name: "天门市"
                    }
                }
            },
            cities192: {
                region_id: "192",
                region_name: "咸宁",
                county: {
                    counties1604: {
                        region_id: "1604",
                        region_name: "咸安区"
                    },
                    counties1605: {
                        region_id: "1605",
                        region_name: "赤壁市"
                    },
                    counties1606: {
                        region_id: "1606",
                        region_name: "嘉鱼县"
                    },
                    counties1607: {
                        region_id: "1607",
                        region_name: "通城县"
                    },
                    counties1608: {
                        region_id: "1608",
                        region_name: "崇阳县"
                    },
                    counties1609: {
                        region_id: "1609",
                        region_name: "通山县"
                    }
                }
            },
            cities193: {
                region_id: "193",
                region_name: "襄樊",
                county: {
                    counties1610: {
                        region_id: "1610",
                        region_name: "襄城区"
                    },
                    counties1611: {
                        region_id: "1611",
                        region_name: "樊城区"
                    },
                    counties1612: {
                        region_id: "1612",
                        region_name: "襄阳区"
                    },
                    counties1613: {
                        region_id: "1613",
                        region_name: "老河口市"
                    },
                    counties1614: {
                        region_id: "1614",
                        region_name: "枣阳市"
                    },
                    counties1615: {
                        region_id: "1615",
                        region_name: "宜城市"
                    },
                    counties1616: {
                        region_id: "1616",
                        region_name: "南漳县"
                    },
                    counties1617: {
                        region_id: "1617",
                        region_name: "谷城县"
                    },
                    counties1618: {
                        region_id: "1618",
                        region_name: "保康县"
                    }
                }
            },
            cities194: {
                region_id: "194",
                region_name: "孝感",
                county: {
                    counties1619: {
                        region_id: "1619",
                        region_name: "孝南区"
                    },
                    counties1620: {
                        region_id: "1620",
                        region_name: "应城市"
                    },
                    counties1621: {
                        region_id: "1621",
                        region_name: "安陆市"
                    },
                    counties1622: {
                        region_id: "1622",
                        region_name: "汉川市"
                    },
                    counties1623: {
                        region_id: "1623",
                        region_name: "孝昌县"
                    },
                    counties1624: {
                        region_id: "1624",
                        region_name: "大悟县"
                    },
                    counties1625: {
                        region_id: "1625",
                        region_name: "云梦县"
                    }
                }
            },
            cities195: {
                region_id: "195",
                region_name: "宜昌",
                county: {
                    counties1626: {
                        region_id: "1626",
                        region_name: "长阳"
                    },
                    counties1627: {
                        region_id: "1627",
                        region_name: "五峰"
                    },
                    counties1628: {
                        region_id: "1628",
                        region_name: "西陵区"
                    },
                    counties1629: {
                        region_id: "1629",
                        region_name: "伍家岗区"
                    },
                    counties1630: {
                        region_id: "1630",
                        region_name: "点军区"
                    },
                    counties1631: {
                        region_id: "1631",
                        region_name: "猇亭区"
                    },
                    counties1632: {
                        region_id: "1632",
                        region_name: "夷陵区"
                    },
                    counties1633: {
                        region_id: "1633",
                        region_name: "宜都市"
                    },
                    counties1634: {
                        region_id: "1634",
                        region_name: "当阳市"
                    },
                    counties1635: {
                        region_id: "1635",
                        region_name: "枝江市"
                    },
                    counties1636: {
                        region_id: "1636",
                        region_name: "远安县"
                    },
                    counties1637: {
                        region_id: "1637",
                        region_name: "兴山县"
                    },
                    counties1638: {
                        region_id: "1638",
                        region_name: "秭归县"
                    }
                }
            },
            cities196: {
                region_id: "196",
                region_name: "恩施",
                county: {
                    counties1639: {
                        region_id: "1639",
                        region_name: "恩施市"
                    },
                    counties1640: {
                        region_id: "1640",
                        region_name: "利川市"
                    },
                    counties1641: {
                        region_id: "1641",
                        region_name: "建始县"
                    },
                    counties1642: {
                        region_id: "1642",
                        region_name: "巴东县"
                    },
                    counties1643: {
                        region_id: "1643",
                        region_name: "宣恩县"
                    },
                    counties1644: {
                        region_id: "1644",
                        region_name: "咸丰县"
                    },
                    counties1645: {
                        region_id: "1645",
                        region_name: "来凤县"
                    },
                    counties1646: {
                        region_id: "1646",
                        region_name: "鹤峰县"
                    }
                }
            }
        }
    },
    provinces14: {
        region_id: "14",
        region_name: "湖南",
        city: {
            cities197: {
                region_id: "197",
                region_name: "长沙",
                county: {
                    counties1647: {
                        region_id: "1647",
                        region_name: "岳麓区"
                    },
                    counties1648: {
                        region_id: "1648",
                        region_name: "芙蓉区"
                    },
                    counties1649: {
                        region_id: "1649",
                        region_name: "天心区"
                    },
                    counties1650: {
                        region_id: "1650",
                        region_name: "开福区"
                    },
                    counties1651: {
                        region_id: "1651",
                        region_name: "雨花区"
                    },
                    counties1652: {
                        region_id: "1652",
                        region_name: "开发区"
                    },
                    counties1653: {
                        region_id: "1653",
                        region_name: "浏阳市"
                    },
                    counties1654: {
                        region_id: "1654",
                        region_name: "长沙县"
                    },
                    counties1655: {
                        region_id: "1655",
                        region_name: "望城县"
                    },
                    counties1656: {
                        region_id: "1656",
                        region_name: "宁乡县"
                    }
                }
            },
            cities198: {
                region_id: "198",
                region_name: "张家界",
                county: {
                    counties1657: {
                        region_id: "1657",
                        region_name: "永定区"
                    },
                    counties1658: {
                        region_id: "1658",
                        region_name: "武陵源区"
                    },
                    counties1659: {
                        region_id: "1659",
                        region_name: "慈利县"
                    },
                    counties1660: {
                        region_id: "1660",
                        region_name: "桑植县"
                    }
                }
            },
            cities199: {
                region_id: "199",
                region_name: "常德",
                county: {
                    counties1661: {
                        region_id: "1661",
                        region_name: "武陵区"
                    },
                    counties1662: {
                        region_id: "1662",
                        region_name: "鼎城区"
                    },
                    counties1663: {
                        region_id: "1663",
                        region_name: "津市市"
                    },
                    counties1664: {
                        region_id: "1664",
                        region_name: "安乡县"
                    },
                    counties1665: {
                        region_id: "1665",
                        region_name: "汉寿县"
                    },
                    counties1666: {
                        region_id: "1666",
                        region_name: "澧县"
                    },
                    counties1667: {
                        region_id: "1667",
                        region_name: "临澧县"
                    },
                    counties1668: {
                        region_id: "1668",
                        region_name: "桃源县"
                    },
                    counties1669: {
                        region_id: "1669",
                        region_name: "石门县"
                    }
                }
            },
            cities200: {
                region_id: "200",
                region_name: "郴州",
                county: {
                    counties1670: {
                        region_id: "1670",
                        region_name: "北湖区"
                    },
                    counties1671: {
                        region_id: "1671",
                        region_name: "苏仙区"
                    },
                    counties1672: {
                        region_id: "1672",
                        region_name: "资兴市"
                    },
                    counties1673: {
                        region_id: "1673",
                        region_name: "桂阳县"
                    },
                    counties1674: {
                        region_id: "1674",
                        region_name: "宜章县"
                    },
                    counties1675: {
                        region_id: "1675",
                        region_name: "永兴县"
                    },
                    counties1676: {
                        region_id: "1676",
                        region_name: "嘉禾县"
                    },
                    counties1677: {
                        region_id: "1677",
                        region_name: "临武县"
                    },
                    counties1678: {
                        region_id: "1678",
                        region_name: "汝城县"
                    },
                    counties1679: {
                        region_id: "1679",
                        region_name: "桂东县"
                    },
                    counties1680: {
                        region_id: "1680",
                        region_name: "安仁县"
                    }
                }
            },
            cities201: {
                region_id: "201",
                region_name: "衡阳",
                county: {
                    counties1681: {
                        region_id: "1681",
                        region_name: "雁峰区"
                    },
                    counties1682: {
                        region_id: "1682",
                        region_name: "珠晖区"
                    },
                    counties1683: {
                        region_id: "1683",
                        region_name: "石鼓区"
                    },
                    counties1684: {
                        region_id: "1684",
                        region_name: "蒸湘区"
                    },
                    counties1685: {
                        region_id: "1685",
                        region_name: "南岳区"
                    },
                    counties1686: {
                        region_id: "1686",
                        region_name: "耒阳市"
                    },
                    counties1687: {
                        region_id: "1687",
                        region_name: "常宁市"
                    },
                    counties1688: {
                        region_id: "1688",
                        region_name: "衡阳县"
                    },
                    counties1689: {
                        region_id: "1689",
                        region_name: "衡南县"
                    },
                    counties1690: {
                        region_id: "1690",
                        region_name: "衡山县"
                    },
                    counties1691: {
                        region_id: "1691",
                        region_name: "衡东县"
                    },
                    counties1692: {
                        region_id: "1692",
                        region_name: "祁东县"
                    }
                }
            },
            cities202: {
                region_id: "202",
                region_name: "怀化",
                county: {
                    counties1693: {
                        region_id: "1693",
                        region_name: "鹤城区"
                    },
                    counties1694: {
                        region_id: "1694",
                        region_name: "靖州"
                    },
                    counties1695: {
                        region_id: "1695",
                        region_name: "麻阳"
                    },
                    counties1696: {
                        region_id: "1696",
                        region_name: "通道"
                    },
                    counties1697: {
                        region_id: "1697",
                        region_name: "新晃"
                    },
                    counties1698: {
                        region_id: "1698",
                        region_name: "芷江"
                    },
                    counties1699: {
                        region_id: "1699",
                        region_name: "沅陵县"
                    },
                    counties1700: {
                        region_id: "1700",
                        region_name: "辰溪县"
                    },
                    counties1701: {
                        region_id: "1701",
                        region_name: "溆浦县"
                    },
                    counties1702: {
                        region_id: "1702",
                        region_name: "中方县"
                    },
                    counties1703: {
                        region_id: "1703",
                        region_name: "会同县"
                    },
                    counties1704: {
                        region_id: "1704",
                        region_name: "洪江市"
                    }
                }
            },
            cities203: {
                region_id: "203",
                region_name: "娄底",
                county: {
                    counties1705: {
                        region_id: "1705",
                        region_name: "娄星区"
                    },
                    counties1706: {
                        region_id: "1706",
                        region_name: "冷水江市"
                    },
                    counties1707: {
                        region_id: "1707",
                        region_name: "涟源市"
                    },
                    counties1708: {
                        region_id: "1708",
                        region_name: "双峰县"
                    },
                    counties1709: {
                        region_id: "1709",
                        region_name: "新化县"
                    }
                }
            },
            cities204: {
                region_id: "204",
                region_name: "邵阳",
                county: {
                    counties1710: {
                        region_id: "1710",
                        region_name: "城步"
                    },
                    counties1711: {
                        region_id: "1711",
                        region_name: "双清区"
                    },
                    counties1712: {
                        region_id: "1712",
                        region_name: "大祥区"
                    },
                    counties1713: {
                        region_id: "1713",
                        region_name: "北塔区"
                    },
                    counties1714: {
                        region_id: "1714",
                        region_name: "武冈市"
                    },
                    counties1715: {
                        region_id: "1715",
                        region_name: "邵东县"
                    },
                    counties1716: {
                        region_id: "1716",
                        region_name: "新邵县"
                    },
                    counties1717: {
                        region_id: "1717",
                        region_name: "邵阳县"
                    },
                    counties1718: {
                        region_id: "1718",
                        region_name: "隆回县"
                    },
                    counties1719: {
                        region_id: "1719",
                        region_name: "洞口县"
                    },
                    counties1720: {
                        region_id: "1720",
                        region_name: "绥宁县"
                    },
                    counties1721: {
                        region_id: "1721",
                        region_name: "新宁县"
                    }
                }
            },
            cities205: {
                region_id: "205",
                region_name: "湘潭",
                county: {
                    counties1722: {
                        region_id: "1722",
                        region_name: "岳塘区"
                    },
                    counties1723: {
                        region_id: "1723",
                        region_name: "雨湖区"
                    },
                    counties1724: {
                        region_id: "1724",
                        region_name: "湘乡市"
                    },
                    counties1725: {
                        region_id: "1725",
                        region_name: "韶山市"
                    },
                    counties1726: {
                        region_id: "1726",
                        region_name: "湘潭县"
                    }
                }
            },
            cities206: {
                region_id: "206",
                region_name: "湘西土家族苗族自治州",
                county: {
                    counties1727: {
                        region_id: "1727",
                        region_name: "吉首市"
                    },
                    counties1728: {
                        region_id: "1728",
                        region_name: "泸溪县"
                    },
                    counties1729: {
                        region_id: "1729",
                        region_name: "凤凰县"
                    },
                    counties1730: {
                        region_id: "1730",
                        region_name: "花垣县"
                    },
                    counties1731: {
                        region_id: "1731",
                        region_name: "保靖县"
                    },
                    counties1732: {
                        region_id: "1732",
                        region_name: "古丈县"
                    },
                    counties1733: {
                        region_id: "1733",
                        region_name: "永顺县"
                    },
                    counties1734: {
                        region_id: "1734",
                        region_name: "龙山县"
                    }
                }
            },
            cities207: {
                region_id: "207",
                region_name: "益阳",
                county: {
                    counties1735: {
                        region_id: "1735",
                        region_name: "赫山区"
                    },
                    counties1736: {
                        region_id: "1736",
                        region_name: "资阳区"
                    },
                    counties1737: {
                        region_id: "1737",
                        region_name: "沅江市"
                    },
                    counties1738: {
                        region_id: "1738",
                        region_name: "南县"
                    },
                    counties1739: {
                        region_id: "1739",
                        region_name: "桃江县"
                    },
                    counties1740: {
                        region_id: "1740",
                        region_name: "安化县"
                    }
                }
            },
            cities208: {
                region_id: "208",
                region_name: "永州",
                county: {
                    counties1741: {
                        region_id: "1741",
                        region_name: "江华"
                    },
                    counties1742: {
                        region_id: "1742",
                        region_name: "冷水滩区"
                    },
                    counties1743: {
                        region_id: "1743",
                        region_name: "零陵区"
                    },
                    counties1744: {
                        region_id: "1744",
                        region_name: "祁阳县"
                    },
                    counties1745: {
                        region_id: "1745",
                        region_name: "东安县"
                    },
                    counties1746: {
                        region_id: "1746",
                        region_name: "双牌县"
                    },
                    counties1747: {
                        region_id: "1747",
                        region_name: "道县"
                    },
                    counties1748: {
                        region_id: "1748",
                        region_name: "江永县"
                    },
                    counties1749: {
                        region_id: "1749",
                        region_name: "宁远县"
                    },
                    counties1750: {
                        region_id: "1750",
                        region_name: "蓝山县"
                    },
                    counties1751: {
                        region_id: "1751",
                        region_name: "新田县"
                    }
                }
            },
            cities209: {
                region_id: "209",
                region_name: "岳阳",
                county: {
                    counties1752: {
                        region_id: "1752",
                        region_name: "岳阳楼区"
                    },
                    counties1753: {
                        region_id: "1753",
                        region_name: "君山区"
                    },
                    counties1754: {
                        region_id: "1754",
                        region_name: "云溪区"
                    },
                    counties1755: {
                        region_id: "1755",
                        region_name: "汨罗市"
                    },
                    counties1756: {
                        region_id: "1756",
                        region_name: "临湘市"
                    },
                    counties1757: {
                        region_id: "1757",
                        region_name: "岳阳县"
                    },
                    counties1758: {
                        region_id: "1758",
                        region_name: "华容县"
                    },
                    counties1759: {
                        region_id: "1759",
                        region_name: "湘阴县"
                    },
                    counties1760: {
                        region_id: "1760",
                        region_name: "平江县"
                    }
                }
            },
            cities210: {
                region_id: "210",
                region_name: "株洲",
                county: {
                    counties1761: {
                        region_id: "1761",
                        region_name: "天元区"
                    },
                    counties1762: {
                        region_id: "1762",
                        region_name: "荷塘区"
                    },
                    counties1763: {
                        region_id: "1763",
                        region_name: "芦淞区"
                    },
                    counties1764: {
                        region_id: "1764",
                        region_name: "石峰区"
                    },
                    counties1765: {
                        region_id: "1765",
                        region_name: "醴陵市"
                    },
                    counties1766: {
                        region_id: "1766",
                        region_name: "株洲县"
                    },
                    counties1767: {
                        region_id: "1767",
                        region_name: "攸县"
                    },
                    counties1768: {
                        region_id: "1768",
                        region_name: "茶陵县"
                    },
                    counties1769: {
                        region_id: "1769",
                        region_name: "炎陵县"
                    }
                }
            }
        }
    },
    provinces15: {
        region_id: "15",
        region_name: "吉林",
        city: {
            cities211: {
                region_id: "211",
                region_name: "长春",
                county: {
                    counties1770: {
                        region_id: "1770",
                        region_name: "朝阳区"
                    },
                    counties1771: {
                        region_id: "1771",
                        region_name: "宽城区"
                    },
                    counties1772: {
                        region_id: "1772",
                        region_name: "二道区"
                    },
                    counties1773: {
                        region_id: "1773",
                        region_name: "南关区"
                    },
                    counties1774: {
                        region_id: "1774",
                        region_name: "绿园区"
                    },
                    counties1775: {
                        region_id: "1775",
                        region_name: "双阳区"
                    },
                    counties1776: {
                        region_id: "1776",
                        region_name: "净月潭开发区"
                    },
                    counties1777: {
                        region_id: "1777",
                        region_name: "高新技术开发区"
                    },
                    counties1778: {
                        region_id: "1778",
                        region_name: "经济技术开发区"
                    },
                    counties1779: {
                        region_id: "1779",
                        region_name: "汽车产业开发区"
                    },
                    counties1780: {
                        region_id: "1780",
                        region_name: "德惠市"
                    },
                    counties1781: {
                        region_id: "1781",
                        region_name: "九台市"
                    },
                    counties1782: {
                        region_id: "1782",
                        region_name: "榆树市"
                    },
                    counties1783: {
                        region_id: "1783",
                        region_name: "农安县"
                    }
                }
            },
            cities212: {
                region_id: "212",
                region_name: "吉林",
                county: {
                    counties1784: {
                        region_id: "1784",
                        region_name: "船营区"
                    },
                    counties1785: {
                        region_id: "1785",
                        region_name: "昌邑区"
                    },
                    counties1786: {
                        region_id: "1786",
                        region_name: "龙潭区"
                    },
                    counties1787: {
                        region_id: "1787",
                        region_name: "丰满区"
                    },
                    counties1788: {
                        region_id: "1788",
                        region_name: "蛟河市"
                    },
                    counties1789: {
                        region_id: "1789",
                        region_name: "桦甸市"
                    },
                    counties1790: {
                        region_id: "1790",
                        region_name: "舒兰市"
                    },
                    counties1791: {
                        region_id: "1791",
                        region_name: "磐石市"
                    },
                    counties1792: {
                        region_id: "1792",
                        region_name: "永吉县"
                    }
                }
            },
            cities213: {
                region_id: "213",
                region_name: "白城",
                county: {
                    counties1793: {
                        region_id: "1793",
                        region_name: "洮北区"
                    },
                    counties1794: {
                        region_id: "1794",
                        region_name: "洮南市"
                    },
                    counties1795: {
                        region_id: "1795",
                        region_name: "大安市"
                    },
                    counties1796: {
                        region_id: "1796",
                        region_name: "镇赉县"
                    },
                    counties1797: {
                        region_id: "1797",
                        region_name: "通榆县"
                    }
                }
            },
            cities214: {
                region_id: "214",
                region_name: "白山",
                county: {
                    counties1798: {
                        region_id: "1798",
                        region_name: "江源区"
                    },
                    counties1799: {
                        region_id: "1799",
                        region_name: "八道江区"
                    },
                    counties1800: {
                        region_id: "1800",
                        region_name: "长白"
                    },
                    counties1801: {
                        region_id: "1801",
                        region_name: "临江市"
                    },
                    counties1802: {
                        region_id: "1802",
                        region_name: "抚松县"
                    },
                    counties1803: {
                        region_id: "1803",
                        region_name: "靖宇县"
                    }
                }
            },
            cities215: {
                region_id: "215",
                region_name: "辽源",
                county: {
                    counties1804: {
                        region_id: "1804",
                        region_name: "龙山区"
                    },
                    counties1805: {
                        region_id: "1805",
                        region_name: "西安区"
                    },
                    counties1806: {
                        region_id: "1806",
                        region_name: "东丰县"
                    },
                    counties1807: {
                        region_id: "1807",
                        region_name: "东辽县"
                    }
                }
            },
            cities216: {
                region_id: "216",
                region_name: "四平",
                county: {
                    counties1808: {
                        region_id: "1808",
                        region_name: "铁西区"
                    },
                    counties1809: {
                        region_id: "1809",
                        region_name: "铁东区"
                    },
                    counties1810: {
                        region_id: "1810",
                        region_name: "伊通"
                    },
                    counties1811: {
                        region_id: "1811",
                        region_name: "公主岭市"
                    },
                    counties1812: {
                        region_id: "1812",
                        region_name: "双辽市"
                    },
                    counties1813: {
                        region_id: "1813",
                        region_name: "梨树县"
                    }
                }
            },
            cities217: {
                region_id: "217",
                region_name: "松原",
                county: {
                    counties1814: {
                        region_id: "1814",
                        region_name: "前郭尔罗斯"
                    },
                    counties1815: {
                        region_id: "1815",
                        region_name: "宁江区"
                    },
                    counties1816: {
                        region_id: "1816",
                        region_name: "长岭县"
                    },
                    counties1817: {
                        region_id: "1817",
                        region_name: "乾安县"
                    },
                    counties1818: {
                        region_id: "1818",
                        region_name: "扶余县"
                    }
                }
            },
            cities218: {
                region_id: "218",
                region_name: "通化",
                county: {
                    counties1819: {
                        region_id: "1819",
                        region_name: "东昌区"
                    },
                    counties1820: {
                        region_id: "1820",
                        region_name: "二道江区"
                    },
                    counties1822: {
                        region_id: "1822",
                        region_name: "集安市"
                    },
                    counties1823: {
                        region_id: "1823",
                        region_name: "通化县"
                    },
                    counties1824: {
                        region_id: "1824",
                        region_name: "辉南县"
                    },
                    counties1825: {
                        region_id: "1825",
                        region_name: "柳河县"
                    }
                }
            },
            cities219: {
                region_id: "219",
                region_name: "延边自治州",
                county: {
                    counties1826: {
                        region_id: "1826",
                        region_name: "延吉市"
                    },
                    counties1827: {
                        region_id: "1827",
                        region_name: "图们市"
                    },
                    counties1828: {
                        region_id: "1828",
                        region_name: "敦化市"
                    },
                    counties1829: {
                        region_id: "1829",
                        region_name: "珲春市"
                    },
                    counties1830: {
                        region_id: "1830",
                        region_name: "龙井市"
                    },
                    counties1831: {
                        region_id: "1831",
                        region_name: "和龙市"
                    },
                    counties1832: {
                        region_id: "1832",
                        region_name: "安图县"
                    },
                    counties1833: {
                        region_id: "1833",
                        region_name: "汪清县"
                    }
                }
            },
            cities3428: {
                region_id: "3428",
                region_name: "梅河口",
                county : {
                    counties3429: {
                        region_id: "3429",
                        region_name: "梅河口"
                    }
                }
            },
            cities3430: {
                region_id: "3430",
                region_name: "长白山管委会",
                county : {
                    counties3431: {
                        region_id: "3431",
                        region_name: "长白山管委会"
                    }
                }
            }
        }
    },
    provinces16: {
        region_id: "16",
        region_name: "江苏",
        city: {
            cities220: {
                region_id: "220",
                region_name: "南京",
                county: {
                    counties1834: {
                        region_id: "1834",
                        region_name: "玄武区"
                    },
                    counties1835: {
                        region_id: "1835",
                        region_name: "鼓楼区"
                    },
                    counties1836: {
                        region_id: "1836",
                        region_name: "白下区"
                    },
                    counties1837: {
                        region_id: "1837",
                        region_name: "建邺区"
                    },
                    counties1838: {
                        region_id: "1838",
                        region_name: "秦淮区"
                    },
                    counties1839: {
                        region_id: "1839",
                        region_name: "雨花台区"
                    },
                    counties1840: {
                        region_id: "1840",
                        region_name: "下关区"
                    },
                    counties1841: {
                        region_id: "1841",
                        region_name: "栖霞区"
                    },
                    counties1842: {
                        region_id: "1842",
                        region_name: "浦口区"
                    },
                    counties1843: {
                        region_id: "1843",
                        region_name: "江宁区"
                    },
                    counties1844: {
                        region_id: "1844",
                        region_name: "六合区"
                    },
                    counties1845: {
                        region_id: "1845",
                        region_name: "溧水县"
                    },
                    counties1846: {
                        region_id: "1846",
                        region_name: "高淳县"
                    }
                }
            },
            cities221: {
                region_id: "221",
                region_name: "苏州",
                county: {
                    counties1847: {
                        region_id: "1847",
                        region_name: "沧浪区"
                    },
                    counties1848: {
                        region_id: "1848",
                        region_name: "金阊区"
                    },
                    counties1849: {
                        region_id: "1849",
                        region_name: "平江区"
                    },
                    counties1850: {
                        region_id: "1850",
                        region_name: "虎丘区"
                    },
                    counties1851: {
                        region_id: "1851",
                        region_name: "吴中区"
                    },
                    counties1852: {
                        region_id: "1852",
                        region_name: "相城区"
                    },
                    counties1853: {
                        region_id: "1853",
                        region_name: "园区"
                    },
                    counties1854: {
                        region_id: "1854",
                        region_name: "新区"
                    },
                    counties1855: {
                        region_id: "1855",
                        region_name: "常熟市"
                    },
                    counties1856: {
                        region_id: "1856",
                        region_name: "张家港市"
                    },
                    counties1857: {
                        region_id: "1857",
                        region_name: "玉山镇"
                    },
                    counties1858: {
                        region_id: "1858",
                        region_name: "巴城镇"
                    },
                    counties1859: {
                        region_id: "1859",
                        region_name: "周市镇"
                    },
                    counties1860: {
                        region_id: "1860",
                        region_name: "陆家镇"
                    },
                    counties1861: {
                        region_id: "1861",
                        region_name: "花桥镇"
                    },
                    counties1862: {
                        region_id: "1862",
                        region_name: "淀山湖镇"
                    },
                    counties1863: {
                        region_id: "1863",
                        region_name: "张浦镇"
                    },
                    counties1864: {
                        region_id: "1864",
                        region_name: "周庄镇"
                    },
                    counties1865: {
                        region_id: "1865",
                        region_name: "千灯镇"
                    },
                    counties1866: {
                        region_id: "1866",
                        region_name: "锦溪镇"
                    },
                    counties1867: {
                        region_id: "1867",
                        region_name: "开发区"
                    },
                    counties1868: {
                        region_id: "1868",
                        region_name: "吴江市"
                    },
                    counties1869: {
                        region_id: "1869",
                        region_name: "太仓市"
                    }
                }
            },
            cities222: {
                region_id: "222",
                region_name: "无锡",
                county: {
                    counties1870: {
                        region_id: "1870",
                        region_name: "崇安区"
                    },
                    counties1871: {
                        region_id: "1871",
                        region_name: "北塘区"
                    },
                    counties1872: {
                        region_id: "1872",
                        region_name: "南长区"
                    },
                    counties1873: {
                        region_id: "1873",
                        region_name: "锡山区"
                    },
                    counties1874: {
                        region_id: "1874",
                        region_name: "惠山区"
                    },
                    counties1875: {
                        region_id: "1875",
                        region_name: "滨湖区"
                    },
                    counties1876: {
                        region_id: "1876",
                        region_name: "新区"
                    },
                    counties1877: {
                        region_id: "1877",
                        region_name: "江阴市"
                    },
                    counties1878: {
                        region_id: "1878",
                        region_name: "宜兴市"
                    }
                }
            },
            cities223: {
                region_id: "223",
                region_name: "常州",
                county: {
                    counties1879: {
                        region_id: "1879",
                        region_name: "天宁区"
                    },
                    counties1880: {
                        region_id: "1880",
                        region_name: "钟楼区"
                    },
                    counties1881: {
                        region_id: "1881",
                        region_name: "戚墅堰区"
                    },
                    counties1882: {
                        region_id: "1882",
                        region_name: "郊区"
                    },
                    counties1883: {
                        region_id: "1883",
                        region_name: "新北区"
                    },
                    counties1884: {
                        region_id: "1884",
                        region_name: "武进区"
                    },
                    counties1885: {
                        region_id: "1885",
                        region_name: "溧阳市"
                    },
                    counties1886: {
                        region_id: "1886",
                        region_name: "金坛市"
                    }
                }
            },
            cities224: {
                region_id: "224",
                region_name: "淮安",
                county: {
                    counties1887: {
                        region_id: "1887",
                        region_name: "清河区"
                    },
                    counties1888: {
                        region_id: "1888",
                        region_name: "清浦区"
                    },
                    counties1889: {
                        region_id: "1889",
                        region_name: "楚州区"
                    },
                    counties1890: {
                        region_id: "1890",
                        region_name: "淮阴区"
                    },
                    counties1891: {
                        region_id: "1891",
                        region_name: "涟水县"
                    },
                    counties1892: {
                        region_id: "1892",
                        region_name: "洪泽县"
                    },
                    counties1893: {
                        region_id: "1893",
                        region_name: "盱眙县"
                    },
                    counties1894: {
                        region_id: "1894",
                        region_name: "金湖县"
                    }
                }
            },
            cities225: {
                region_id: "225",
                region_name: "连云港",
                county: {
                    counties1895: {
                        region_id: "1895",
                        region_name: "新浦区"
                    },
                    counties1896: {
                        region_id: "1896",
                        region_name: "连云区"
                    },
                    counties1897: {
                        region_id: "1897",
                        region_name: "海州区"
                    },
                    counties1898: {
                        region_id: "1898",
                        region_name: "赣榆县"
                    },
                    counties1899: {
                        region_id: "1899",
                        region_name: "东海县"
                    },
                    counties1900: {
                        region_id: "1900",
                        region_name: "灌云县"
                    },
                    counties1901: {
                        region_id: "1901",
                        region_name: "灌南县"
                    }
                }
            },
            cities226: {
                region_id: "226",
                region_name: "南通",
                county: {
                    counties1902: {
                        region_id: "1902",
                        region_name: "崇川区"
                    },
                    counties1903: {
                        region_id: "1903",
                        region_name: "港闸区"
                    },
                    counties1904: {
                        region_id: "1904",
                        region_name: "经济开发区"
                    },
                    counties1905: {
                        region_id: "1905",
                        region_name: "启东市"
                    },
                    counties1906: {
                        region_id: "1906",
                        region_name: "如皋市"
                    },
                    counties1907: {
                        region_id: "1907",
                        region_name: "通州市"
                    },
                    counties1908: {
                        region_id: "1908",
                        region_name: "海门市"
                    },
                    counties1909: {
                        region_id: "1909",
                        region_name: "海安县"
                    },
                    counties1910: {
                        region_id: "1910",
                        region_name: "如东县"
                    }
                }
            },
            cities227: {
                region_id: "227",
                region_name: "宿迁",
                county: {
                    counties1911: {
                        region_id: "1911",
                        region_name: "宿城区"
                    },
                    counties1912: {
                        region_id: "1912",
                        region_name: "宿豫区"
                    },
                    counties1913: {
                        region_id: "1913",
                        region_name: "宿豫县"
                    },
                    counties1914: {
                        region_id: "1914",
                        region_name: "沭阳县"
                    },
                    counties1915: {
                        region_id: "1915",
                        region_name: "泗阳县"
                    },
                    counties1916: {
                        region_id: "1916",
                        region_name: "泗洪县"
                    }
                }
            },
            cities228: {
                region_id: "228",
                region_name: "泰州",
                county: {
                    counties1917: {
                        region_id: "1917",
                        region_name: "海陵区"
                    },
                    counties1918: {
                        region_id: "1918",
                        region_name: "高港区"
                    },
                    counties1919: {
                        region_id: "1919",
                        region_name: "兴化市"
                    },
                    counties1920: {
                        region_id: "1920",
                        region_name: "靖江市"
                    },
                    counties1921: {
                        region_id: "1921",
                        region_name: "泰兴市"
                    },
                    counties1922: {
                        region_id: "1922",
                        region_name: "姜堰市"
                    }
                }
            },
            cities229: {
                region_id: "229",
                region_name: "徐州",
                county: {
                    counties1923: {
                        region_id: "1923",
                        region_name: "云龙区"
                    },
                    counties1924: {
                        region_id: "1924",
                        region_name: "鼓楼区"
                    },
                    counties1925: {
                        region_id: "1925",
                        region_name: "九里区"
                    },
                    counties1926: {
                        region_id: "1926",
                        region_name: "贾汪区"
                    },
                    counties1927: {
                        region_id: "1927",
                        region_name: "泉山区"
                    },
                    counties1928: {
                        region_id: "1928",
                        region_name: "新沂市"
                    },
                    counties1929: {
                        region_id: "1929",
                        region_name: "邳州市"
                    },
                    counties1930: {
                        region_id: "1930",
                        region_name: "丰县"
                    },
                    counties1931: {
                        region_id: "1931",
                        region_name: "沛县"
                    },
                    counties1932: {
                        region_id: "1932",
                        region_name: "铜山县"
                    },
                    counties1933: {
                        region_id: "1933",
                        region_name: "睢宁县"
                    }
                }
            },
            cities230: {
                region_id: "230",
                region_name: "盐城",
                county: {
                    counties1934: {
                        region_id: "1934",
                        region_name: "城区"
                    },
                    counties1935: {
                        region_id: "1935",
                        region_name: "亭湖区"
                    },
                    counties1936: {
                        region_id: "1936",
                        region_name: "盐都区"
                    },
                    counties1937: {
                        region_id: "1937",
                        region_name: "盐都县"
                    },
                    counties1938: {
                        region_id: "1938",
                        region_name: "东台市"
                    },
                    counties1939: {
                        region_id: "1939",
                        region_name: "大丰市"
                    },
                    counties1940: {
                        region_id: "1940",
                        region_name: "响水县"
                    },
                    counties1941: {
                        region_id: "1941",
                        region_name: "滨海县"
                    },
                    counties1942: {
                        region_id: "1942",
                        region_name: "阜宁县"
                    },
                    counties1943: {
                        region_id: "1943",
                        region_name: "射阳县"
                    },
                    counties1944: {
                        region_id: "1944",
                        region_name: "建湖县"
                    }
                }
            },
            cities231: {
                region_id: "231",
                region_name: "扬州",
                county: {
                    counties1945: {
                        region_id: "1945",
                        region_name: "广陵区"
                    },
                    counties1946: {
                        region_id: "1946",
                        region_name: "维扬区"
                    },
                    counties1947: {
                        region_id: "1947",
                        region_name: "邗江区"
                    },
                    counties1948: {
                        region_id: "1948",
                        region_name: "仪征市"
                    },
                    counties1949: {
                        region_id: "1949",
                        region_name: "高邮市"
                    },
                    counties1950: {
                        region_id: "1950",
                        region_name: "江都市"
                    },
                    counties1951: {
                        region_id: "1951",
                        region_name: "宝应县"
                    }
                }
            },
            cities232: {
                region_id: "232",
                region_name: "镇江",
                county: {
                    counties1952: {
                        region_id: "1952",
                        region_name: "京口区"
                    },
                    counties1953: {
                        region_id: "1953",
                        region_name: "润州区"
                    },
                    counties1954: {
                        region_id: "1954",
                        region_name: "丹徒区"
                    },
                    counties1955: {
                        region_id: "1955",
                        region_name: "丹阳市"
                    },
                    counties1956: {
                        region_id: "1956",
                        region_name: "扬中市"
                    },
                    counties1957: {
                        region_id: "1957",
                        region_name: "句容市"
                    }
                }
            }
        }
    },
    provinces17: {
        region_id: "17",
        region_name: "江西",
        city: {
            cities233: {
                region_id: "233",
                region_name: "南昌",
                county: {
                    counties1958: {
                        region_id: "1958",
                        region_name: "东湖区"
                    },
                    counties1959: {
                        region_id: "1959",
                        region_name: "西湖区"
                    },
                    counties1960: {
                        region_id: "1960",
                        region_name: "青云谱区"
                    },
                    counties1961: {
                        region_id: "1961",
                        region_name: "湾里区"
                    },
                    counties1962: {
                        region_id: "1962",
                        region_name: "青山湖区"
                    },
                    counties1963: {
                        region_id: "1963",
                        region_name: "红谷滩新区"
                    },
                    counties1964: {
                        region_id: "1964",
                        region_name: "昌北区"
                    },
                    counties1965: {
                        region_id: "1965",
                        region_name: "高新区"
                    },
                    counties1966: {
                        region_id: "1966",
                        region_name: "南昌县"
                    },
                    counties1967: {
                        region_id: "1967",
                        region_name: "新建县"
                    },
                    counties1968: {
                        region_id: "1968",
                        region_name: "安义县"
                    },
                    counties1969: {
                        region_id: "1969",
                        region_name: "进贤县"
                    }
                }
            },
            cities234: {
                region_id: "234",
                region_name: "抚州",
                county: {
                    counties1970: {
                        region_id: "1970",
                        region_name: "临川区"
                    },
                    counties1971: {
                        region_id: "1971",
                        region_name: "南城县"
                    },
                    counties1972: {
                        region_id: "1972",
                        region_name: "黎川县"
                    },
                    counties1973: {
                        region_id: "1973",
                        region_name: "南丰县"
                    },
                    counties1974: {
                        region_id: "1974",
                        region_name: "崇仁县"
                    },
                    counties1975: {
                        region_id: "1975",
                        region_name: "乐安县"
                    },
                    counties1976: {
                        region_id: "1976",
                        region_name: "宜黄县"
                    },
                    counties1977: {
                        region_id: "1977",
                        region_name: "金溪县"
                    },
                    counties1978: {
                        region_id: "1978",
                        region_name: "资溪县"
                    },
                    counties1979: {
                        region_id: "1979",
                        region_name: "东乡县"
                    },
                    counties1980: {
                        region_id: "1980",
                        region_name: "广昌县"
                    }
                }
            },
            cities235: {
                region_id: "235",
                region_name: "赣州",
                county: {
                    counties1981: {
                        region_id: "1981",
                        region_name: "章贡区"
                    },
                    counties1982: {
                        region_id: "1982",
                        region_name: "于都县"
                    },
                    counties1983: {
                        region_id: "1983",
                        region_name: "瑞金市"
                    },
                    counties1984: {
                        region_id: "1984",
                        region_name: "南康市"
                    },
                    counties1985: {
                        region_id: "1985",
                        region_name: "赣县"
                    },
                    counties1986: {
                        region_id: "1986",
                        region_name: "信丰县"
                    },
                    counties1987: {
                        region_id: "1987",
                        region_name: "大余县"
                    },
                    counties1988: {
                        region_id: "1988",
                        region_name: "上犹县"
                    },
                    counties1989: {
                        region_id: "1989",
                        region_name: "崇义县"
                    },
                    counties1990: {
                        region_id: "1990",
                        region_name: "安远县"
                    },
                    counties1991: {
                        region_id: "1991",
                        region_name: "龙南县"
                    },
                    counties1992: {
                        region_id: "1992",
                        region_name: "定南县"
                    },
                    counties1993: {
                        region_id: "1993",
                        region_name: "全南县"
                    },
                    counties1994: {
                        region_id: "1994",
                        region_name: "宁都县"
                    },
                    counties1995: {
                        region_id: "1995",
                        region_name: "兴国县"
                    },
                    counties1996: {
                        region_id: "1996",
                        region_name: "会昌县"
                    },
                    counties1997: {
                        region_id: "1997",
                        region_name: "寻乌县"
                    },
                    counties1998: {
                        region_id: "1998",
                        region_name: "石城县"
                    }
                }
            },
            cities236: {
                region_id: "236",
                region_name: "吉安",
                county: {
                    counties1999: {
                        region_id: "1999",
                        region_name: "安福县"
                    },
                    counties2000: {
                        region_id: "2000",
                        region_name: "吉州区"
                    },
                    counties2001: {
                        region_id: "2001",
                        region_name: "青原区"
                    },
                    counties2002: {
                        region_id: "2002",
                        region_name: "井冈山市"
                    },
                    counties2003: {
                        region_id: "2003",
                        region_name: "吉安县"
                    },
                    counties2004: {
                        region_id: "2004",
                        region_name: "吉水县"
                    },
                    counties2005: {
                        region_id: "2005",
                        region_name: "峡江县"
                    },
                    counties2006: {
                        region_id: "2006",
                        region_name: "新干县"
                    },
                    counties2007: {
                        region_id: "2007",
                        region_name: "永丰县"
                    },
                    counties2008: {
                        region_id: "2008",
                        region_name: "泰和县"
                    },
                    counties2009: {
                        region_id: "2009",
                        region_name: "遂川县"
                    },
                    counties2010: {
                        region_id: "2010",
                        region_name: "万安县"
                    },
                    counties2011: {
                        region_id: "2011",
                        region_name: "永新县"
                    }
                }
            },
            cities237: {
                region_id: "237",
                region_name: "景德镇",
                county: {
                    counties2012: {
                        region_id: "2012",
                        region_name: "珠山区"
                    },
                    counties2013: {
                        region_id: "2013",
                        region_name: "昌江区"
                    },
                    counties2014: {
                        region_id: "2014",
                        region_name: "乐平市"
                    },
                    counties2015: {
                        region_id: "2015",
                        region_name: "浮梁县"
                    }
                }
            },
            cities238: {
                region_id: "238",
                region_name: "九江",
                county: {
                    counties2016: {
                        region_id: "2016",
                        region_name: "浔阳区"
                    },
                    counties2017: {
                        region_id: "2017",
                        region_name: "庐山区"
                    },
                    counties2018: {
                        region_id: "2018",
                        region_name: "瑞昌市"
                    },
                    counties2019: {
                        region_id: "2019",
                        region_name: "九江县"
                    },
                    counties2020: {
                        region_id: "2020",
                        region_name: "武宁县"
                    },
                    counties2021: {
                        region_id: "2021",
                        region_name: "修水县"
                    },
                    counties2022: {
                        region_id: "2022",
                        region_name: "永修县"
                    },
                    counties2023: {
                        region_id: "2023",
                        region_name: "德安县"
                    },
                    counties2024: {
                        region_id: "2024",
                        region_name: "星子县"
                    },
                    counties2025: {
                        region_id: "2025",
                        region_name: "都昌县"
                    },
                    counties2026: {
                        region_id: "2026",
                        region_name: "湖口县"
                    },
                    counties2027: {
                        region_id: "2027",
                        region_name: "彭泽县"
                    }
                }
            },
            cities239: {
                region_id: "239",
                region_name: "萍乡",
                county: {
                    counties2028: {
                        region_id: "2028",
                        region_name: "安源区"
                    },
                    counties2029: {
                        region_id: "2029",
                        region_name: "湘东区"
                    },
                    counties2030: {
                        region_id: "2030",
                        region_name: "莲花县"
                    },
                    counties2031: {
                        region_id: "2031",
                        region_name: "芦溪县"
                    },
                    counties2032: {
                        region_id: "2032",
                        region_name: "上栗县"
                    }
                }
            },
            cities240: {
                region_id: "240",
                region_name: "上饶",
                county: {
                    counties2033: {
                        region_id: "2033",
                        region_name: "信州区"
                    },
                    counties2034: {
                        region_id: "2034",
                        region_name: "德兴市"
                    },
                    counties2035: {
                        region_id: "2035",
                        region_name: "上饶县"
                    },
                    counties2036: {
                        region_id: "2036",
                        region_name: "广丰县"
                    },
                    counties2037: {
                        region_id: "2037",
                        region_name: "玉山县"
                    },
                    counties2038: {
                        region_id: "2038",
                        region_name: "铅山县"
                    },
                    counties2039: {
                        region_id: "2039",
                        region_name: "横峰县"
                    },
                    counties2040: {
                        region_id: "2040",
                        region_name: "弋阳县"
                    },
                    counties2041: {
                        region_id: "2041",
                        region_name: "余干县"
                    },
                    counties2042: {
                        region_id: "2042",
                        region_name: "波阳县"
                    },
                    counties2043: {
                        region_id: "2043",
                        region_name: "万年县"
                    },
                    counties2044: {
                        region_id: "2044",
                        region_name: "婺源县"
                    }
                }
            },
            cities241: {
                region_id: "241",
                region_name: "新余",
                county: {
                    counties2045: {
                        region_id: "2045",
                        region_name: "渝水区"
                    },
                    counties2046: {
                        region_id: "2046",
                        region_name: "分宜县"
                    }
                }
            },
            cities242: {
                region_id: "242",
                region_name: "宜春",
                county: {
                    counties2047: {
                        region_id: "2047",
                        region_name: "袁州区"
                    },
                    counties2048: {
                        region_id: "2048",
                        region_name: "丰城市"
                    },
                    counties2049: {
                        region_id: "2049",
                        region_name: "樟树市"
                    },
                    counties2050: {
                        region_id: "2050",
                        region_name: "高安市"
                    },
                    counties2051: {
                        region_id: "2051",
                        region_name: "奉新县"
                    },
                    counties2052: {
                        region_id: "2052",
                        region_name: "万载县"
                    },
                    counties2053: {
                        region_id: "2053",
                        region_name: "上高县"
                    },
                    counties2054: {
                        region_id: "2054",
                        region_name: "宜丰县"
                    },
                    counties2055: {
                        region_id: "2055",
                        region_name: "靖安县"
                    },
                    counties2056: {
                        region_id: "2056",
                        region_name: "铜鼓县"
                    }
                }
            },
            cities243: {
                region_id: "243",
                region_name: "鹰潭",
                county: {
                    counties2057: {
                        region_id: "2057",
                        region_name: "月湖区"
                    },
                    counties2058: {
                        region_id: "2058",
                        region_name: "贵溪市"
                    },
                    counties2059: {
                        region_id: "2059",
                        region_name: "余江县"
                    }
                }
            }
        }
    },
    provinces18: {
        region_id: "18",
        region_name: "辽宁",
        city: {
            cities244: {
                region_id: "244",
                region_name: "沈阳",
                county: {
                    counties2060: {
                        region_id: "2060",
                        region_name: "沈河区"
                    },
                    counties2061: {
                        region_id: "2061",
                        region_name: "皇姑区"
                    },
                    counties2062: {
                        region_id: "2062",
                        region_name: "和平区"
                    },
                    counties2063: {
                        region_id: "2063",
                        region_name: "大东区"
                    },
                    counties2064: {
                        region_id: "2064",
                        region_name: "铁西区"
                    },
                    counties2065: {
                        region_id: "2065",
                        region_name: "苏家屯区"
                    },
                    counties2066: {
                        region_id: "2066",
                        region_name: "东陵区"
                    },
                    counties2067: {
                        region_id: "2067",
                        region_name: "沈北新区"
                    },
                    counties2068: {
                        region_id: "2068",
                        region_name: "于洪区"
                    },
                    counties2069: {
                        region_id: "2069",
                        region_name: "浑南新区"
                    },
                    counties2070: {
                        region_id: "2070",
                        region_name: "新民市"
                    },
                    counties2071: {
                        region_id: "2071",
                        region_name: "辽中县"
                    },
                    counties2072: {
                        region_id: "2072",
                        region_name: "康平县"
                    },
                    counties2073: {
                        region_id: "2073",
                        region_name: "法库县"
                    }
                }
            },
            cities245: {
                region_id: "245",
                region_name: "大连",
                county: {
                    counties2074: {
                        region_id: "2074",
                        region_name: "西岗区"
                    },
                    counties2075: {
                        region_id: "2075",
                        region_name: "中山区"
                    },
                    counties2076: {
                        region_id: "2076",
                        region_name: "沙河口区"
                    },
                    counties2077: {
                        region_id: "2077",
                        region_name: "甘井子区"
                    },
                    counties2078: {
                        region_id: "2078",
                        region_name: "旅顺口区"
                    },
                    counties2079: {
                        region_id: "2079",
                        region_name: "金州区"
                    },
                    counties2080: {
                        region_id: "2080",
                        region_name: "开发区"
                    },
                    counties2081: {
                        region_id: "2081",
                        region_name: "瓦房店市"
                    },
                    counties2082: {
                        region_id: "2082",
                        region_name: "普兰店市"
                    },
                    counties2083: {
                        region_id: "2083",
                        region_name: "庄河市"
                    },
                    counties2084: {
                        region_id: "2084",
                        region_name: "长海县"
                    }
                }
            },
            cities246: {
                region_id: "246",
                region_name: "鞍山",
                county: {
                    counties2085: {
                        region_id: "2085",
                        region_name: "铁东区"
                    },
                    counties2086: {
                        region_id: "2086",
                        region_name: "铁西区"
                    },
                    counties2087: {
                        region_id: "2087",
                        region_name: "立山区"
                    },
                    counties2088: {
                        region_id: "2088",
                        region_name: "千山区"
                    },
                    counties2089: {
                        region_id: "2089",
                        region_name: "岫岩"
                    },
                    counties2090: {
                        region_id: "2090",
                        region_name: "海城市"
                    },
                    counties2091: {
                        region_id: "2091",
                        region_name: "台安县"
                    }
                }
            },
            cities247: {
                region_id: "247",
                region_name: "本溪",
                county: {
                    counties2092: {
                        region_id: "2092",
                        region_name: "本溪"
                    },
                    counties2093: {
                        region_id: "2093",
                        region_name: "平山区"
                    },
                    counties2094: {
                        region_id: "2094",
                        region_name: "明山区"
                    },
                    counties2095: {
                        region_id: "2095",
                        region_name: "溪湖区"
                    },
                    counties2096: {
                        region_id: "2096",
                        region_name: "南芬区"
                    },
                    counties2097: {
                        region_id: "2097",
                        region_name: "桓仁"
                    }
                }
            },
            cities248: {
                region_id: "248",
                region_name: "朝阳",
                county: {
                    counties2098: {
                        region_id: "2098",
                        region_name: "双塔区"
                    },
                    counties2099: {
                        region_id: "2099",
                        region_name: "龙城区"
                    },
                    counties2100: {
                        region_id: "2100",
                        region_name: "喀喇沁左翼蒙古族自治县"
                    },
                    counties2101: {
                        region_id: "2101",
                        region_name: "北票市"
                    },
                    counties2102: {
                        region_id: "2102",
                        region_name: "凌源市"
                    },
                    counties2103: {
                        region_id: "2103",
                        region_name: "朝阳县"
                    },
                    counties2104: {
                        region_id: "2104",
                        region_name: "建平县"
                    }
                }
            },
            cities249: {
                region_id: "249",
                region_name: "丹东",
                county: {
                    counties2105: {
                        region_id: "2105",
                        region_name: "振兴区"
                    },
                    counties2106: {
                        region_id: "2106",
                        region_name: "元宝区"
                    },
                    counties2107: {
                        region_id: "2107",
                        region_name: "振安区"
                    },
                    counties2108: {
                        region_id: "2108",
                        region_name: "宽甸"
                    },
                    counties2109: {
                        region_id: "2109",
                        region_name: "东港市"
                    },
                    counties2110: {
                        region_id: "2110",
                        region_name: "凤城市"
                    }
                }
            },
            cities250: {
                region_id: "250",
                region_name: "抚顺",
                county: {
                    counties2111: {
                        region_id: "2111",
                        region_name: "顺城区"
                    },
                    counties2112: {
                        region_id: "2112",
                        region_name: "新抚区"
                    },
                    counties2113: {
                        region_id: "2113",
                        region_name: "东洲区"
                    },
                    counties2114: {
                        region_id: "2114",
                        region_name: "望花区"
                    },
                    counties2115: {
                        region_id: "2115",
                        region_name: "清原"
                    },
                    counties2116: {
                        region_id: "2116",
                        region_name: "新宾"
                    },
                    counties2117: {
                        region_id: "2117",
                        region_name: "抚顺县"
                    }
                }
            },
            cities251: {
                region_id: "251",
                region_name: "阜新",
                county: {
                    counties2118: {
                        region_id: "2118",
                        region_name: "阜新"
                    },
                    counties2119: {
                        region_id: "2119",
                        region_name: "海州区"
                    },
                    counties2120: {
                        region_id: "2120",
                        region_name: "新邱区"
                    },
                    counties2121: {
                        region_id: "2121",
                        region_name: "太平区"
                    },
                    counties2122: {
                        region_id: "2122",
                        region_name: "清河门区"
                    },
                    counties2123: {
                        region_id: "2123",
                        region_name: "细河区"
                    },
                    counties2124: {
                        region_id: "2124",
                        region_name: "彰武县"
                    }
                }
            },
            cities252: {
                region_id: "252",
                region_name: "葫芦岛",
                county: {
                    counties2125: {
                        region_id: "2125",
                        region_name: "龙港区"
                    },
                    counties2126: {
                        region_id: "2126",
                        region_name: "南票区"
                    },
                    counties2127: {
                        region_id: "2127",
                        region_name: "连山区"
                    },
                    counties2128: {
                        region_id: "2128",
                        region_name: "兴城市"
                    },
                    counties2129: {
                        region_id: "2129",
                        region_name: "绥中县"
                    },
                    counties2130: {
                        region_id: "2130",
                        region_name: "建昌县"
                    }
                }
            },
            cities253: {
                region_id: "253",
                region_name: "锦州",
                county: {
                    counties2131: {
                        region_id: "2131",
                        region_name: "太和区"
                    },
                    counties2132: {
                        region_id: "2132",
                        region_name: "古塔区"
                    },
                    counties2133: {
                        region_id: "2133",
                        region_name: "凌河区"
                    },
                    counties2134: {
                        region_id: "2134",
                        region_name: "凌海市"
                    },
                    counties2135: {
                        region_id: "2135",
                        region_name: "北镇市"
                    },
                    counties2136: {
                        region_id: "2136",
                        region_name: "黑山县"
                    },
                    counties2137: {
                        region_id: "2137",
                        region_name: "义县"
                    }
                }
            },
            cities254: {
                region_id: "254",
                region_name: "辽阳",
                county: {
                    counties2138: {
                        region_id: "2138",
                        region_name: "白塔区"
                    },
                    counties2139: {
                        region_id: "2139",
                        region_name: "文圣区"
                    },
                    counties2140: {
                        region_id: "2140",
                        region_name: "宏伟区"
                    },
                    counties2141: {
                        region_id: "2141",
                        region_name: "太子河区"
                    },
                    counties2142: {
                        region_id: "2142",
                        region_name: "弓长岭区"
                    },
                    counties2143: {
                        region_id: "2143",
                        region_name: "灯塔市"
                    },
                    counties2144: {
                        region_id: "2144",
                        region_name: "辽阳县"
                    }
                }
            },
            cities255: {
                region_id: "255",
                region_name: "盘锦",
                county: {
                    counties2145: {
                        region_id: "2145",
                        region_name: "双台子区"
                    },
                    counties2146: {
                        region_id: "2146",
                        region_name: "兴隆台区"
                    },
                    counties2147: {
                        region_id: "2147",
                        region_name: "大洼县"
                    },
                    counties2148: {
                        region_id: "2148",
                        region_name: "盘山县"
                    }
                }
            },
            cities256: {
                region_id: "256",
                region_name: "铁岭",
                county: {
                    counties2149: {
                        region_id: "2149",
                        region_name: "银州区"
                    },
                    counties2150: {
                        region_id: "2150",
                        region_name: "清河区"
                    },
                    counties2151: {
                        region_id: "2151",
                        region_name: "调兵山市"
                    },
                    counties2152: {
                        region_id: "2152",
                        region_name: "开原市"
                    },
                    counties2153: {
                        region_id: "2153",
                        region_name: "铁岭县"
                    },
                    counties2154: {
                        region_id: "2154",
                        region_name: "西丰县"
                    },
                    counties2155: {
                        region_id: "2155",
                        region_name: "昌图县"
                    }
                }
            },
            cities257: {
                region_id: "257",
                region_name: "营口",
                county: {
                    counties2156: {
                        region_id: "2156",
                        region_name: "站前区"
                    },
                    counties2157: {
                        region_id: "2157",
                        region_name: "西市区"
                    },
                    counties2158: {
                        region_id: "2158",
                        region_name: "鲅鱼圈区"
                    },
                    counties2159: {
                        region_id: "2159",
                        region_name: "老边区"
                    },
                    counties2160: {
                        region_id: "2160",
                        region_name: "盖州市"
                    },
                    counties2161: {
                        region_id: "2161",
                        region_name: "大石桥市"
                    }
                }
            }
        }
    },
    provinces19: {
        region_id: "19",
        region_name: "内蒙古",
        city: {
            cities258: {
                region_id: "258",
                region_name: "呼和浩特",
                county: {
                    counties2162: {
                        region_id: "2162",
                        region_name: "回民区"
                    },
                    counties2163: {
                        region_id: "2163",
                        region_name: "玉泉区"
                    },
                    counties2164: {
                        region_id: "2164",
                        region_name: "新城区"
                    },
                    counties2165: {
                        region_id: "2165",
                        region_name: "赛罕区"
                    },
                    counties2166: {
                        region_id: "2166",
                        region_name: "清水河县"
                    },
                    counties2167: {
                        region_id: "2167",
                        region_name: "土默特左旗"
                    },
                    counties2168: {
                        region_id: "2168",
                        region_name: "托克托县"
                    },
                    counties2169: {
                        region_id: "2169",
                        region_name: "和林格尔县"
                    },
                    counties2170: {
                        region_id: "2170",
                        region_name: "武川县"
                    }
                }
            },
            cities259: {
                region_id: "259",
                region_name: "阿拉善盟",
                county: {
                    counties2171: {
                        region_id: "2171",
                        region_name: "阿拉善左旗"
                    },
                    counties2172: {
                        region_id: "2172",
                        region_name: "阿拉善右旗"
                    },
                    counties2173: {
                        region_id: "2173",
                        region_name: "额济纳旗"
                    }
                }
            },
            cities260: {
                region_id: "260",
                region_name: "巴彦淖尔盟",
                county: {
                    counties2174: {
                        region_id: "2174",
                        region_name: "临河区"
                    },
                    counties2175: {
                        region_id: "2175",
                        region_name: "五原县"
                    },
                    counties2176: {
                        region_id: "2176",
                        region_name: "磴口县"
                    },
                    counties2177: {
                        region_id: "2177",
                        region_name: "乌拉特前旗"
                    },
                    counties2178: {
                        region_id: "2178",
                        region_name: "乌拉特中旗"
                    },
                    counties2179: {
                        region_id: "2179",
                        region_name: "乌拉特后旗"
                    },
                    counties2180: {
                        region_id: "2180",
                        region_name: "杭锦后旗"
                    }
                }
            },
            cities261: {
                region_id: "261",
                region_name: "包头",
                county: {
                    counties2181: {
                        region_id: "2181",
                        region_name: "昆都仑区"
                    },
                    counties2182: {
                        region_id: "2182",
                        region_name: "青山区"
                    },
                    counties2183: {
                        region_id: "2183",
                        region_name: "东河区"
                    },
                    counties2184: {
                        region_id: "2184",
                        region_name: "九原区"
                    },
                    counties2185: {
                        region_id: "2185",
                        region_name: "石拐区"
                    },
                    counties2186: {
                        region_id: "2186",
                        region_name: "白云矿区"
                    },
                    counties2187: {
                        region_id: "2187",
                        region_name: "土默特右旗"
                    },
                    counties2188: {
                        region_id: "2188",
                        region_name: "固阳县"
                    },
                    counties2189: {
                        region_id: "2189",
                        region_name: "达尔罕茂明安联合旗"
                    }
                }
            },
            cities262: {
                region_id: "262",
                region_name: "赤峰",
                county: {
                    counties2190: {
                        region_id: "2190",
                        region_name: "红山区"
                    },
                    counties2191: {
                        region_id: "2191",
                        region_name: "元宝山区"
                    },
                    counties2192: {
                        region_id: "2192",
                        region_name: "松山区"
                    },
                    counties2193: {
                        region_id: "2193",
                        region_name: "阿鲁科尔沁旗"
                    },
                    counties2194: {
                        region_id: "2194",
                        region_name: "巴林左旗"
                    },
                    counties2195: {
                        region_id: "2195",
                        region_name: "巴林右旗"
                    },
                    counties2196: {
                        region_id: "2196",
                        region_name: "林西县"
                    },
                    counties2197: {
                        region_id: "2197",
                        region_name: "克什克腾旗"
                    },
                    counties2198: {
                        region_id: "2198",
                        region_name: "翁牛特旗"
                    },
                    counties2199: {
                        region_id: "2199",
                        region_name: "喀喇沁旗"
                    },
                    counties2200: {
                        region_id: "2200",
                        region_name: "宁城县"
                    },
                    counties2201: {
                        region_id: "2201",
                        region_name: "敖汉旗"
                    }
                }
            },
            cities263: {
                region_id: "263",
                region_name: "鄂尔多斯",
                county: {
                    counties2202: {
                        region_id: "2202",
                        region_name: "东胜区"
                    },
                    counties2203: {
                        region_id: "2203",
                        region_name: "达拉特旗"
                    },
                    counties2204: {
                        region_id: "2204",
                        region_name: "准格尔旗"
                    },
                    counties2205: {
                        region_id: "2205",
                        region_name: "鄂托克前旗"
                    },
                    counties2206: {
                        region_id: "2206",
                        region_name: "鄂托克旗"
                    },
                    counties2207: {
                        region_id: "2207",
                        region_name: "杭锦旗"
                    },
                    counties2208: {
                        region_id: "2208",
                        region_name: "乌审旗"
                    },
                    counties2209: {
                        region_id: "2209",
                        region_name: "伊金霍洛旗"
                    }
                }
            },
            cities264: {
                region_id: "264",
                region_name: "呼伦贝尔",
                county: {
                    counties2210: {
                        region_id: "2210",
                        region_name: "海拉尔区"
                    },
                    counties2211: {
                        region_id: "2211",
                        region_name: "莫力达瓦"
                    },
                    counties2212: {
                        region_id: "2212",
                        region_name: "满洲里市"
                    },
                    counties2213: {
                        region_id: "2213",
                        region_name: "牙克石市"
                    },
                    counties2214: {
                        region_id: "2214",
                        region_name: "扎兰屯市"
                    },
                    counties2215: {
                        region_id: "2215",
                        region_name: "额尔古纳市"
                    },
                    counties2216: {
                        region_id: "2216",
                        region_name: "根河市"
                    },
                    counties2217: {
                        region_id: "2217",
                        region_name: "阿荣旗"
                    },
                    counties2218: {
                        region_id: "2218",
                        region_name: "鄂伦春自治旗"
                    },
                    counties2219: {
                        region_id: "2219",
                        region_name: "鄂温克族自治旗"
                    },
                    counties2220: {
                        region_id: "2220",
                        region_name: "陈巴尔虎旗"
                    },
                    counties2221: {
                        region_id: "2221",
                        region_name: "新巴尔虎左旗"
                    },
                    counties2222: {
                        region_id: "2222",
                        region_name: "新巴尔虎右旗"
                    }
                }
            },
            cities265: {
                region_id: "265",
                region_name: "通辽",
                county: {
                    counties2223: {
                        region_id: "2223",
                        region_name: "科尔沁区"
                    },
                    counties2224: {
                        region_id: "2224",
                        region_name: "霍林郭勒市"
                    },
                    counties2225: {
                        region_id: "2225",
                        region_name: "科尔沁左翼中旗"
                    },
                    counties2226: {
                        region_id: "2226",
                        region_name: "科尔沁左翼后旗"
                    },
                    counties2227: {
                        region_id: "2227",
                        region_name: "开鲁县"
                    },
                    counties2228: {
                        region_id: "2228",
                        region_name: "库伦旗"
                    },
                    counties2229: {
                        region_id: "2229",
                        region_name: "奈曼旗"
                    },
                    counties2230: {
                        region_id: "2230",
                        region_name: "扎鲁特旗"
                    }
                }
            },
            cities266: {
                region_id: "266",
                region_name: "乌海",
                county: {
                    counties2231: {
                        region_id: "2231",
                        region_name: "海勃湾区"
                    },
                    counties2232: {
                        region_id: "2232",
                        region_name: "乌达区"
                    },
                    counties2233: {
                        region_id: "2233",
                        region_name: "海南区"
                    }
                }
            },
            cities267: {
                region_id: "267",
                region_name: "乌兰察布市",
                county: {
                    counties2234: {
                        region_id: "2234",
                        region_name: "化德县"
                    },
                    counties2235: {
                        region_id: "2235",
                        region_name: "集宁区"
                    },
                    counties2236: {
                        region_id: "2236",
                        region_name: "丰镇市"
                    },
                    counties2237: {
                        region_id: "2237",
                        region_name: "卓资县"
                    },
                    counties2238: {
                        region_id: "2238",
                        region_name: "商都县"
                    },
                    counties2239: {
                        region_id: "2239",
                        region_name: "兴和县"
                    },
                    counties2240: {
                        region_id: "2240",
                        region_name: "凉城县"
                    },
                    counties2241: {
                        region_id: "2241",
                        region_name: "察哈尔右翼前旗"
                    },
                    counties2242: {
                        region_id: "2242",
                        region_name: "察哈尔右翼中旗"
                    },
                    counties2243: {
                        region_id: "2243",
                        region_name: "察哈尔右翼后旗"
                    },
                    counties2244: {
                        region_id: "2244",
                        region_name: "四子王旗"
                    }
                }
            },
            cities268: {
                region_id: "268",
                region_name: "锡林郭勒盟",
                county: {
                    counties2245: {
                        region_id: "2245",
                        region_name: "二连浩特市"
                    },
                    counties2246: {
                        region_id: "2246",
                        region_name: "锡林浩特市"
                    },
                    counties2247: {
                        region_id: "2247",
                        region_name: "阿巴嘎旗"
                    },
                    counties2248: {
                        region_id: "2248",
                        region_name: "苏尼特左旗"
                    },
                    counties2249: {
                        region_id: "2249",
                        region_name: "苏尼特右旗"
                    },
                    counties2250: {
                        region_id: "2250",
                        region_name: "东乌珠穆沁旗"
                    },
                    counties2251: {
                        region_id: "2251",
                        region_name: "西乌珠穆沁旗"
                    },
                    counties2252: {
                        region_id: "2252",
                        region_name: "太仆寺旗"
                    },
                    counties2253: {
                        region_id: "2253",
                        region_name: "镶黄旗"
                    },
                    counties2254: {
                        region_id: "2254",
                        region_name: "正镶白旗"
                    },
                    counties2255: {
                        region_id: "2255",
                        region_name: "正蓝旗"
                    },
                    counties2256: {
                        region_id: "2256",
                        region_name: "多伦县"
                    }
                }
            },
            cities269: {
                region_id: "269",
                region_name: "兴安盟",
                county: {
                    counties2257: {
                        region_id: "2257",
                        region_name: "乌兰浩特市"
                    },
                    counties2258: {
                        region_id: "2258",
                        region_name: "阿尔山市"
                    },
                    counties2259: {
                        region_id: "2259",
                        region_name: "科尔沁右翼前旗"
                    },
                    counties2260: {
                        region_id: "2260",
                        region_name: "科尔沁右翼中旗"
                    },
                    counties2261: {
                        region_id: "2261",
                        region_name: "扎赉特旗"
                    },
                    counties2262: {
                        region_id: "2262",
                        region_name: "突泉县"
                    }
                }
            }
        }
    },
    provinces20: {
        region_id: "20",
        region_name: "宁夏",
        city: {
            cities270: {
                region_id: "270",
                region_name: "银川",
                county: {
                    counties2263: {
                        region_id: "2263",
                        region_name: "西夏区"
                    },
                    counties2264: {
                        region_id: "2264",
                        region_name: "金凤区"
                    },
                    counties2265: {
                        region_id: "2265",
                        region_name: "兴庆区"
                    },
                    counties2266: {
                        region_id: "2266",
                        region_name: "灵武市"
                    },
                    counties2267: {
                        region_id: "2267",
                        region_name: "永宁县"
                    },
                    counties2268: {
                        region_id: "2268",
                        region_name: "贺兰县"
                    }
                }
            },
            cities271: {
                region_id: "271",
                region_name: "固原",
                county: {
                    counties2269: {
                        region_id: "2269",
                        region_name: "原州区"
                    },
                    counties2270: {
                        region_id: "2270",
                        region_name: "海原县"
                    },
                    counties2271: {
                        region_id: "2271",
                        region_name: "西吉县"
                    },
                    counties2272: {
                        region_id: "2272",
                        region_name: "隆德县"
                    },
                    counties2273: {
                        region_id: "2273",
                        region_name: "泾源县"
                    },
                    counties2274: {
                        region_id: "2274",
                        region_name: "彭阳县"
                    }
                }
            },
            cities272: {
                region_id: "272",
                region_name: "石嘴山",
                county: {
                    counties2275: {
                        region_id: "2275",
                        region_name: "惠农县"
                    },
                    counties2276: {
                        region_id: "2276",
                        region_name: "大武口区"
                    },
                    counties2277: {
                        region_id: "2277",
                        region_name: "惠农区"
                    },
                    counties2278: {
                        region_id: "2278",
                        region_name: "陶乐县"
                    },
                    counties2279: {
                        region_id: "2279",
                        region_name: "平罗县"
                    }
                }
            },
            cities273: {
                region_id: "273",
                region_name: "吴忠",
                county: {
                    counties2280: {
                        region_id: "2280",
                        region_name: "利通区"
                    },
                    counties2281: {
                        region_id: "2281",
                        region_name: "中卫县"
                    },
                    counties2282: {
                        region_id: "2282",
                        region_name: "青铜峡市"
                    },
                    counties2283: {
                        region_id: "2283",
                        region_name: "中宁县"
                    },
                    counties2284: {
                        region_id: "2284",
                        region_name: "盐池县"
                    },
                    counties2285: {
                        region_id: "2285",
                        region_name: "同心县"
                    }
                }
            },
            cities274: {
                region_id: "274",
                region_name: "中卫",
                county: {
                    counties2286: {
                        region_id: "2286",
                        region_name: "沙坡头区"
                    },
                    counties2287: {
                        region_id: "2287",
                        region_name: "海原县"
                    },
                    counties2288: {
                        region_id: "2288",
                        region_name: "中宁县"
                    }
                }
            }
        }
    },
    provinces21: {
        region_id: "21",
        region_name: "青海",
        city: {
            cities275: {
                region_id: "275",
                region_name: "西宁",
                county: {
                    counties2289: {
                        region_id: "2289",
                        region_name: "城中区"
                    },
                    counties2290: {
                        region_id: "2290",
                        region_name: "城东区"
                    },
                    counties2291: {
                        region_id: "2291",
                        region_name: "城西区"
                    },
                    counties2292: {
                        region_id: "2292",
                        region_name: "城北区"
                    },
                    counties2293: {
                        region_id: "2293",
                        region_name: "湟中县"
                    },
                    counties2294: {
                        region_id: "2294",
                        region_name: "湟源县"
                    },
                    counties2295: {
                        region_id: "2295",
                        region_name: "大通"
                    }
                }
            },
            cities276: {
                region_id: "276",
                region_name: "果洛",
                county: {
                    counties2296: {
                        region_id: "2296",
                        region_name: "玛沁县"
                    },
                    counties2297: {
                        region_id: "2297",
                        region_name: "班玛县"
                    },
                    counties2298: {
                        region_id: "2298",
                        region_name: "甘德县"
                    },
                    counties2299: {
                        region_id: "2299",
                        region_name: "达日县"
                    },
                    counties2300: {
                        region_id: "2300",
                        region_name: "久治县"
                    },
                    counties2301: {
                        region_id: "2301",
                        region_name: "玛多县"
                    }
                }
            },
            cities277: {
                region_id: "277",
                region_name: "海北",
                county: {
                    counties2302: {
                        region_id: "2302",
                        region_name: "海晏县"
                    },
                    counties2303: {
                        region_id: "2303",
                        region_name: "祁连县"
                    },
                    counties2304: {
                        region_id: "2304",
                        region_name: "刚察县"
                    },
                    counties2305: {
                        region_id: "2305",
                        region_name: "门源"
                    }
                }
            },
            cities278: {
                region_id: "278",
                region_name: "海东",
                county: {
                    counties2306: {
                        region_id: "2306",
                        region_name: "平安县"
                    },
                    counties2307: {
                        region_id: "2307",
                        region_name: "乐都县"
                    },
                    counties2308: {
                        region_id: "2308",
                        region_name: "民和"
                    },
                    counties2309: {
                        region_id: "2309",
                        region_name: "互助"
                    },
                    counties2310: {
                        region_id: "2310",
                        region_name: "化隆"
                    },
                    counties2311: {
                        region_id: "2311",
                        region_name: "循化"
                    }
                }
            },
            cities279: {
                region_id: "279",
                region_name: "海南",
                county: {
                    counties2312: {
                        region_id: "2312",
                        region_name: "共和县"
                    },
                    counties2313: {
                        region_id: "2313",
                        region_name: "同德县"
                    },
                    counties2314: {
                        region_id: "2314",
                        region_name: "贵德县"
                    },
                    counties2315: {
                        region_id: "2315",
                        region_name: "兴海县"
                    },
                    counties2316: {
                        region_id: "2316",
                        region_name: "贵南县"
                    }
                }
            },
            cities280: {
                region_id: "280",
                region_name: "海西",
                county: {
                    counties2317: {
                        region_id: "2317",
                        region_name: "德令哈市"
                    },
                    counties2318: {
                        region_id: "2318",
                        region_name: "格尔木市"
                    },
                    counties2319: {
                        region_id: "2319",
                        region_name: "乌兰县"
                    },
                    counties2320: {
                        region_id: "2320",
                        region_name: "都兰县"
                    },
                    counties2321: {
                        region_id: "2321",
                        region_name: "天峻县"
                    }
                }
            },
            cities281: {
                region_id: "281",
                region_name: "黄南",
                county: {
                    counties2322: {
                        region_id: "2322",
                        region_name: "同仁县"
                    },
                    counties2323: {
                        region_id: "2323",
                        region_name: "尖扎县"
                    },
                    counties2324: {
                        region_id: "2324",
                        region_name: "泽库县"
                    },
                    counties2325: {
                        region_id: "2325",
                        region_name: "河南蒙古族自治县"
                    }
                }
            },
            cities282: {
                region_id: "282",
                region_name: "玉树",
                county: {
                    counties2326: {
                        region_id: "2326",
                        region_name: "玉树县"
                    },
                    counties2327: {
                        region_id: "2327",
                        region_name: "杂多县"
                    },
                    counties2328: {
                        region_id: "2328",
                        region_name: "称多县"
                    },
                    counties2329: {
                        region_id: "2329",
                        region_name: "治多县"
                    },
                    counties2330: {
                        region_id: "2330",
                        region_name: "囊谦县"
                    },
                    counties2331: {
                        region_id: "2331",
                        region_name: "曲麻莱县"
                    }
                }
            }
        }
    },
    provinces22: {
        region_id: "22",
        region_name: "山东",
        city: {
            cities283: {
                region_id: "283",
                region_name: "济南",
                county: {
                    counties2332: {
                        region_id: "2332",
                        region_name: "市中区"
                    },
                    counties2333: {
                        region_id: "2333",
                        region_name: "历下区"
                    },
                    counties2334: {
                        region_id: "2334",
                        region_name: "天桥区"
                    },
                    counties2335: {
                        region_id: "2335",
                        region_name: "槐荫区"
                    },
                    counties2336: {
                        region_id: "2336",
                        region_name: "历城区"
                    },
                    counties2337: {
                        region_id: "2337",
                        region_name: "长清区"
                    },
                    counties2338: {
                        region_id: "2338",
                        region_name: "章丘市"
                    },
                    counties2339: {
                        region_id: "2339",
                        region_name: "平阴县"
                    },
                    counties2340: {
                        region_id: "2340",
                        region_name: "济阳县"
                    },
                    counties2341: {
                        region_id: "2341",
                        region_name: "商河县"
                    }
                }
            },
            cities284: {
                region_id: "284",
                region_name: "青岛",
                county: {
                    counties2342: {
                        region_id: "2342",
                        region_name: "市南区"
                    },
                    counties2343: {
                        region_id: "2343",
                        region_name: "市北区"
                    },
                    counties2344: {
                        region_id: "2344",
                        region_name: "城阳区"
                    },
                    counties2345: {
                        region_id: "2345",
                        region_name: "四方区"
                    },
                    counties2346: {
                        region_id: "2346",
                        region_name: "李沧区"
                    },
                    counties2347: {
                        region_id: "2347",
                        region_name: "黄岛区"
                    },
                    counties2348: {
                        region_id: "2348",
                        region_name: "崂山区"
                    },
                    counties2349: {
                        region_id: "2349",
                        region_name: "胶州市"
                    },
                    counties2350: {
                        region_id: "2350",
                        region_name: "即墨市"
                    },
                    counties2351: {
                        region_id: "2351",
                        region_name: "平度市"
                    },
                    counties2352: {
                        region_id: "2352",
                        region_name: "胶南市"
                    },
                    counties2353: {
                        region_id: "2353",
                        region_name: "莱西市"
                    }
                }
            },
            cities285: {
                region_id: "285",
                region_name: "滨州",
                county: {
                    counties2354: {
                        region_id: "2354",
                        region_name: "滨城区"
                    },
                    counties2355: {
                        region_id: "2355",
                        region_name: "惠民县"
                    },
                    counties2356: {
                        region_id: "2356",
                        region_name: "阳信县"
                    },
                    counties2357: {
                        region_id: "2357",
                        region_name: "无棣县"
                    },
                    counties2358: {
                        region_id: "2358",
                        region_name: "沾化县"
                    },
                    counties2359: {
                        region_id: "2359",
                        region_name: "博兴县"
                    },
                    counties2360: {
                        region_id: "2360",
                        region_name: "邹平县"
                    }
                }
            },
            cities286: {
                region_id: "286",
                region_name: "德州",
                county: {
                    counties2361: {
                        region_id: "2361",
                        region_name: "德城区"
                    },
                    counties2362: {
                        region_id: "2362",
                        region_name: "陵县"
                    },
                    counties2363: {
                        region_id: "2363",
                        region_name: "乐陵市"
                    },
                    counties2364: {
                        region_id: "2364",
                        region_name: "禹城市"
                    },
                    counties2365: {
                        region_id: "2365",
                        region_name: "宁津县"
                    },
                    counties2366: {
                        region_id: "2366",
                        region_name: "庆云县"
                    },
                    counties2367: {
                        region_id: "2367",
                        region_name: "临邑县"
                    },
                    counties2368: {
                        region_id: "2368",
                        region_name: "齐河县"
                    },
                    counties2369: {
                        region_id: "2369",
                        region_name: "平原县"
                    },
                    counties2370: {
                        region_id: "2370",
                        region_name: "夏津县"
                    },
                    counties2371: {
                        region_id: "2371",
                        region_name: "武城县"
                    }
                }
            },
            cities287: {
                region_id: "287",
                region_name: "东营",
                county: {
                    counties2372: {
                        region_id: "2372",
                        region_name: "东营区"
                    },
                    counties2373: {
                        region_id: "2373",
                        region_name: "河口区"
                    },
                    counties2374: {
                        region_id: "2374",
                        region_name: "垦利县"
                    },
                    counties2375: {
                        region_id: "2375",
                        region_name: "利津县"
                    },
                    counties2376: {
                        region_id: "2376",
                        region_name: "广饶县"
                    }
                }
            },
            cities288: {
                region_id: "288",
                region_name: "菏泽",
                county: {
                    counties2377: {
                        region_id: "2377",
                        region_name: "牡丹区"
                    },
                    counties2378: {
                        region_id: "2378",
                        region_name: "曹县"
                    },
                    counties2379: {
                        region_id: "2379",
                        region_name: "单县"
                    },
                    counties2380: {
                        region_id: "2380",
                        region_name: "成武县"
                    },
                    counties2381: {
                        region_id: "2381",
                        region_name: "巨野县"
                    },
                    counties2382: {
                        region_id: "2382",
                        region_name: "郓城县"
                    },
                    counties2383: {
                        region_id: "2383",
                        region_name: "鄄城县"
                    },
                    counties2384: {
                        region_id: "2384",
                        region_name: "定陶县"
                    },
                    counties2385: {
                        region_id: "2385",
                        region_name: "东明县"
                    }
                }
            },
            cities289: {
                region_id: "289",
                region_name: "济宁",
                county: {
                    counties2386: {
                        region_id: "2386",
                        region_name: "市中区"
                    },
                    counties2387: {
                        region_id: "2387",
                        region_name: "任城区"
                    },
                    counties2388: {
                        region_id: "2388",
                        region_name: "曲阜市"
                    },
                    counties2389: {
                        region_id: "2389",
                        region_name: "兖州市"
                    },
                    counties2390: {
                        region_id: "2390",
                        region_name: "邹城市"
                    },
                    counties2391: {
                        region_id: "2391",
                        region_name: "微山县"
                    },
                    counties2392: {
                        region_id: "2392",
                        region_name: "鱼台县"
                    },
                    counties2393: {
                        region_id: "2393",
                        region_name: "金乡县"
                    },
                    counties2394: {
                        region_id: "2394",
                        region_name: "嘉祥县"
                    },
                    counties2395: {
                        region_id: "2395",
                        region_name: "汶上县"
                    },
                    counties2396: {
                        region_id: "2396",
                        region_name: "泗水县"
                    },
                    counties2397: {
                        region_id: "2397",
                        region_name: "梁山县"
                    }
                }
            },
            cities290: {
                region_id: "290",
                region_name: "莱芜",
                county: {
                    counties2398: {
                        region_id: "2398",
                        region_name: "莱城区"
                    },
                    counties2399: {
                        region_id: "2399",
                        region_name: "钢城区"
                    }
                }
            },
            cities291: {
                region_id: "291",
                region_name: "聊城",
                county: {
                    counties2400: {
                        region_id: "2400",
                        region_name: "东昌府区"
                    },
                    counties2401: {
                        region_id: "2401",
                        region_name: "临清市"
                    },
                    counties2402: {
                        region_id: "2402",
                        region_name: "阳谷县"
                    },
                    counties2403: {
                        region_id: "2403",
                        region_name: "莘县"
                    },
                    counties2404: {
                        region_id: "2404",
                        region_name: "茌平县"
                    },
                    counties2405: {
                        region_id: "2405",
                        region_name: "东阿县"
                    },
                    counties2406: {
                        region_id: "2406",
                        region_name: "冠县"
                    },
                    counties2407: {
                        region_id: "2407",
                        region_name: "高唐县"
                    }
                }
            },
            cities292: {
                region_id: "292",
                region_name: "临沂",
                county: {
                    counties2408: {
                        region_id: "2408",
                        region_name: "兰山区"
                    },
                    counties2409: {
                        region_id: "2409",
                        region_name: "罗庄区"
                    },
                    counties2410: {
                        region_id: "2410",
                        region_name: "河东区"
                    },
                    counties2411: {
                        region_id: "2411",
                        region_name: "沂南县"
                    },
                    counties2412: {
                        region_id: "2412",
                        region_name: "郯城县"
                    },
                    counties2413: {
                        region_id: "2413",
                        region_name: "沂水县"
                    },
                    counties2414: {
                        region_id: "2414",
                        region_name: "苍山县"
                    },
                    counties2415: {
                        region_id: "2415",
                        region_name: "费县"
                    },
                    counties2416: {
                        region_id: "2416",
                        region_name: "平邑县"
                    },
                    counties2417: {
                        region_id: "2417",
                        region_name: "莒南县"
                    },
                    counties2418: {
                        region_id: "2418",
                        region_name: "蒙阴县"
                    },
                    counties2419: {
                        region_id: "2419",
                        region_name: "临沭县"
                    }
                }
            },
            cities293: {
                region_id: "293",
                region_name: "日照",
                county: {
                    counties2420: {
                        region_id: "2420",
                        region_name: "东港区"
                    },
                    counties2421: {
                        region_id: "2421",
                        region_name: "岚山区"
                    },
                    counties2422: {
                        region_id: "2422",
                        region_name: "五莲县"
                    },
                    counties2423: {
                        region_id: "2423",
                        region_name: "莒县"
                    }
                }
            },
            cities294: {
                region_id: "294",
                region_name: "泰安",
                county: {
                    counties2424: {
                        region_id: "2424",
                        region_name: "泰山区"
                    },
                    counties2425: {
                        region_id: "2425",
                        region_name: "岱岳区"
                    },
                    counties2426: {
                        region_id: "2426",
                        region_name: "新泰市"
                    },
                    counties2427: {
                        region_id: "2427",
                        region_name: "肥城市"
                    },
                    counties2428: {
                        region_id: "2428",
                        region_name: "宁阳县"
                    },
                    counties2429: {
                        region_id: "2429",
                        region_name: "东平县"
                    }
                }
            },
            cities295: {
                region_id: "295",
                region_name: "威海",
                county: {
                    counties2430: {
                        region_id: "2430",
                        region_name: "荣成市"
                    },
                    counties2431: {
                        region_id: "2431",
                        region_name: "乳山市"
                    },
                    counties2432: {
                        region_id: "2432",
                        region_name: "环翠区"
                    },
                    counties2433: {
                        region_id: "2433",
                        region_name: "文登市"
                    }
                }
            },
            cities296: {
                region_id: "296",
                region_name: "潍坊",
                county: {
                    counties2434: {
                        region_id: "2434",
                        region_name: "潍城区"
                    },
                    counties2435: {
                        region_id: "2435",
                        region_name: "寒亭区"
                    },
                    counties2436: {
                        region_id: "2436",
                        region_name: "坊子区"
                    },
                    counties2437: {
                        region_id: "2437",
                        region_name: "奎文区"
                    },
                    counties2438: {
                        region_id: "2438",
                        region_name: "青州市"
                    },
                    counties2439: {
                        region_id: "2439",
                        region_name: "诸城市"
                    },
                    counties2440: {
                        region_id: "2440",
                        region_name: "寿光市"
                    },
                    counties2441: {
                        region_id: "2441",
                        region_name: "安丘市"
                    },
                    counties2442: {
                        region_id: "2442",
                        region_name: "高密市"
                    },
                    counties2443: {
                        region_id: "2443",
                        region_name: "昌邑市"
                    },
                    counties2444: {
                        region_id: "2444",
                        region_name: "临朐县"
                    },
                    counties2445: {
                        region_id: "2445",
                        region_name: "昌乐县"
                    }
                }
            },
            cities297: {
                region_id: "297",
                region_name: "烟台",
                county: {
                    counties2446: {
                        region_id: "2446",
                        region_name: "芝罘区"
                    },
                    counties2447: {
                        region_id: "2447",
                        region_name: "福山区"
                    },
                    counties2448: {
                        region_id: "2448",
                        region_name: "牟平区"
                    },
                    counties2449: {
                        region_id: "2449",
                        region_name: "莱山区"
                    },
                    counties2450: {
                        region_id: "2450",
                        region_name: "开发区"
                    },
                    counties2451: {
                        region_id: "2451",
                        region_name: "龙口市"
                    },
                    counties2452: {
                        region_id: "2452",
                        region_name: "莱阳市"
                    },
                    counties2453: {
                        region_id: "2453",
                        region_name: "莱州市"
                    },
                    counties2454: {
                        region_id: "2454",
                        region_name: "蓬莱市"
                    },
                    counties2455: {
                        region_id: "2455",
                        region_name: "招远市"
                    },
                    counties2456: {
                        region_id: "2456",
                        region_name: "栖霞市"
                    },
                    counties2457: {
                        region_id: "2457",
                        region_name: "海阳市"
                    },
                    counties2458: {
                        region_id: "2458",
                        region_name: "长岛县"
                    }
                }
            },
            cities298: {
                region_id: "298",
                region_name: "枣庄",
                county: {
                    counties2459: {
                        region_id: "2459",
                        region_name: "市中区"
                    },
                    counties2460: {
                        region_id: "2460",
                        region_name: "山亭区"
                    },
                    counties2461: {
                        region_id: "2461",
                        region_name: "峄城区"
                    },
                    counties2462: {
                        region_id: "2462",
                        region_name: "台儿庄区"
                    },
                    counties2463: {
                        region_id: "2463",
                        region_name: "薛城区"
                    },
                    counties2464: {
                        region_id: "2464",
                        region_name: "滕州市"
                    }
                }
            },
            cities299: {
                region_id: "299",
                region_name: "淄博",
                county: {
                    counties2465: {
                        region_id: "2465",
                        region_name: "张店区"
                    },
                    counties2466: {
                        region_id: "2466",
                        region_name: "临淄区"
                    },
                    counties2467: {
                        region_id: "2467",
                        region_name: "淄川区"
                    },
                    counties2468: {
                        region_id: "2468",
                        region_name: "博山区"
                    },
                    counties2469: {
                        region_id: "2469",
                        region_name: "周村区"
                    },
                    counties2470: {
                        region_id: "2470",
                        region_name: "桓台县"
                    },
                    counties2471: {
                        region_id: "2471",
                        region_name: "高青县"
                    },
                    counties2472: {
                        region_id: "2472",
                        region_name: "沂源县"
                    }
                }
            }
        }
    },
    provinces23: {
        region_id: "23",
        region_name: "山西",
        city: {
            cities300: {
                region_id: "300",
                region_name: "太原",
                county: {
                    counties2473: {
                        region_id: "2473",
                        region_name: "杏花岭区"
                    },
                    counties2474: {
                        region_id: "2474",
                        region_name: "小店区"
                    },
                    counties2475: {
                        region_id: "2475",
                        region_name: "迎泽区"
                    },
                    counties2476: {
                        region_id: "2476",
                        region_name: "尖草坪区"
                    },
                    counties2477: {
                        region_id: "2477",
                        region_name: "万柏林区"
                    },
                    counties2478: {
                        region_id: "2478",
                        region_name: "晋源区"
                    },
                    counties2479: {
                        region_id: "2479",
                        region_name: "高新开发区"
                    },
                    counties2480: {
                        region_id: "2480",
                        region_name: "民营经济开发区"
                    },
                    counties2481: {
                        region_id: "2481",
                        region_name: "经济技术开发区"
                    },
                    counties2482: {
                        region_id: "2482",
                        region_name: "清徐县"
                    },
                    counties2483: {
                        region_id: "2483",
                        region_name: "阳曲县"
                    },
                    counties2484: {
                        region_id: "2484",
                        region_name: "娄烦县"
                    },
                    counties2485: {
                        region_id: "2485",
                        region_name: "古交市"
                    }
                }
            },
            cities301: {
                region_id: "301",
                region_name: "长治",
                county: {
                    counties2486: {
                        region_id: "2486",
                        region_name: "城区"
                    },
                    counties2487: {
                        region_id: "2487",
                        region_name: "郊区"
                    },
                    counties2488: {
                        region_id: "2488",
                        region_name: "沁县"
                    },
                    counties2489: {
                        region_id: "2489",
                        region_name: "潞城市"
                    },
                    counties2490: {
                        region_id: "2490",
                        region_name: "长治县"
                    },
                    counties2491: {
                        region_id: "2491",
                        region_name: "襄垣县"
                    },
                    counties2492: {
                        region_id: "2492",
                        region_name: "屯留县"
                    },
                    counties2493: {
                        region_id: "2493",
                        region_name: "平顺县"
                    },
                    counties2494: {
                        region_id: "2494",
                        region_name: "黎城县"
                    },
                    counties2495: {
                        region_id: "2495",
                        region_name: "壶关县"
                    },
                    counties2496: {
                        region_id: "2496",
                        region_name: "长子县"
                    },
                    counties2497: {
                        region_id: "2497",
                        region_name: "武乡县"
                    },
                    counties2498: {
                        region_id: "2498",
                        region_name: "沁源县"
                    }
                }
            },
            cities302: {
                region_id: "302",
                region_name: "大同",
                county: {
                    counties2499: {
                        region_id: "2499",
                        region_name: "城区"
                    },
                    counties2500: {
                        region_id: "2500",
                        region_name: "矿区"
                    },
                    counties2501: {
                        region_id: "2501",
                        region_name: "南郊区"
                    },
                    counties2502: {
                        region_id: "2502",
                        region_name: "新荣区"
                    },
                    counties2503: {
                        region_id: "2503",
                        region_name: "阳高县"
                    },
                    counties2504: {
                        region_id: "2504",
                        region_name: "天镇县"
                    },
                    counties2505: {
                        region_id: "2505",
                        region_name: "广灵县"
                    },
                    counties2506: {
                        region_id: "2506",
                        region_name: "灵丘县"
                    },
                    counties2507: {
                        region_id: "2507",
                        region_name: "浑源县"
                    },
                    counties2508: {
                        region_id: "2508",
                        region_name: "左云县"
                    },
                    counties2509: {
                        region_id: "2509",
                        region_name: "大同县"
                    }
                }
            },
            cities303: {
                region_id: "303",
                region_name: "晋城",
                county: {
                    counties2510: {
                        region_id: "2510",
                        region_name: "城区"
                    },
                    counties2511: {
                        region_id: "2511",
                        region_name: "高平市"
                    },
                    counties2512: {
                        region_id: "2512",
                        region_name: "沁水县"
                    },
                    counties2513: {
                        region_id: "2513",
                        region_name: "阳城县"
                    },
                    counties2514: {
                        region_id: "2514",
                        region_name: "陵川县"
                    },
                    counties2515: {
                        region_id: "2515",
                        region_name: "泽州县"
                    }
                }
            },
            cities304: {
                region_id: "304",
                region_name: "晋中",
                county: {
                    counties2516: {
                        region_id: "2516",
                        region_name: "榆次区"
                    },
                    counties2517: {
                        region_id: "2517",
                        region_name: "介休市"
                    },
                    counties2518: {
                        region_id: "2518",
                        region_name: "榆社县"
                    },
                    counties2519: {
                        region_id: "2519",
                        region_name: "左权县"
                    },
                    counties2520: {
                        region_id: "2520",
                        region_name: "和顺县"
                    },
                    counties2521: {
                        region_id: "2521",
                        region_name: "昔阳县"
                    },
                    counties2522: {
                        region_id: "2522",
                        region_name: "寿阳县"
                    },
                    counties2523: {
                        region_id: "2523",
                        region_name: "太谷县"
                    },
                    counties2524: {
                        region_id: "2524",
                        region_name: "祁县"
                    },
                    counties2525: {
                        region_id: "2525",
                        region_name: "平遥县"
                    },
                    counties2526: {
                        region_id: "2526",
                        region_name: "灵石县"
                    }
                }
            },
            cities305: {
                region_id: "305",
                region_name: "临汾",
                county: {
                    counties2527: {
                        region_id: "2527",
                        region_name: "尧都区"
                    },
                    counties2528: {
                        region_id: "2528",
                        region_name: "侯马市"
                    },
                    counties2529: {
                        region_id: "2529",
                        region_name: "霍州市"
                    },
                    counties2530: {
                        region_id: "2530",
                        region_name: "曲沃县"
                    },
                    counties2531: {
                        region_id: "2531",
                        region_name: "翼城县"
                    },
                    counties2532: {
                        region_id: "2532",
                        region_name: "襄汾县"
                    },
                    counties2533: {
                        region_id: "2533",
                        region_name: "洪洞县"
                    },
                    counties2534: {
                        region_id: "2534",
                        region_name: "吉县"
                    },
                    counties2535: {
                        region_id: "2535",
                        region_name: "安泽县"
                    },
                    counties2536: {
                        region_id: "2536",
                        region_name: "浮山县"
                    },
                    counties2537: {
                        region_id: "2537",
                        region_name: "古县"
                    },
                    counties2538: {
                        region_id: "2538",
                        region_name: "乡宁县"
                    },
                    counties2539: {
                        region_id: "2539",
                        region_name: "大宁县"
                    },
                    counties2540: {
                        region_id: "2540",
                        region_name: "隰县"
                    },
                    counties2541: {
                        region_id: "2541",
                        region_name: "永和县"
                    },
                    counties2542: {
                        region_id: "2542",
                        region_name: "蒲县"
                    },
                    counties2543: {
                        region_id: "2543",
                        region_name: "汾西县"
                    }
                }
            },
            cities306: {
                region_id: "306",
                region_name: "吕梁地区",
                county: {
                    counties2544: {
                        region_id: "2544",
                        region_name: "离石市"
                    },
                    counties2545: {
                        region_id: "2545",
                        region_name: "离石区"
                    },
                    counties2546: {
                        region_id: "2546",
                        region_name: "孝义市"
                    },
                    counties2547: {
                        region_id: "2547",
                        region_name: "汾阳市"
                    },
                    counties2548: {
                        region_id: "2548",
                        region_name: "文水县"
                    },
                    counties2549: {
                        region_id: "2549",
                        region_name: "交城县"
                    },
                    counties2550: {
                        region_id: "2550",
                        region_name: "兴县"
                    },
                    counties2551: {
                        region_id: "2551",
                        region_name: "临县"
                    },
                    counties2552: {
                        region_id: "2552",
                        region_name: "柳林县"
                    },
                    counties2553: {
                        region_id: "2553",
                        region_name: "石楼县"
                    },
                    counties2554: {
                        region_id: "2554",
                        region_name: "岚县"
                    },
                    counties2555: {
                        region_id: "2555",
                        region_name: "方山县"
                    },
                    counties2556: {
                        region_id: "2556",
                        region_name: "中阳县"
                    },
                    counties2557: {
                        region_id: "2557",
                        region_name: "交口县"
                    }
                }
            },
            cities307: {
                region_id: "307",
                region_name: "朔州",
                county: {
                    counties2558: {
                        region_id: "2558",
                        region_name: "朔城区"
                    },
                    counties2559: {
                        region_id: "2559",
                        region_name: "平鲁区"
                    },
                    counties2560: {
                        region_id: "2560",
                        region_name: "山阴县"
                    },
                    counties2561: {
                        region_id: "2561",
                        region_name: "应县"
                    },
                    counties2562: {
                        region_id: "2562",
                        region_name: "右玉县"
                    },
                    counties2563: {
                        region_id: "2563",
                        region_name: "怀仁县"
                    }
                }
            },
            cities308: {
                region_id: "308",
                region_name: "忻州",
                county: {
                    counties2564: {
                        region_id: "2564",
                        region_name: "忻府区"
                    },
                    counties2565: {
                        region_id: "2565",
                        region_name: "原平市"
                    },
                    counties2566: {
                        region_id: "2566",
                        region_name: "定襄县"
                    },
                    counties2567: {
                        region_id: "2567",
                        region_name: "五台县"
                    },
                    counties2568: {
                        region_id: "2568",
                        region_name: "代县"
                    },
                    counties2569: {
                        region_id: "2569",
                        region_name: "繁峙县"
                    },
                    counties2570: {
                        region_id: "2570",
                        region_name: "宁武县"
                    },
                    counties2571: {
                        region_id: "2571",
                        region_name: "静乐县"
                    },
                    counties2572: {
                        region_id: "2572",
                        region_name: "神池县"
                    },
                    counties2573: {
                        region_id: "2573",
                        region_name: "五寨县"
                    },
                    counties2574: {
                        region_id: "2574",
                        region_name: "岢岚县"
                    },
                    counties2575: {
                        region_id: "2575",
                        region_name: "河曲县"
                    },
                    counties2576: {
                        region_id: "2576",
                        region_name: "保德县"
                    },
                    counties2577: {
                        region_id: "2577",
                        region_name: "偏关县"
                    }
                }
            },
            cities309: {
                region_id: "309",
                region_name: "阳泉",
                county: {
                    counties2578: {
                        region_id: "2578",
                        region_name: "城区"
                    },
                    counties2579: {
                        region_id: "2579",
                        region_name: "矿区"
                    },
                    counties2580: {
                        region_id: "2580",
                        region_name: "郊区"
                    },
                    counties2581: {
                        region_id: "2581",
                        region_name: "平定县"
                    },
                    counties2582: {
                        region_id: "2582",
                        region_name: "盂县"
                    }
                }
            },
            cities310: {
                region_id: "310",
                region_name: "运城",
                county: {
                    counties2583: {
                        region_id: "2583",
                        region_name: "盐湖区"
                    },
                    counties2584: {
                        region_id: "2584",
                        region_name: "永济市"
                    },
                    counties2585: {
                        region_id: "2585",
                        region_name: "河津市"
                    },
                    counties2586: {
                        region_id: "2586",
                        region_name: "临猗县"
                    },
                    counties2587: {
                        region_id: "2587",
                        region_name: "万荣县"
                    },
                    counties2588: {
                        region_id: "2588",
                        region_name: "闻喜县"
                    },
                    counties2589: {
                        region_id: "2589",
                        region_name: "稷山县"
                    },
                    counties2590: {
                        region_id: "2590",
                        region_name: "新绛县"
                    },
                    counties2591: {
                        region_id: "2591",
                        region_name: "绛县"
                    },
                    counties2592: {
                        region_id: "2592",
                        region_name: "垣曲县"
                    },
                    counties2593: {
                        region_id: "2593",
                        region_name: "夏县"
                    },
                    counties2594: {
                        region_id: "2594",
                        region_name: "平陆县"
                    },
                    counties2595: {
                        region_id: "2595",
                        region_name: "芮城县"
                    }
                }
            }
        }
    },
    provinces24: {
        region_id: "24",
        region_name: "陕西",
        city: {
            cities311: {
                region_id: "311",
                region_name: "西安",
                county: {
                    counties2596: {
                        region_id: "2596",
                        region_name: "莲湖区"
                    },
                    counties2597: {
                        region_id: "2597",
                        region_name: "新城区"
                    },
                    counties2598: {
                        region_id: "2598",
                        region_name: "碑林区"
                    },
                    counties2599: {
                        region_id: "2599",
                        region_name: "雁塔区"
                    },
                    counties2600: {
                        region_id: "2600",
                        region_name: "灞桥区"
                    },
                    counties2601: {
                        region_id: "2601",
                        region_name: "未央区"
                    },
                    counties2602: {
                        region_id: "2602",
                        region_name: "阎良区"
                    },
                    counties2603: {
                        region_id: "2603",
                        region_name: "临潼区"
                    },
                    counties2604: {
                        region_id: "2604",
                        region_name: "长安区"
                    },
                    counties2605: {
                        region_id: "2605",
                        region_name: "蓝田县"
                    },
                    counties2606: {
                        region_id: "2606",
                        region_name: "周至县"
                    },
                    counties2607: {
                        region_id: "2607",
                        region_name: "户县"
                    },
                    counties2608: {
                        region_id: "2608",
                        region_name: "高陵县"
                    }
                }
            },
            cities312: {
                region_id: "312",
                region_name: "安康",
                county: {
                    counties2609: {
                        region_id: "2609",
                        region_name: "汉滨区"
                    },
                    counties2610: {
                        region_id: "2610",
                        region_name: "汉阴县"
                    },
                    counties2611: {
                        region_id: "2611",
                        region_name: "石泉县"
                    },
                    counties2612: {
                        region_id: "2612",
                        region_name: "宁陕县"
                    },
                    counties2613: {
                        region_id: "2613",
                        region_name: "紫阳县"
                    },
                    counties2614: {
                        region_id: "2614",
                        region_name: "岚皋县"
                    },
                    counties2615: {
                        region_id: "2615",
                        region_name: "平利县"
                    },
                    counties2616: {
                        region_id: "2616",
                        region_name: "镇坪县"
                    },
                    counties2617: {
                        region_id: "2617",
                        region_name: "旬阳县"
                    },
                    counties2618: {
                        region_id: "2618",
                        region_name: "白河县"
                    }
                }
            },
            cities313: {
                region_id: "313",
                region_name: "宝鸡",
                county: {
                    counties2619: {
                        region_id: "2619",
                        region_name: "陈仓区"
                    },
                    counties2620: {
                        region_id: "2620",
                        region_name: "渭滨区"
                    },
                    counties2621: {
                        region_id: "2621",
                        region_name: "金台区"
                    },
                    counties2622: {
                        region_id: "2622",
                        region_name: "凤翔县"
                    },
                    counties2623: {
                        region_id: "2623",
                        region_name: "岐山县"
                    },
                    counties2624: {
                        region_id: "2624",
                        region_name: "扶风县"
                    },
                    counties2625: {
                        region_id: "2625",
                        region_name: "眉县"
                    },
                    counties2626: {
                        region_id: "2626",
                        region_name: "陇县"
                    },
                    counties2627: {
                        region_id: "2627",
                        region_name: "千阳县"
                    },
                    counties2628: {
                        region_id: "2628",
                        region_name: "麟游县"
                    },
                    counties2629: {
                        region_id: "2629",
                        region_name: "凤县"
                    },
                    counties2630: {
                        region_id: "2630",
                        region_name: "太白县"
                    }
                }
            },
            cities314: {
                region_id: "314",
                region_name: "汉中",
                county: {
                    counties2631: {
                        region_id: "2631",
                        region_name: "汉台区"
                    },
                    counties2632: {
                        region_id: "2632",
                        region_name: "南郑县"
                    },
                    counties2633: {
                        region_id: "2633",
                        region_name: "城固县"
                    },
                    counties2634: {
                        region_id: "2634",
                        region_name: "洋县"
                    },
                    counties2635: {
                        region_id: "2635",
                        region_name: "西乡县"
                    },
                    counties2636: {
                        region_id: "2636",
                        region_name: "勉县"
                    },
                    counties2637: {
                        region_id: "2637",
                        region_name: "宁强县"
                    },
                    counties2638: {
                        region_id: "2638",
                        region_name: "略阳县"
                    },
                    counties2639: {
                        region_id: "2639",
                        region_name: "镇巴县"
                    },
                    counties2640: {
                        region_id: "2640",
                        region_name: "留坝县"
                    },
                    counties2641: {
                        region_id: "2641",
                        region_name: "佛坪县"
                    }
                }
            },
            cities315: {
                region_id: "315",
                region_name: "商洛",
                county: {
                    counties2642: {
                        region_id: "2642",
                        region_name: "商州区"
                    },
                    counties2643: {
                        region_id: "2643",
                        region_name: "洛南县"
                    },
                    counties2644: {
                        region_id: "2644",
                        region_name: "丹凤县"
                    },
                    counties2645: {
                        region_id: "2645",
                        region_name: "商南县"
                    },
                    counties2646: {
                        region_id: "2646",
                        region_name: "山阳县"
                    },
                    counties2647: {
                        region_id: "2647",
                        region_name: "镇安县"
                    },
                    counties2648: {
                        region_id: "2648",
                        region_name: "柞水县"
                    }
                }
            },
            cities316: {
                region_id: "316",
                region_name: "铜川",
                county: {
                    counties2649: {
                        region_id: "2649",
                        region_name: "耀州区"
                    },
                    counties2650: {
                        region_id: "2650",
                        region_name: "王益区"
                    },
                    counties2651: {
                        region_id: "2651",
                        region_name: "印台区"
                    },
                    counties2652: {
                        region_id: "2652",
                        region_name: "宜君县"
                    }
                }
            },
            cities317: {
                region_id: "317",
                region_name: "渭南",
                county: {
                    counties2653: {
                        region_id: "2653",
                        region_name: "临渭区"
                    },
                    counties2654: {
                        region_id: "2654",
                        region_name: "韩城市"
                    },
                    counties2655: {
                        region_id: "2655",
                        region_name: "华阴市"
                    },
                    counties2656: {
                        region_id: "2656",
                        region_name: "华县"
                    },
                    counties2657: {
                        region_id: "2657",
                        region_name: "潼关县"
                    },
                    counties2658: {
                        region_id: "2658",
                        region_name: "大荔县"
                    },
                    counties2659: {
                        region_id: "2659",
                        region_name: "合阳县"
                    },
                    counties2660: {
                        region_id: "2660",
                        region_name: "澄城县"
                    },
                    counties2661: {
                        region_id: "2661",
                        region_name: "蒲城县"
                    },
                    counties2662: {
                        region_id: "2662",
                        region_name: "白水县"
                    },
                    counties2663: {
                        region_id: "2663",
                        region_name: "富平县"
                    }
                }
            },
            cities318: {
                region_id: "318",
                region_name: "咸阳",
                county: {
                    counties2664: {
                        region_id: "2664",
                        region_name: "秦都区"
                    },
                    counties2665: {
                        region_id: "2665",
                        region_name: "渭城区"
                    },
                    counties2666: {
                        region_id: "2666",
                        region_name: "杨陵区"
                    },
                    counties2667: {
                        region_id: "2667",
                        region_name: "兴平市"
                    },
                    counties2668: {
                        region_id: "2668",
                        region_name: "三原县"
                    },
                    counties2669: {
                        region_id: "2669",
                        region_name: "泾阳县"
                    },
                    counties2670: {
                        region_id: "2670",
                        region_name: "乾县"
                    },
                    counties2671: {
                        region_id: "2671",
                        region_name: "礼泉县"
                    },
                    counties2672: {
                        region_id: "2672",
                        region_name: "永寿县"
                    },
                    counties2673: {
                        region_id: "2673",
                        region_name: "彬县"
                    },
                    counties2674: {
                        region_id: "2674",
                        region_name: "长武县"
                    },
                    counties2675: {
                        region_id: "2675",
                        region_name: "旬邑县"
                    },
                    counties2676: {
                        region_id: "2676",
                        region_name: "淳化县"
                    },
                    counties2677: {
                        region_id: "2677",
                        region_name: "武功县"
                    }
                }
            },
            cities319: {
                region_id: "319",
                region_name: "延安",
                county: {
                    counties2678: {
                        region_id: "2678",
                        region_name: "吴起县"
                    },
                    counties2679: {
                        region_id: "2679",
                        region_name: "宝塔区"
                    },
                    counties2680: {
                        region_id: "2680",
                        region_name: "延长县"
                    },
                    counties2681: {
                        region_id: "2681",
                        region_name: "延川县"
                    },
                    counties2682: {
                        region_id: "2682",
                        region_name: "子长县"
                    },
                    counties2683: {
                        region_id: "2683",
                        region_name: "安塞县"
                    },
                    counties2684: {
                        region_id: "2684",
                        region_name: "志丹县"
                    },
                    counties2685: {
                        region_id: "2685",
                        region_name: "甘泉县"
                    },
                    counties2686: {
                        region_id: "2686",
                        region_name: "富县"
                    },
                    counties2687: {
                        region_id: "2687",
                        region_name: "洛川县"
                    },
                    counties2688: {
                        region_id: "2688",
                        region_name: "宜川县"
                    },
                    counties2689: {
                        region_id: "2689",
                        region_name: "黄龙县"
                    },
                    counties2690: {
                        region_id: "2690",
                        region_name: "黄陵县"
                    }
                }
            },
            cities320: {
                region_id: "320",
                region_name: "榆林",
                county: {
                    counties2691: {
                        region_id: "2691",
                        region_name: "榆阳区"
                    },
                    counties2692: {
                        region_id: "2692",
                        region_name: "神木县"
                    },
                    counties2693: {
                        region_id: "2693",
                        region_name: "府谷县"
                    },
                    counties2694: {
                        region_id: "2694",
                        region_name: "横山县"
                    },
                    counties2695: {
                        region_id: "2695",
                        region_name: "靖边县"
                    },
                    counties2696: {
                        region_id: "2696",
                        region_name: "定边县"
                    },
                    counties2697: {
                        region_id: "2697",
                        region_name: "绥德县"
                    },
                    counties2698: {
                        region_id: "2698",
                        region_name: "米脂县"
                    },
                    counties2699: {
                        region_id: "2699",
                        region_name: "佳县"
                    },
                    counties2700: {
                        region_id: "2700",
                        region_name: "吴堡县"
                    },
                    counties2701: {
                        region_id: "2701",
                        region_name: "清涧县"
                    },
                    counties2702: {
                        region_id: "2702",
                        region_name: "子洲县"
                    }
                }
            }
        }
    },
    provinces25: {
        region_id: "25",
        region_name: "上海",
        city: {
            cities321: {
                region_id: "321",
                region_name: "上海",
                county: {
                    counties2703: {
                        region_id: "2703",
                        region_name: "长宁区"
                    },
                    counties2704: {
                        region_id: "2704",
                        region_name: "闸北区"
                    },
                    counties2705: {
                        region_id: "2705",
                        region_name: "闵行区"
                    },
                    counties2706: {
                        region_id: "2706",
                        region_name: "徐汇区"
                    },
                    counties2707: {
                        region_id: "2707",
                        region_name: "浦东新区"
                    },
                    counties2708: {
                        region_id: "2708",
                        region_name: "杨浦区"
                    },
                    counties2709: {
                        region_id: "2709",
                        region_name: "普陀区"
                    },
                    counties2710: {
                        region_id: "2710",
                        region_name: "静安区"
                    },
                    counties2711: {
                        region_id: "2711",
                        region_name: "卢湾区"
                    },
                    counties2712: {
                        region_id: "2712",
                        region_name: "虹口区"
                    },
                    counties2713: {
                        region_id: "2713",
                        region_name: "黄浦区"
                    },
                    counties2714: {
                        region_id: "2714",
                        region_name: "南汇区"
                    },
                    counties2715: {
                        region_id: "2715",
                        region_name: "松江区"
                    },
                    counties2716: {
                        region_id: "2716",
                        region_name: "嘉定区"
                    },
                    counties2717: {
                        region_id: "2717",
                        region_name: "宝山区"
                    },
                    counties2718: {
                        region_id: "2718",
                        region_name: "青浦区"
                    },
                    counties2719: {
                        region_id: "2719",
                        region_name: "金山区"
                    },
                    counties2720: {
                        region_id: "2720",
                        region_name: "奉贤区"
                    },
                    counties2721: {
                        region_id: "2721",
                        region_name: "崇明县"
                    }
                }
            }
        }
    },
    provinces26: {
        region_id: "26",
        region_name: "四川",
        city: {
            cities322: {
                region_id: "322",
                region_name: "成都",
                county: {
                    counties2722: {
                        region_id: "2722",
                        region_name: "青羊区"
                    },
                    counties2723: {
                        region_id: "2723",
                        region_name: "锦江区"
                    },
                    counties2724: {
                        region_id: "2724",
                        region_name: "金牛区"
                    },
                    counties2725: {
                        region_id: "2725",
                        region_name: "武侯区"
                    },
                    counties2726: {
                        region_id: "2726",
                        region_name: "成华区"
                    },
                    counties2727: {
                        region_id: "2727",
                        region_name: "龙泉驿区"
                    },
                    counties2728: {
                        region_id: "2728",
                        region_name: "青白江区"
                    },
                    counties2729: {
                        region_id: "2729",
                        region_name: "新都区"
                    },
                    counties2730: {
                        region_id: "2730",
                        region_name: "温江区"
                    },
                    counties2731: {
                        region_id: "2731",
                        region_name: "高新区"
                    },
                    counties2732: {
                        region_id: "2732",
                        region_name: "高新西区"
                    },
                    counties2733: {
                        region_id: "2733",
                        region_name: "都江堰市"
                    },
                    counties2734: {
                        region_id: "2734",
                        region_name: "彭州市"
                    },
                    counties2735: {
                        region_id: "2735",
                        region_name: "邛崃市"
                    },
                    counties2736: {
                        region_id: "2736",
                        region_name: "崇州市"
                    },
                    counties2737: {
                        region_id: "2737",
                        region_name: "金堂县"
                    },
                    counties2738: {
                        region_id: "2738",
                        region_name: "双流县"
                    },
                    counties2739: {
                        region_id: "2739",
                        region_name: "郫县"
                    },
                    counties2740: {
                        region_id: "2740",
                        region_name: "大邑县"
                    },
                    counties2741: {
                        region_id: "2741",
                        region_name: "蒲江县"
                    },
                    counties2742: {
                        region_id: "2742",
                        region_name: "新津县"
                    },
                    counties2743: {
                        region_id: "2743",
                        region_name: "都江堰市"
                    },
                    counties2744: {
                        region_id: "2744",
                        region_name: "彭州市"
                    },
                    counties2745: {
                        region_id: "2745",
                        region_name: "邛崃市"
                    },
                    counties2746: {
                        region_id: "2746",
                        region_name: "崇州市"
                    },
                    counties2747: {
                        region_id: "2747",
                        region_name: "金堂县"
                    },
                    counties2748: {
                        region_id: "2748",
                        region_name: "双流县"
                    },
                    counties2749: {
                        region_id: "2749",
                        region_name: "郫县"
                    },
                    counties2750: {
                        region_id: "2750",
                        region_name: "大邑县"
                    },
                    counties2751: {
                        region_id: "2751",
                        region_name: "蒲江县"
                    },
                    counties2752: {
                        region_id: "2752",
                        region_name: "新津县"
                    }
                }
            },
            cities323: {
                region_id: "323",
                region_name: "绵阳",
                county: {
                    counties2753: {
                        region_id: "2753",
                        region_name: "涪城区"
                    },
                    counties2754: {
                        region_id: "2754",
                        region_name: "游仙区"
                    },
                    counties2755: {
                        region_id: "2755",
                        region_name: "江油市"
                    },
                    counties2756: {
                        region_id: "2756",
                        region_name: "盐亭县"
                    },
                    counties2757: {
                        region_id: "2757",
                        region_name: "三台县"
                    },
                    counties2758: {
                        region_id: "2758",
                        region_name: "平武县"
                    },
                    counties2759: {
                        region_id: "2759",
                        region_name: "安县"
                    },
                    counties2760: {
                        region_id: "2760",
                        region_name: "梓潼县"
                    },
                    counties2761: {
                        region_id: "2761",
                        region_name: "北川县"
                    }
                }
            },
            cities324: {
                region_id: "324",
                region_name: "阿坝藏族羌族自治州",
                county: {
                    counties2762: {
                        region_id: "2762",
                        region_name: "马尔康县"
                    },
                    counties2763: {
                        region_id: "2763",
                        region_name: "汶川县"
                    },
                    counties2764: {
                        region_id: "2764",
                        region_name: "理县"
                    },
                    counties2765: {
                        region_id: "2765",
                        region_name: "茂县"
                    },
                    counties2766: {
                        region_id: "2766",
                        region_name: "松潘县"
                    },
                    counties2767: {
                        region_id: "2767",
                        region_name: "九寨沟县"
                    },
                    counties2768: {
                        region_id: "2768",
                        region_name: "金川县"
                    },
                    counties2769: {
                        region_id: "2769",
                        region_name: "小金县"
                    },
                    counties2770: {
                        region_id: "2770",
                        region_name: "黑水县"
                    },
                    counties2771: {
                        region_id: "2771",
                        region_name: "壤塘县"
                    },
                    counties2772: {
                        region_id: "2772",
                        region_name: "阿坝县"
                    },
                    counties2773: {
                        region_id: "2773",
                        region_name: "若尔盖县"
                    },
                    counties2774: {
                        region_id: "2774",
                        region_name: "红原县"
                    }
                }
            },
            cities325: {
                region_id: "325",
                region_name: "巴中",
                county: {
                    counties2775: {
                        region_id: "2775",
                        region_name: "巴州区"
                    },
                    counties2776: {
                        region_id: "2776",
                        region_name: "通江县"
                    },
                    counties2777: {
                        region_id: "2777",
                        region_name: "南江县"
                    },
                    counties2778: {
                        region_id: "2778",
                        region_name: "平昌县"
                    }
                }
            },
            cities326: {
                region_id: "326",
                region_name: "达州",
                county: {
                    counties2779: {
                        region_id: "2779",
                        region_name: "通川区"
                    },
                    counties2780: {
                        region_id: "2780",
                        region_name: "万源市"
                    },
                    counties2781: {
                        region_id: "2781",
                        region_name: "达县"
                    },
                    counties2782: {
                        region_id: "2782",
                        region_name: "宣汉县"
                    },
                    counties2783: {
                        region_id: "2783",
                        region_name: "开江县"
                    },
                    counties2784: {
                        region_id: "2784",
                        region_name: "大竹县"
                    },
                    counties2785: {
                        region_id: "2785",
                        region_name: "渠县"
                    }
                }
            },
            cities327: {
                region_id: "327",
                region_name: "德阳",
                county: {
                    counties2786: {
                        region_id: "2786",
                        region_name: "旌阳区"
                    },
                    counties2787: {
                        region_id: "2787",
                        region_name: "广汉市"
                    },
                    counties2788: {
                        region_id: "2788",
                        region_name: "什邡市"
                    },
                    counties2789: {
                        region_id: "2789",
                        region_name: "绵竹市"
                    },
                    counties2790: {
                        region_id: "2790",
                        region_name: "罗江县"
                    },
                    counties2791: {
                        region_id: "2791",
                        region_name: "中江县"
                    }
                }
            },
            cities328: {
                region_id: "328",
                region_name: "甘孜藏族自治州",
                county: {
                    counties2792: {
                        region_id: "2792",
                        region_name: "康定县"
                    },
                    counties2793: {
                        region_id: "2793",
                        region_name: "丹巴县"
                    },
                    counties2794: {
                        region_id: "2794",
                        region_name: "泸定县"
                    },
                    counties2795: {
                        region_id: "2795",
                        region_name: "炉霍县"
                    },
                    counties2796: {
                        region_id: "2796",
                        region_name: "九龙县"
                    },
                    counties2797: {
                        region_id: "2797",
                        region_name: "甘孜县"
                    },
                    counties2798: {
                        region_id: "2798",
                        region_name: "雅江县"
                    },
                    counties2799: {
                        region_id: "2799",
                        region_name: "新龙县"
                    },
                    counties2800: {
                        region_id: "2800",
                        region_name: "道孚县"
                    },
                    counties2801: {
                        region_id: "2801",
                        region_name: "白玉县"
                    },
                    counties2802: {
                        region_id: "2802",
                        region_name: "理塘县"
                    },
                    counties2803: {
                        region_id: "2803",
                        region_name: "德格县"
                    },
                    counties2804: {
                        region_id: "2804",
                        region_name: "乡城县"
                    },
                    counties2805: {
                        region_id: "2805",
                        region_name: "石渠县"
                    },
                    counties2806: {
                        region_id: "2806",
                        region_name: "稻城县"
                    },
                    counties2807: {
                        region_id: "2807",
                        region_name: "色达县"
                    },
                    counties2808: {
                        region_id: "2808",
                        region_name: "巴塘县"
                    },
                    counties2809: {
                        region_id: "2809",
                        region_name: "得荣县"
                    }
                }
            },
            cities329: {
                region_id: "329",
                region_name: "广安",
                county: {
                    counties2810: {
                        region_id: "2810",
                        region_name: "广安区"
                    },
                    counties2811: {
                        region_id: "2811",
                        region_name: "华蓥市"
                    },
                    counties2812: {
                        region_id: "2812",
                        region_name: "岳池县"
                    },
                    counties2813: {
                        region_id: "2813",
                        region_name: "武胜县"
                    },
                    counties2814: {
                        region_id: "2814",
                        region_name: "邻水县"
                    }
                }
            },
            cities330: {
                region_id: "330",
                region_name: "广元",
                county: {
                    counties2815: {
                        region_id: "2815",
                        region_name: "利州区"
                    },
                    counties2816: {
                        region_id: "2816",
                        region_name: "元坝区"
                    },
                    counties2817: {
                        region_id: "2817",
                        region_name: "朝天区"
                    },
                    counties2818: {
                        region_id: "2818",
                        region_name: "旺苍县"
                    },
                    counties2819: {
                        region_id: "2819",
                        region_name: "青川县"
                    },
                    counties2820: {
                        region_id: "2820",
                        region_name: "剑阁县"
                    },
                    counties2821: {
                        region_id: "2821",
                        region_name: "苍溪县"
                    }
                }
            },
            cities331: {
                region_id: "331",
                region_name: "乐山",
                county: {
                    counties2822: {
                        region_id: "2822",
                        region_name: "峨眉山市"
                    },
                    counties2823: {
                        region_id: "2823",
                        region_name: "乐山市"
                    },
                    counties2824: {
                        region_id: "2824",
                        region_name: "犍为县"
                    },
                    counties2825: {
                        region_id: "2825",
                        region_name: "井研县"
                    },
                    counties2826: {
                        region_id: "2826",
                        region_name: "夹江县"
                    },
                    counties2827: {
                        region_id: "2827",
                        region_name: "沐川县"
                    },
                    counties2828: {
                        region_id: "2828",
                        region_name: "峨边"
                    },
                    counties2829: {
                        region_id: "2829",
                        region_name: "马边"
                    }
                }
            },
            cities332: {
                region_id: "332",
                region_name: "凉山彝族自治州",
                county: {
                    counties2830: {
                        region_id: "2830",
                        region_name: "西昌市"
                    },
                    counties2831: {
                        region_id: "2831",
                        region_name: "盐源县"
                    },
                    counties2832: {
                        region_id: "2832",
                        region_name: "德昌县"
                    },
                    counties2833: {
                        region_id: "2833",
                        region_name: "会理县"
                    },
                    counties2834: {
                        region_id: "2834",
                        region_name: "会东县"
                    },
                    counties2835: {
                        region_id: "2835",
                        region_name: "宁南县"
                    },
                    counties2836: {
                        region_id: "2836",
                        region_name: "普格县"
                    },
                    counties2837: {
                        region_id: "2837",
                        region_name: "布拖县"
                    },
                    counties2838: {
                        region_id: "2838",
                        region_name: "金阳县"
                    },
                    counties2839: {
                        region_id: "2839",
                        region_name: "昭觉县"
                    },
                    counties2840: {
                        region_id: "2840",
                        region_name: "喜德县"
                    },
                    counties2841: {
                        region_id: "2841",
                        region_name: "冕宁县"
                    },
                    counties2842: {
                        region_id: "2842",
                        region_name: "越西县"
                    },
                    counties2843: {
                        region_id: "2843",
                        region_name: "甘洛县"
                    },
                    counties2844: {
                        region_id: "2844",
                        region_name: "美姑县"
                    },
                    counties2845: {
                        region_id: "2845",
                        region_name: "雷波县"
                    },
                    counties2846: {
                        region_id: "2846",
                        region_name: "木里"
                    }
                }
            },
            cities333: {
                region_id: "333",
                region_name: "眉山",
                county: {
                    counties2847: {
                        region_id: "2847",
                        region_name: "东坡区"
                    },
                    counties2848: {
                        region_id: "2848",
                        region_name: "仁寿县"
                    },
                    counties2849: {
                        region_id: "2849",
                        region_name: "彭山县"
                    },
                    counties2850: {
                        region_id: "2850",
                        region_name: "洪雅县"
                    },
                    counties2851: {
                        region_id: "2851",
                        region_name: "丹棱县"
                    },
                    counties2852: {
                        region_id: "2852",
                        region_name: "青神县"
                    }
                }
            },
            cities334: {
                region_id: "334",
                region_name: "南充",
                county: {
                    counties2853: {
                        region_id: "2853",
                        region_name: "阆中市"
                    },
                    counties2854: {
                        region_id: "2854",
                        region_name: "南部县"
                    },
                    counties2855: {
                        region_id: "2855",
                        region_name: "营山县"
                    },
                    counties2856: {
                        region_id: "2856",
                        region_name: "蓬安县"
                    },
                    counties2857: {
                        region_id: "2857",
                        region_name: "仪陇县"
                    },
                    counties2858: {
                        region_id: "2858",
                        region_name: "顺庆区"
                    },
                    counties2859: {
                        region_id: "2859",
                        region_name: "高坪区"
                    },
                    counties2860: {
                        region_id: "2860",
                        region_name: "嘉陵区"
                    },
                    counties2861: {
                        region_id: "2861",
                        region_name: "西充县"
                    }
                }
            },
            cities335: {
                region_id: "335",
                region_name: "内江",
                county: {
                    counties2862: {
                        region_id: "2862",
                        region_name: "市中区"
                    },
                    counties2863: {
                        region_id: "2863",
                        region_name: "东兴区"
                    },
                    counties2864: {
                        region_id: "2864",
                        region_name: "威远县"
                    },
                    counties2865: {
                        region_id: "2865",
                        region_name: "资中县"
                    },
                    counties2866: {
                        region_id: "2866",
                        region_name: "隆昌县"
                    }
                }
            },
            cities336: {
                region_id: "336",
                region_name: "攀枝花",
                county: {
                    counties2867: {
                        region_id: "2867",
                        region_name: "东 区"
                    },
                    counties2868: {
                        region_id: "2868",
                        region_name: "西 区"
                    },
                    counties2869: {
                        region_id: "2869",
                        region_name: "仁和区"
                    },
                    counties2870: {
                        region_id: "2870",
                        region_name: "米易县"
                    },
                    counties2871: {
                        region_id: "2871",
                        region_name: "盐边县"
                    }
                }
            },
            cities337: {
                region_id: "337",
                region_name: "遂宁",
                county: {
                    counties2872: {
                        region_id: "2872",
                        region_name: "船山区"
                    },
                    counties2873: {
                        region_id: "2873",
                        region_name: "安居区"
                    },
                    counties2874: {
                        region_id: "2874",
                        region_name: "蓬溪县"
                    },
                    counties2875: {
                        region_id: "2875",
                        region_name: "射洪县"
                    },
                    counties2876: {
                        region_id: "2876",
                        region_name: "大英县"
                    }
                }
            },
            cities338: {
                region_id: "338",
                region_name: "雅安",
                county: {
                    counties2877: {
                        region_id: "2877",
                        region_name: "雨城区"
                    },
                    counties2878: {
                        region_id: "2878",
                        region_name: "名山县"
                    },
                    counties2879: {
                        region_id: "2879",
                        region_name: "荥经县"
                    },
                    counties2880: {
                        region_id: "2880",
                        region_name: "汉源县"
                    },
                    counties2881: {
                        region_id: "2881",
                        region_name: "石棉县"
                    },
                    counties2882: {
                        region_id: "2882",
                        region_name: "天全县"
                    },
                    counties2883: {
                        region_id: "2883",
                        region_name: "芦山县"
                    },
                    counties2884: {
                        region_id: "2884",
                        region_name: "宝兴县"
                    }
                }
            },
            cities339: {
                region_id: "339",
                region_name: "宜宾",
                county: {
                    counties2885: {
                        region_id: "2885",
                        region_name: "翠屏区"
                    },
                    counties2886: {
                        region_id: "2886",
                        region_name: "宜宾县"
                    },
                    counties2887: {
                        region_id: "2887",
                        region_name: "南溪县"
                    },
                    counties2888: {
                        region_id: "2888",
                        region_name: "江安县"
                    },
                    counties2889: {
                        region_id: "2889",
                        region_name: "长宁县"
                    },
                    counties2890: {
                        region_id: "2890",
                        region_name: "高县"
                    },
                    counties2891: {
                        region_id: "2891",
                        region_name: "珙县"
                    },
                    counties2892: {
                        region_id: "2892",
                        region_name: "筠连县"
                    },
                    counties2893: {
                        region_id: "2893",
                        region_name: "兴文县"
                    },
                    counties2894: {
                        region_id: "2894",
                        region_name: "屏山县"
                    }
                }
            },
            cities340: {
                region_id: "340",
                region_name: "资阳",
                county: {
                    counties2895: {
                        region_id: "2895",
                        region_name: "雁江区"
                    },
                    counties2896: {
                        region_id: "2896",
                        region_name: "简阳市"
                    },
                    counties2897: {
                        region_id: "2897",
                        region_name: "安岳县"
                    },
                    counties2898: {
                        region_id: "2898",
                        region_name: "乐至县"
                    }
                }
            },
            cities341: {
                region_id: "341",
                region_name: "自贡",
                county: {
                    counties2899: {
                        region_id: "2899",
                        region_name: "大安区"
                    },
                    counties2900: {
                        region_id: "2900",
                        region_name: "自流井区"
                    },
                    counties2901: {
                        region_id: "2901",
                        region_name: "贡井区"
                    },
                    counties2902: {
                        region_id: "2902",
                        region_name: "沿滩区"
                    },
                    counties2903: {
                        region_id: "2903",
                        region_name: "荣县"
                    },
                    counties2904: {
                        region_id: "2904",
                        region_name: "富顺县"
                    }
                }
            },
            cities342: {
                region_id: "342",
                region_name: "泸州",
                county: {
                    counties2905: {
                        region_id: "2905",
                        region_name: "江阳区"
                    },
                    counties2906: {
                        region_id: "2906",
                        region_name: "纳溪区"
                    },
                    counties2907: {
                        region_id: "2907",
                        region_name: "龙马潭区"
                    },
                    counties2908: {
                        region_id: "2908",
                        region_name: "泸县"
                    },
                    counties2909: {
                        region_id: "2909",
                        region_name: "合江县"
                    },
                    counties2910: {
                        region_id: "2910",
                        region_name: "叙永县"
                    },
                    counties2911: {
                        region_id: "2911",
                        region_name: "古蔺县"
                    }
                }
            }
        }
    },
    provinces27: {
        region_id: "27",
        region_name: "天津",
        city: {
            cities343: {
                region_id: "343",
                region_name: "天津",
                county: {
                    counties2912: {
                        region_id: "2912",
                        region_name: "和平区"
                    },
                    counties2913: {
                        region_id: "2913",
                        region_name: "河西区"
                    },
                    counties2914: {
                        region_id: "2914",
                        region_name: "南开区"
                    },
                    counties2915: {
                        region_id: "2915",
                        region_name: "河北区"
                    },
                    counties2916: {
                        region_id: "2916",
                        region_name: "河东区"
                    },
                    counties2917: {
                        region_id: "2917",
                        region_name: "红桥区"
                    },
                    counties2918: {
                        region_id: "2918",
                        region_name: "东丽区"
                    },
                    counties2919: {
                        region_id: "2919",
                        region_name: "津南区"
                    },
                    counties2920: {
                        region_id: "2920",
                        region_name: "西青区"
                    },
                    counties2921: {
                        region_id: "2921",
                        region_name: "北辰区"
                    },
                    counties2922: {
                        region_id: "2922",
                        region_name: "塘沽区"
                    },
                    counties2923: {
                        region_id: "2923",
                        region_name: "汉沽区"
                    },
                    counties2924: {
                        region_id: "2924",
                        region_name: "大港区"
                    },
                    counties2925: {
                        region_id: "2925",
                        region_name: "武清区"
                    },
                    counties2926: {
                        region_id: "2926",
                        region_name: "宝坻区"
                    },
                    counties2927: {
                        region_id: "2927",
                        region_name: "经济开发区"
                    },
                    counties2928: {
                        region_id: "2928",
                        region_name: "宁河县"
                    },
                    counties2929: {
                        region_id: "2929",
                        region_name: "静海县"
                    },
                    counties2930: {
                        region_id: "2930",
                        region_name: "蓟县"
                    }
                }
            }
        }
    },
    provinces28: {
        region_id: "28",
        region_name: "西藏",
        city: {
            cities344: {
                region_id: "344",
                region_name: "拉萨",
                county: {
                    counties2931: {
                        region_id: "2931",
                        region_name: "城关区"
                    },
                    counties2932: {
                        region_id: "2932",
                        region_name: "林周县"
                    },
                    counties2933: {
                        region_id: "2933",
                        region_name: "当雄县"
                    },
                    counties2934: {
                        region_id: "2934",
                        region_name: "尼木县"
                    },
                    counties2935: {
                        region_id: "2935",
                        region_name: "曲水县"
                    },
                    counties2936: {
                        region_id: "2936",
                        region_name: "堆龙德庆县"
                    },
                    counties2937: {
                        region_id: "2937",
                        region_name: "达孜县"
                    },
                    counties2938: {
                        region_id: "2938",
                        region_name: "墨竹工卡县"
                    }
                }
            },
            cities345: {
                region_id: "345",
                region_name: "阿里",
                county: {
                    counties2939: {
                        region_id: "2939",
                        region_name: "噶尔县"
                    },
                    counties2940: {
                        region_id: "2940",
                        region_name: "普兰县"
                    },
                    counties2941: {
                        region_id: "2941",
                        region_name: "札达县"
                    },
                    counties2942: {
                        region_id: "2942",
                        region_name: "日土县"
                    },
                    counties2943: {
                        region_id: "2943",
                        region_name: "革吉县"
                    },
                    counties2944: {
                        region_id: "2944",
                        region_name: "改则县"
                    },
                    counties2945: {
                        region_id: "2945",
                        region_name: "措勤县"
                    }
                }
            },
            cities346: {
                region_id: "346",
                region_name: "昌都",
                county: {
                    counties2946: {
                        region_id: "2946",
                        region_name: "昌都县"
                    },
                    counties2947: {
                        region_id: "2947",
                        region_name: "江达县"
                    },
                    counties2948: {
                        region_id: "2948",
                        region_name: "贡觉县"
                    },
                    counties2949: {
                        region_id: "2949",
                        region_name: "类乌齐县"
                    },
                    counties2950: {
                        region_id: "2950",
                        region_name: "丁青县"
                    },
                    counties2951: {
                        region_id: "2951",
                        region_name: "察雅县"
                    },
                    counties2952: {
                        region_id: "2952",
                        region_name: "八宿县"
                    },
                    counties2953: {
                        region_id: "2953",
                        region_name: "左贡县"
                    },
                    counties2954: {
                        region_id: "2954",
                        region_name: "芒康县"
                    },
                    counties2955: {
                        region_id: "2955",
                        region_name: "洛隆县"
                    },
                    counties2956: {
                        region_id: "2956",
                        region_name: "边坝县"
                    }
                }
            },
            cities347: {
                region_id: "347",
                region_name: "林芝",
                county: {
                    counties2957: {
                        region_id: "2957",
                        region_name: "林芝县"
                    },
                    counties2958: {
                        region_id: "2958",
                        region_name: "工布江达县"
                    },
                    counties2959: {
                        region_id: "2959",
                        region_name: "米林县"
                    },
                    counties2960: {
                        region_id: "2960",
                        region_name: "墨脱县"
                    },
                    counties2961: {
                        region_id: "2961",
                        region_name: "波密县"
                    },
                    counties2962: {
                        region_id: "2962",
                        region_name: "察隅县"
                    },
                    counties2963: {
                        region_id: "2963",
                        region_name: "朗县"
                    }
                }
            },
            cities348: {
                region_id: "348",
                region_name: "那曲",
                county: {
                    counties2964: {
                        region_id: "2964",
                        region_name: "那曲县"
                    },
                    counties2965: {
                        region_id: "2965",
                        region_name: "嘉黎县"
                    },
                    counties2966: {
                        region_id: "2966",
                        region_name: "比如县"
                    },
                    counties2967: {
                        region_id: "2967",
                        region_name: "聂荣县"
                    },
                    counties2968: {
                        region_id: "2968",
                        region_name: "安多县"
                    },
                    counties2969: {
                        region_id: "2969",
                        region_name: "申扎县"
                    },
                    counties2970: {
                        region_id: "2970",
                        region_name: "索县"
                    },
                    counties2971: {
                        region_id: "2971",
                        region_name: "班戈县"
                    },
                    counties2972: {
                        region_id: "2972",
                        region_name: "巴青县"
                    },
                    counties2973: {
                        region_id: "2973",
                        region_name: "尼玛县"
                    }
                }
            },
            cities349: {
                region_id: "349",
                region_name: "日喀则",
                county: {
                    counties2974: {
                        region_id: "2974",
                        region_name: "日喀则市"
                    },
                    counties2975: {
                        region_id: "2975",
                        region_name: "南木林县"
                    },
                    counties2976: {
                        region_id: "2976",
                        region_name: "江孜县"
                    },
                    counties2977: {
                        region_id: "2977",
                        region_name: "定日县"
                    },
                    counties2978: {
                        region_id: "2978",
                        region_name: "萨迦县"
                    },
                    counties2979: {
                        region_id: "2979",
                        region_name: "拉孜县"
                    },
                    counties2980: {
                        region_id: "2980",
                        region_name: "昂仁县"
                    },
                    counties2981: {
                        region_id: "2981",
                        region_name: "谢通门县"
                    },
                    counties2982: {
                        region_id: "2982",
                        region_name: "白朗县"
                    },
                    counties2983: {
                        region_id: "2983",
                        region_name: "仁布县"
                    },
                    counties2984: {
                        region_id: "2984",
                        region_name: "康马县"
                    },
                    counties2985: {
                        region_id: "2985",
                        region_name: "定结县"
                    },
                    counties2986: {
                        region_id: "2986",
                        region_name: "仲巴县"
                    },
                    counties2987: {
                        region_id: "2987",
                        region_name: "亚东县"
                    },
                    counties2988: {
                        region_id: "2988",
                        region_name: "吉隆县"
                    },
                    counties2989: {
                        region_id: "2989",
                        region_name: "聂拉木县"
                    },
                    counties2990: {
                        region_id: "2990",
                        region_name: "萨嘎县"
                    },
                    counties2991: {
                        region_id: "2991",
                        region_name: "岗巴县"
                    }
                }
            },
            cities350: {
                region_id: "350",
                region_name: "山南",
                county: {
                    counties2992: {
                        region_id: "2992",
                        region_name: "乃东县"
                    },
                    counties2993: {
                        region_id: "2993",
                        region_name: "扎囊县"
                    },
                    counties2994: {
                        region_id: "2994",
                        region_name: "贡嘎县"
                    },
                    counties2995: {
                        region_id: "2995",
                        region_name: "桑日县"
                    },
                    counties2996: {
                        region_id: "2996",
                        region_name: "琼结县"
                    },
                    counties2997: {
                        region_id: "2997",
                        region_name: "曲松县"
                    },
                    counties2998: {
                        region_id: "2998",
                        region_name: "措美县"
                    },
                    counties2999: {
                        region_id: "2999",
                        region_name: "洛扎县"
                    },
                    counties3000: {
                        region_id: "3000",
                        region_name: "加查县"
                    },
                    counties3001: {
                        region_id: "3001",
                        region_name: "隆子县"
                    },
                    counties3002: {
                        region_id: "3002",
                        region_name: "错那县"
                    },
                    counties3003: {
                        region_id: "3003",
                        region_name: "浪卡子县"
                    }
                }
            }
        }
    },
    provinces29: {
        region_id: "29",
        region_name: "新疆",
        city: {
            cities351: {
                region_id: "351",
                region_name: "乌鲁木齐",
                county: {
                    counties3004: {
                        region_id: "3004",
                        region_name: "天山区"
                    },
                    counties3005: {
                        region_id: "3005",
                        region_name: "沙依巴克区"
                    },
                    counties3006: {
                        region_id: "3006",
                        region_name: "新市区"
                    },
                    counties3007: {
                        region_id: "3007",
                        region_name: "水磨沟区"
                    },
                    counties3008: {
                        region_id: "3008",
                        region_name: "头屯河区"
                    },
                    counties3009: {
                        region_id: "3009",
                        region_name: "达坂城区"
                    },
                    counties3010: {
                        region_id: "3010",
                        region_name: "米东区"
                    },
                    counties3011: {
                        region_id: "3011",
                        region_name: "乌鲁木齐县"
                    }
                }
            },
            cities352: {
                region_id: "352",
                region_name: "阿克苏",
                county: {
                    counties3012: {
                        region_id: "3012",
                        region_name: "阿克苏市"
                    },
                    counties3013: {
                        region_id: "3013",
                        region_name: "温宿县"
                    },
                    counties3014: {
                        region_id: "3014",
                        region_name: "库车县"
                    },
                    counties3015: {
                        region_id: "3015",
                        region_name: "沙雅县"
                    },
                    counties3016: {
                        region_id: "3016",
                        region_name: "新和县"
                    },
                    counties3017: {
                        region_id: "3017",
                        region_name: "拜城县"
                    },
                    counties3018: {
                        region_id: "3018",
                        region_name: "乌什县"
                    },
                    counties3019: {
                        region_id: "3019",
                        region_name: "阿瓦提县"
                    },
                    counties3020: {
                        region_id: "3020",
                        region_name: "柯坪县"
                    }
                }
            },
            cities353: {
                region_id: "353",
                region_name: "阿拉尔",
                county: {
                    counties3021: {
                        region_id: "3021",
                        region_name: "阿拉尔市"
                    }
                }
            },
            cities354: {
                region_id: "354",
                region_name: "巴音郭楞",
                county: {
                    counties3022: {
                        region_id: "3022",
                        region_name: "库尔勒市"
                    },
                    counties3023: {
                        region_id: "3023",
                        region_name: "轮台县"
                    },
                    counties3024: {
                        region_id: "3024",
                        region_name: "尉犁县"
                    },
                    counties3025: {
                        region_id: "3025",
                        region_name: "若羌县"
                    },
                    counties3026: {
                        region_id: "3026",
                        region_name: "且末县"
                    },
                    counties3027: {
                        region_id: "3027",
                        region_name: "焉耆"
                    },
                    counties3028: {
                        region_id: "3028",
                        region_name: "和静县"
                    },
                    counties3029: {
                        region_id: "3029",
                        region_name: "和硕县"
                    },
                    counties3030: {
                        region_id: "3030",
                        region_name: "博湖县"
                    }
                }
            },
            cities355: {
                region_id: "355",
                region_name: "博尔塔拉",
                county: {
                    counties3031: {
                        region_id: "3031",
                        region_name: "博乐市"
                    },
                    counties3032: {
                        region_id: "3032",
                        region_name: "精河县"
                    },
                    counties3033: {
                        region_id: "3033",
                        region_name: "温泉县"
                    }
                }
            },
            cities356: {
                region_id: "356",
                region_name: "昌吉",
                county: {
                    counties3034: {
                        region_id: "3034",
                        region_name: "呼图壁县"
                    },
                    counties3035: {
                        region_id: "3035",
                        region_name: "米泉市"
                    },
                    counties3036: {
                        region_id: "3036",
                        region_name: "昌吉市"
                    },
                    counties3037: {
                        region_id: "3037",
                        region_name: "阜康市"
                    },
                    counties3038: {
                        region_id: "3038",
                        region_name: "玛纳斯县"
                    },
                    counties3039: {
                        region_id: "3039",
                        region_name: "奇台县"
                    },
                    counties3040: {
                        region_id: "3040",
                        region_name: "吉木萨尔县"
                    },
                    counties3041: {
                        region_id: "3041",
                        region_name: "木垒"
                    }
                }
            },
            cities357: {
                region_id: "357",
                region_name: "哈密",
                county: {
                    counties3042: {
                        region_id: "3042",
                        region_name: "哈密市"
                    },
                    counties3043: {
                        region_id: "3043",
                        region_name: "伊吾县"
                    },
                    counties3044: {
                        region_id: "3044",
                        region_name: "巴里坤"
                    }
                }
            },
            cities358: {
                region_id: "358",
                region_name: "和田",
                county: {
                    counties3045: {
                        region_id: "3045",
                        region_name: "和田市"
                    },
                    counties3046: {
                        region_id: "3046",
                        region_name: "和田县"
                    },
                    counties3047: {
                        region_id: "3047",
                        region_name: "墨玉县"
                    },
                    counties3048: {
                        region_id: "3048",
                        region_name: "皮山县"
                    },
                    counties3049: {
                        region_id: "3049",
                        region_name: "洛浦县"
                    },
                    counties3050: {
                        region_id: "3050",
                        region_name: "策勒县"
                    },
                    counties3051: {
                        region_id: "3051",
                        region_name: "于田县"
                    },
                    counties3052: {
                        region_id: "3052",
                        region_name: "民丰县"
                    }
                }
            },
            cities359: {
                region_id: "359",
                region_name: "喀什",
                county: {
                    counties3053: {
                        region_id: "3053",
                        region_name: "喀什市"
                    },
                    counties3054: {
                        region_id: "3054",
                        region_name: "疏附县"
                    },
                    counties3055: {
                        region_id: "3055",
                        region_name: "疏勒县"
                    },
                    counties3056: {
                        region_id: "3056",
                        region_name: "英吉沙县"
                    },
                    counties3057: {
                        region_id: "3057",
                        region_name: "泽普县"
                    },
                    counties3058: {
                        region_id: "3058",
                        region_name: "莎车县"
                    },
                    counties3059: {
                        region_id: "3059",
                        region_name: "叶城县"
                    },
                    counties3060: {
                        region_id: "3060",
                        region_name: "麦盖提县"
                    },
                    counties3061: {
                        region_id: "3061",
                        region_name: "岳普湖县"
                    },
                    counties3062: {
                        region_id: "3062",
                        region_name: "伽师县"
                    },
                    counties3063: {
                        region_id: "3063",
                        region_name: "巴楚县"
                    },
                    counties3064: {
                        region_id: "3064",
                        region_name: "塔什库尔干"
                    }
                }
            },
            cities360: {
                region_id: "360",
                region_name: "克拉玛依",
                county: {
                    counties3065: {
                        region_id: "3065",
                        region_name: "克拉玛依市"
                    }
                }
            },
            cities361: {
                region_id: "361",
                region_name: "克孜勒苏柯尔克孜",
                county: {
                    counties3066: {
                        region_id: "3066",
                        region_name: "阿图什市"
                    },
                    counties3067: {
                        region_id: "3067",
                        region_name: "阿克陶县"
                    },
                    counties3068: {
                        region_id: "3068",
                        region_name: "阿合奇县"
                    },
                    counties3069: {
                        region_id: "3069",
                        region_name: "乌恰县"
                    }
                }
            },
            cities362: {
                region_id: "362",
                region_name: "石河子",
                county: {
                    counties3070: {
                        region_id: "3070",
                        region_name: "石河子市"
                    }
                }
            },
            cities363: {
                region_id: "363",
                region_name: "图木舒克",
                county: {
                    counties3071: {
                        region_id: "3071",
                        region_name: "图木舒克市"
                    }
                }
            },
            cities364: {
                region_id: "364",
                region_name: "吐鲁番",
                county: {
                    counties3072: {
                        region_id: "3072",
                        region_name: "吐鲁番市"
                    },
                    counties3073: {
                        region_id: "3073",
                        region_name: "鄯善县"
                    },
                    counties3074: {
                        region_id: "3074",
                        region_name: "托克逊县"
                    }
                }
            },
            cities365: {
                region_id: "365",
                region_name: "五家渠",
                county: {
                    counties3075: {
                        region_id: "3075",
                        region_name: "五家渠市"
                    }
                }
            },
            cities366: {
                region_id: "366",
                region_name: "伊犁",
                county: {
                    counties3077: {
                        region_id: "3077",
                        region_name: "布克赛尔"
                    },
                    counties3078: {
                        region_id: "3078",
                        region_name: "伊宁市"
                    },
                    counties3079: {
                        region_id: "3079",
                        region_name: "布尔津县"
                    },
                    counties3080: {
                        region_id: "3080",
                        region_name: "奎屯市"
                    },
                    counties3081: {
                        region_id: "3081",
                        region_name: "乌苏市"
                    },
                    counties3082: {
                        region_id: "3082",
                        region_name: "额敏县"
                    },
                    counties3083: {
                        region_id: "3083",
                        region_name: "富蕴县"
                    },
                    counties3084: {
                        region_id: "3084",
                        region_name: "伊宁县"
                    },
                    counties3085: {
                        region_id: "3085",
                        region_name: "福海县"
                    },
                    counties3086: {
                        region_id: "3086",
                        region_name: "霍城县"
                    },
                    counties3087: {
                        region_id: "3087",
                        region_name: "沙湾县"
                    },
                    counties3088: {
                        region_id: "3088",
                        region_name: "巩留县"
                    },
                    counties3089: {
                        region_id: "3089",
                        region_name: "哈巴河县"
                    },
                    counties3090: {
                        region_id: "3090",
                        region_name: "托里县"
                    },
                    counties3091: {
                        region_id: "3091",
                        region_name: "青河县"
                    },
                    counties3092: {
                        region_id: "3092",
                        region_name: "新源县"
                    },
                    counties3093: {
                        region_id: "3093",
                        region_name: "裕民县"
                    },
                    counties3094: {
                        region_id: "3094",
                        region_name: "和布克赛尔"
                    },
                    counties3095: {
                        region_id: "3095",
                        region_name: "吉木乃县"
                    },
                    counties3096: {
                        region_id: "3096",
                        region_name: "昭苏县"
                    },
                    counties3097: {
                        region_id: "3097",
                        region_name: "特克斯县"
                    },
                    counties3098: {
                        region_id: "3098",
                        region_name: "尼勒克县"
                    },
                    counties3099: {
                        region_id: "3099",
                        region_name: "察布查尔"
                    }
                }
            },
            cities3076: {
                region_id: "3076",
                region_name: "阿勒泰地区",
                county: {
                    counties3434: {
                        region_id: "3434",
                        region_name: "阿勒泰地区"
                    }
                }
            },
            cities3435: {
                region_id: "3435",
                region_name: "塔城",
                county: {
                    counties3436: {
                        region_id: "3436",
                        region_name: "塔城"
                    }
                }
            }
        }
    },
    provinces30: {
        region_id: "30",
        region_name: "云南",
        city: {
            cities367: {
                region_id: "367",
                region_name: "昆明",
                county: {
                    counties3100: {
                        region_id: "3100",
                        region_name: "盘龙区"
                    },
                    counties3101: {
                        region_id: "3101",
                        region_name: "五华区"
                    },
                    counties3102: {
                        region_id: "3102",
                        region_name: "官渡区"
                    },
                    counties3103: {
                        region_id: "3103",
                        region_name: "西山区"
                    },
                    counties3104: {
                        region_id: "3104",
                        region_name: "东川区"
                    },
                    counties3105: {
                        region_id: "3105",
                        region_name: "安宁市"
                    },
                    counties3106: {
                        region_id: "3106",
                        region_name: "呈贡县"
                    },
                    counties3107: {
                        region_id: "3107",
                        region_name: "晋宁县"
                    },
                    counties3108: {
                        region_id: "3108",
                        region_name: "富民县"
                    },
                    counties3109: {
                        region_id: "3109",
                        region_name: "宜良县"
                    },
                    counties3110: {
                        region_id: "3110",
                        region_name: "嵩明县"
                    },
                    counties3111: {
                        region_id: "3111",
                        region_name: "石林县"
                    },
                    counties3112: {
                        region_id: "3112",
                        region_name: "禄劝"
                    },
                    counties3113: {
                        region_id: "3113",
                        region_name: "寻甸"
                    }
                }
            },
            cities368: {
                region_id: "368",
                region_name: "怒江",
                county: {
                    counties3114: {
                        region_id: "3114",
                        region_name: "兰坪"
                    },
                    counties3115: {
                        region_id: "3115",
                        region_name: "泸水县"
                    },
                    counties3116: {
                        region_id: "3116",
                        region_name: "福贡县"
                    },
                    counties3117: {
                        region_id: "3117",
                        region_name: "贡山"
                    }
                }
            },
            cities369: {
                region_id: "369",
                region_name: "普洱",
                county: {
                    counties3118: {
                        region_id: "3118",
                        region_name: "宁洱"
                    },
                    counties3119: {
                        region_id: "3119",
                        region_name: "思茅区"
                    },
                    counties3120: {
                        region_id: "3120",
                        region_name: "墨江"
                    },
                    counties3121: {
                        region_id: "3121",
                        region_name: "景东"
                    },
                    counties3122: {
                        region_id: "3122",
                        region_name: "景谷"
                    },
                    counties3123: {
                        region_id: "3123",
                        region_name: "镇沅"
                    },
                    counties3124: {
                        region_id: "3124",
                        region_name: "江城"
                    },
                    counties3125: {
                        region_id: "3125",
                        region_name: "孟连"
                    },
                    counties3126: {
                        region_id: "3126",
                        region_name: "澜沧"
                    },
                    counties3127: {
                        region_id: "3127",
                        region_name: "西盟"
                    }
                }
            },
            cities370: {
                region_id: "370",
                region_name: "丽江",
                county: {
                    counties3128: {
                        region_id: "3128",
                        region_name: "古城区"
                    },
                    counties3129: {
                        region_id: "3129",
                        region_name: "宁蒗"
                    },
                    counties3130: {
                        region_id: "3130",
                        region_name: "玉龙"
                    },
                    counties3131: {
                        region_id: "3131",
                        region_name: "永胜县"
                    },
                    counties3132: {
                        region_id: "3132",
                        region_name: "华坪县"
                    }
                }
            },
            cities371: {
                region_id: "371",
                region_name: "保山",
                county: {
                    counties3133: {
                        region_id: "3133",
                        region_name: "隆阳区"
                    },
                    counties3134: {
                        region_id: "3134",
                        region_name: "施甸县"
                    },
                    counties3135: {
                        region_id: "3135",
                        region_name: "腾冲县"
                    },
                    counties3136: {
                        region_id: "3136",
                        region_name: "龙陵县"
                    },
                    counties3137: {
                        region_id: "3137",
                        region_name: "昌宁县"
                    }
                }
            },
            cities372: {
                region_id: "372",
                region_name: "楚雄",
                county: {
                    counties3138: {
                        region_id: "3138",
                        region_name: "楚雄市"
                    },
                    counties3139: {
                        region_id: "3139",
                        region_name: "双柏县"
                    },
                    counties3140: {
                        region_id: "3140",
                        region_name: "牟定县"
                    },
                    counties3141: {
                        region_id: "3141",
                        region_name: "南华县"
                    },
                    counties3142: {
                        region_id: "3142",
                        region_name: "姚安县"
                    },
                    counties3143: {
                        region_id: "3143",
                        region_name: "大姚县"
                    },
                    counties3144: {
                        region_id: "3144",
                        region_name: "永仁县"
                    },
                    counties3145: {
                        region_id: "3145",
                        region_name: "元谋县"
                    },
                    counties3146: {
                        region_id: "3146",
                        region_name: "武定县"
                    },
                    counties3147: {
                        region_id: "3147",
                        region_name: "禄丰县"
                    }
                }
            },
            cities373: {
                region_id: "373",
                region_name: "大理",
                county: {
                    counties3148: {
                        region_id: "3148",
                        region_name: "大理市"
                    },
                    counties3149: {
                        region_id: "3149",
                        region_name: "祥云县"
                    },
                    counties3150: {
                        region_id: "3150",
                        region_name: "宾川县"
                    },
                    counties3151: {
                        region_id: "3151",
                        region_name: "弥渡县"
                    },
                    counties3152: {
                        region_id: "3152",
                        region_name: "永平县"
                    },
                    counties3153: {
                        region_id: "3153",
                        region_name: "云龙县"
                    },
                    counties3154: {
                        region_id: "3154",
                        region_name: "洱源县"
                    },
                    counties3155: {
                        region_id: "3155",
                        region_name: "剑川县"
                    },
                    counties3156: {
                        region_id: "3156",
                        region_name: "鹤庆县"
                    },
                    counties3157: {
                        region_id: "3157",
                        region_name: "漾濞"
                    },
                    counties3158: {
                        region_id: "3158",
                        region_name: "南涧"
                    },
                    counties3159: {
                        region_id: "3159",
                        region_name: "巍山"
                    }
                }
            },
            cities374: {
                region_id: "374",
                region_name: "德宏",
                county: {
                    counties3160: {
                        region_id: "3160",
                        region_name: "潞西市"
                    },
                    counties3161: {
                        region_id: "3161",
                        region_name: "瑞丽市"
                    },
                    counties3162: {
                        region_id: "3162",
                        region_name: "梁河县"
                    },
                    counties3163: {
                        region_id: "3163",
                        region_name: "盈江县"
                    },
                    counties3164: {
                        region_id: "3164",
                        region_name: "陇川县"
                    }
                }
            },
            cities375: {
                region_id: "375",
                region_name: "迪庆",
                county: {
                    counties3165: {
                        region_id: "3165",
                        region_name: "香格里拉县"
                    },
                    counties3166: {
                        region_id: "3166",
                        region_name: "德钦县"
                    },
                    counties3167: {
                        region_id: "3167",
                        region_name: "维西"
                    }
                }
            },
            cities376: {
                region_id: "376",
                region_name: "红河自治州",
                county: {
                    counties3168: {
                        region_id: "3168",
                        region_name: "泸西县"
                    },
                    counties3169: {
                        region_id: "3169",
                        region_name: "蒙自县"
                    },
                    counties3170: {
                        region_id: "3170",
                        region_name: "个旧市"
                    },
                    counties3171: {
                        region_id: "3171",
                        region_name: "开远市"
                    },
                    counties3172: {
                        region_id: "3172",
                        region_name: "绿春县"
                    },
                    counties3173: {
                        region_id: "3173",
                        region_name: "建水县"
                    },
                    counties3174: {
                        region_id: "3174",
                        region_name: "石屏县"
                    },
                    counties3175: {
                        region_id: "3175",
                        region_name: "弥勒县"
                    },
                    counties3176: {
                        region_id: "3176",
                        region_name: "元阳县"
                    },
                    counties3177: {
                        region_id: "3177",
                        region_name: "红河县"
                    },
                    counties3178: {
                        region_id: "3178",
                        region_name: "金平"
                    },
                    counties3179: {
                        region_id: "3179",
                        region_name: "河口"
                    },
                    counties3180: {
                        region_id: "3180",
                        region_name: "屏边"
                    }
                }
            },
            cities377: {
                region_id: "377",
                region_name: "临沧",
                county: {
                    counties3181: {
                        region_id: "3181",
                        region_name: "临翔区"
                    },
                    counties3182: {
                        region_id: "3182",
                        region_name: "凤庆县"
                    },
                    counties3183: {
                        region_id: "3183",
                        region_name: "云县"
                    },
                    counties3184: {
                        region_id: "3184",
                        region_name: "永德县"
                    },
                    counties3185: {
                        region_id: "3185",
                        region_name: "镇康县"
                    },
                    counties3186: {
                        region_id: "3186",
                        region_name: "双江"
                    },
                    counties3187: {
                        region_id: "3187",
                        region_name: "耿马"
                    },
                    counties3188: {
                        region_id: "3188",
                        region_name: "沧源"
                    }
                }
            },
            cities378: {
                region_id: "378",
                region_name: "曲靖",
                county: {
                    counties3189: {
                        region_id: "3189",
                        region_name: "麒麟区"
                    },
                    counties3190: {
                        region_id: "3190",
                        region_name: "宣威市"
                    },
                    counties3191: {
                        region_id: "3191",
                        region_name: "马龙县"
                    },
                    counties3192: {
                        region_id: "3192",
                        region_name: "陆良县"
                    },
                    counties3193: {
                        region_id: "3193",
                        region_name: "师宗县"
                    },
                    counties3194: {
                        region_id: "3194",
                        region_name: "罗平县"
                    },
                    counties3195: {
                        region_id: "3195",
                        region_name: "富源县"
                    },
                    counties3196: {
                        region_id: "3196",
                        region_name: "会泽县"
                    },
                    counties3197: {
                        region_id: "3197",
                        region_name: "沾益县"
                    }
                }
            },
            cities379: {
                region_id: "379",
                region_name: "文山",
                county: {
                    counties3198: {
                        region_id: "3198",
                        region_name: "文山县"
                    },
                    counties3199: {
                        region_id: "3199",
                        region_name: "砚山县"
                    },
                    counties3200: {
                        region_id: "3200",
                        region_name: "西畴县"
                    },
                    counties3201: {
                        region_id: "3201",
                        region_name: "麻栗坡县"
                    },
                    counties3202: {
                        region_id: "3202",
                        region_name: "马关县"
                    },
                    counties3203: {
                        region_id: "3203",
                        region_name: "丘北县"
                    },
                    counties3204: {
                        region_id: "3204",
                        region_name: "广南县"
                    },
                    counties3205: {
                        region_id: "3205",
                        region_name: "富宁县"
                    }
                }
            },
            cities380: {
                region_id: "380",
                region_name: "西双版纳",
                county: {
                    counties3206: {
                        region_id: "3206",
                        region_name: "景洪市"
                    },
                    counties3207: {
                        region_id: "3207",
                        region_name: "勐海县"
                    },
                    counties3208: {
                        region_id: "3208",
                        region_name: "勐腊县"
                    }
                }
            },
            cities381: {
                region_id: "381",
                region_name: "玉溪",
                county: {
                    counties3209: {
                        region_id: "3209",
                        region_name: "红塔区"
                    },
                    counties3210: {
                        region_id: "3210",
                        region_name: "江川县"
                    },
                    counties3211: {
                        region_id: "3211",
                        region_name: "澄江县"
                    },
                    counties3212: {
                        region_id: "3212",
                        region_name: "通海县"
                    },
                    counties3213: {
                        region_id: "3213",
                        region_name: "华宁县"
                    },
                    counties3214: {
                        region_id: "3214",
                        region_name: "易门县"
                    },
                    counties3215: {
                        region_id: "3215",
                        region_name: "峨山"
                    },
                    counties3216: {
                        region_id: "3216",
                        region_name: "新平"
                    },
                    counties3217: {
                        region_id: "3217",
                        region_name: "元江"
                    }
                }
            },
            cities382: {
                region_id: "382",
                region_name: "昭通",
                county: {
                    counties3218: {
                        region_id: "3218",
                        region_name: "昭阳区"
                    },
                    counties3219: {
                        region_id: "3219",
                        region_name: "鲁甸县"
                    },
                    counties3220: {
                        region_id: "3220",
                        region_name: "巧家县"
                    },
                    counties3221: {
                        region_id: "3221",
                        region_name: "盐津县"
                    },
                    counties3222: {
                        region_id: "3222",
                        region_name: "大关县"
                    },
                    counties3223: {
                        region_id: "3223",
                        region_name: "永善县"
                    },
                    counties3224: {
                        region_id: "3224",
                        region_name: "绥江县"
                    },
                    counties3225: {
                        region_id: "3225",
                        region_name: "镇雄县"
                    },
                    counties3226: {
                        region_id: "3226",
                        region_name: "彝良县"
                    },
                    counties3227: {
                        region_id: "3227",
                        region_name: "威信县"
                    },
                    counties3228: {
                        region_id: "3228",
                        region_name: "水富县"
                    }
                }
            }
        }
    },
    provinces31: {
        region_id: "31",
        region_name: "浙江",
        city: {
            cities383: {
                region_id: "383",
                region_name: "杭州",
                county: {
                    counties3229: {
                        region_id: "3229",
                        region_name: "西湖区"
                    },
                    counties3230: {
                        region_id: "3230",
                        region_name: "上城区"
                    },
                    counties3231: {
                        region_id: "3231",
                        region_name: "下城区"
                    },
                    counties3232: {
                        region_id: "3232",
                        region_name: "拱墅区"
                    },
                    counties3233: {
                        region_id: "3233",
                        region_name: "滨江区"
                    },
                    counties3234: {
                        region_id: "3234",
                        region_name: "江干区"
                    },
                    counties3235: {
                        region_id: "3235",
                        region_name: "萧山区"
                    },
                    counties3236: {
                        region_id: "3236",
                        region_name: "余杭区"
                    },
                    counties3237: {
                        region_id: "3237",
                        region_name: "市郊"
                    },
                    counties3238: {
                        region_id: "3238",
                        region_name: "建德市"
                    },
                    counties3239: {
                        region_id: "3239",
                        region_name: "富阳市"
                    },
                    counties3240: {
                        region_id: "3240",
                        region_name: "临安市"
                    },
                    counties3241: {
                        region_id: "3241",
                        region_name: "桐庐县"
                    },
                    counties3242: {
                        region_id: "3242",
                        region_name: "淳安县"
                    }
                }
            },
            cities384: {
                region_id: "384",
                region_name: "湖州",
                county: {
                    counties3243: {
                        region_id: "3243",
                        region_name: "吴兴区"
                    },
                    counties3244: {
                        region_id: "3244",
                        region_name: "南浔区"
                    },
                    counties3245: {
                        region_id: "3245",
                        region_name: "德清县"
                    },
                    counties3246: {
                        region_id: "3246",
                        region_name: "长兴县"
                    },
                    counties3247: {
                        region_id: "3247",
                        region_name: "安吉县"
                    }
                }
            },
            cities385: {
                region_id: "385",
                region_name: "嘉兴",
                county: {
                    counties3248: {
                        region_id: "3248",
                        region_name: "南湖区"
                    },
                    counties3249: {
                        region_id: "3249",
                        region_name: "秀洲区"
                    },
                    counties3250: {
                        region_id: "3250",
                        region_name: "海宁市"
                    },
                    counties3251: {
                        region_id: "3251",
                        region_name: "嘉善县"
                    },
                    counties3252: {
                        region_id: "3252",
                        region_name: "平湖市"
                    },
                    counties3253: {
                        region_id: "3253",
                        region_name: "桐乡市"
                    },
                    counties3254: {
                        region_id: "3254",
                        region_name: "海盐县"
                    }
                }
            },
            cities386: {
                region_id: "386",
                region_name: "金华",
                county: {
                    counties3255: {
                        region_id: "3255",
                        region_name: "婺城区"
                    },
                    counties3256: {
                        region_id: "3256",
                        region_name: "金东区"
                    },
                    counties3257: {
                        region_id: "3257",
                        region_name: "兰溪市"
                    },
                    counties3258: {
                        region_id: "3258",
                        region_name: "市区"
                    },
                    counties3259: {
                        region_id: "3259",
                        region_name: "佛堂镇"
                    },
                    counties3260: {
                        region_id: "3260",
                        region_name: "上溪镇"
                    },
                    counties3261: {
                        region_id: "3261",
                        region_name: "义亭镇"
                    },
                    counties3262: {
                        region_id: "3262",
                        region_name: "大陈镇"
                    },
                    counties3263: {
                        region_id: "3263",
                        region_name: "苏溪镇"
                    },
                    counties3264: {
                        region_id: "3264",
                        region_name: "赤岸镇"
                    },
                    counties3265: {
                        region_id: "3265",
                        region_name: "东阳市"
                    },
                    counties3266: {
                        region_id: "3266",
                        region_name: "永康市"
                    },
                    counties3267: {
                        region_id: "3267",
                        region_name: "武义县"
                    },
                    counties3268: {
                        region_id: "3268",
                        region_name: "浦江县"
                    },
                    counties3269: {
                        region_id: "3269",
                        region_name: "磐安县"
                    }
                }
            },
            cities387: {
                region_id: "387",
                region_name: "丽水",
                county: {
                    counties3270: {
                        region_id: "3270",
                        region_name: "莲都区"
                    },
                    counties3271: {
                        region_id: "3271",
                        region_name: "龙泉市"
                    },
                    counties3272: {
                        region_id: "3272",
                        region_name: "青田县"
                    },
                    counties3273: {
                        region_id: "3273",
                        region_name: "缙云县"
                    },
                    counties3274: {
                        region_id: "3274",
                        region_name: "遂昌县"
                    },
                    counties3275: {
                        region_id: "3275",
                        region_name: "松阳县"
                    },
                    counties3276: {
                        region_id: "3276",
                        region_name: "云和县"
                    },
                    counties3277: {
                        region_id: "3277",
                        region_name: "庆元县"
                    },
                    counties3278: {
                        region_id: "3278",
                        region_name: "景宁"
                    }
                }
            },
            cities388: {
                region_id: "388",
                region_name: "宁波",
                county: {
                    counties3279: {
                        region_id: "3279",
                        region_name: "海曙区"
                    },
                    counties3280: {
                        region_id: "3280",
                        region_name: "江东区"
                    },
                    counties3281: {
                        region_id: "3281",
                        region_name: "江北区"
                    },
                    counties3282: {
                        region_id: "3282",
                        region_name: "镇海区"
                    },
                    counties3283: {
                        region_id: "3283",
                        region_name: "北仑区"
                    },
                    counties3284: {
                        region_id: "3284",
                        region_name: "鄞州区"
                    },
                    counties3285: {
                        region_id: "3285",
                        region_name: "余姚市"
                    },
                    counties3286: {
                        region_id: "3286",
                        region_name: "慈溪市"
                    },
                    counties3287: {
                        region_id: "3287",
                        region_name: "奉化市"
                    },
                    counties3288: {
                        region_id: "3288",
                        region_name: "象山县"
                    },
                    counties3289: {
                        region_id: "3289",
                        region_name: "宁海县"
                    }
                }
            },
            cities389: {
                region_id: "389",
                region_name: "绍兴",
                county: {
                    counties3290: {
                        region_id: "3290",
                        region_name: "越城区"
                    },
                    counties3291: {
                        region_id: "3291",
                        region_name: "上虞市"
                    },
                    counties3292: {
                        region_id: "3292",
                        region_name: "嵊州市"
                    },
                    counties3293: {
                        region_id: "3293",
                        region_name: "绍兴县"
                    },
                    counties3294: {
                        region_id: "3294",
                        region_name: "新昌县"
                    },
                    counties3295: {
                        region_id: "3295",
                        region_name: "诸暨市"
                    }
                }
            },
            cities390: {
                region_id: "390",
                region_name: "台州",
                county: {
                    counties3296: {
                        region_id: "3296",
                        region_name: "椒江区"
                    },
                    counties3297: {
                        region_id: "3297",
                        region_name: "黄岩区"
                    },
                    counties3298: {
                        region_id: "3298",
                        region_name: "路桥区"
                    },
                    counties3299: {
                        region_id: "3299",
                        region_name: "温岭市"
                    },
                    counties3300: {
                        region_id: "3300",
                        region_name: "临海市"
                    },
                    counties3301: {
                        region_id: "3301",
                        region_name: "玉环县"
                    },
                    counties3302: {
                        region_id: "3302",
                        region_name: "三门县"
                    },
                    counties3303: {
                        region_id: "3303",
                        region_name: "天台县"
                    },
                    counties3304: {
                        region_id: "3304",
                        region_name: "仙居县"
                    }
                }
            },
            cities391: {
                region_id: "391",
                region_name: "温州",
                county: {
                    counties3305: {
                        region_id: "3305",
                        region_name: "鹿城区"
                    },
                    counties3306: {
                        region_id: "3306",
                        region_name: "龙湾区"
                    },
                    counties3307: {
                        region_id: "3307",
                        region_name: "瓯海区"
                    },
                    counties3308: {
                        region_id: "3308",
                        region_name: "瑞安市"
                    },
                    counties3309: {
                        region_id: "3309",
                        region_name: "乐清市"
                    },
                    counties3310: {
                        region_id: "3310",
                        region_name: "洞头县"
                    },
                    counties3311: {
                        region_id: "3311",
                        region_name: "永嘉县"
                    },
                    counties3312: {
                        region_id: "3312",
                        region_name: "平阳县"
                    },
                    counties3313: {
                        region_id: "3313",
                        region_name: "苍南县"
                    },
                    counties3314: {
                        region_id: "3314",
                        region_name: "文成县"
                    },
                    counties3315: {
                        region_id: "3315",
                        region_name: "泰顺县"
                    }
                }
            },
            cities392: {
                region_id: "392",
                region_name: "舟山",
                county: {
                    counties3316: {
                        region_id: "3316",
                        region_name: "定海区"
                    },
                    counties3317: {
                        region_id: "3317",
                        region_name: "普陀区"
                    },
                    counties3318: {
                        region_id: "3318",
                        region_name: "岱山县"
                    },
                    counties3319: {
                        region_id: "3319",
                        region_name: "嵊泗县"
                    }
                }
            },
            cities393: {
                region_id: "393",
                region_name: "衢州",
                county: {
                    counties3320: {
                        region_id: "3320",
                        region_name: "衢州市"
                    },
                    counties3321: {
                        region_id: "3321",
                        region_name: "江山市"
                    },
                    counties3322: {
                        region_id: "3322",
                        region_name: "常山县"
                    },
                    counties3323: {
                        region_id: "3323",
                        region_name: "开化县"
                    },
                    counties3324: {
                        region_id: "3324",
                        region_name: "龙游县"
                    }
                }
            }
        }
    },
    provinces32: {
        region_id: "32",
        region_name: "重庆",
        city: {
            cities394: {
                region_id: "394",
                region_name: "重庆",
                county: {
                    counties3325: {
                        region_id: "3325",
                        region_name: "合川区"
                    },
                    counties3326: {
                        region_id: "3326",
                        region_name: "江津区"
                    },
                    counties3327: {
                        region_id: "3327",
                        region_name: "南川区"
                    },
                    counties3328: {
                        region_id: "3328",
                        region_name: "永川区"
                    },
                    counties3329: {
                        region_id: "3329",
                        region_name: "南岸区"
                    },
                    counties3330: {
                        region_id: "3330",
                        region_name: "渝北区"
                    },
                    counties3331: {
                        region_id: "3331",
                        region_name: "万盛区"
                    },
                    counties3332: {
                        region_id: "3332",
                        region_name: "大渡口区"
                    },
                    counties3333: {
                        region_id: "3333",
                        region_name: "万州区"
                    },
                    counties3334: {
                        region_id: "3334",
                        region_name: "北碚区"
                    },
                    counties3335: {
                        region_id: "3335",
                        region_name: "沙坪坝区"
                    },
                    counties3336: {
                        region_id: "3336",
                        region_name: "巴南区"
                    },
                    counties3337: {
                        region_id: "3337",
                        region_name: "涪陵区"
                    },
                    counties3338: {
                        region_id: "3338",
                        region_name: "江北区"
                    },
                    counties3339: {
                        region_id: "3339",
                        region_name: "九龙坡区"
                    },
                    counties3340: {
                        region_id: "3340",
                        region_name: "渝中区"
                    },
                    counties3341: {
                        region_id: "3341",
                        region_name: "黔江开发区"
                    },
                    counties3342: {
                        region_id: "3342",
                        region_name: "长寿区"
                    },
                    counties3343: {
                        region_id: "3343",
                        region_name: "双桥区"
                    },
                    counties3344: {
                        region_id: "3344",
                        region_name: "綦江县"
                    },
                    counties3345: {
                        region_id: "3345",
                        region_name: "潼南县"
                    },
                    counties3346: {
                        region_id: "3346",
                        region_name: "铜梁县"
                    },
                    counties3347: {
                        region_id: "3347",
                        region_name: "大足县"
                    },
                    counties3348: {
                        region_id: "3348",
                        region_name: "荣昌县"
                    },
                    counties3349: {
                        region_id: "3349",
                        region_name: "璧山县"
                    },
                    counties3350: {
                        region_id: "3350",
                        region_name: "垫江县"
                    },
                    counties3351: {
                        region_id: "3351",
                        region_name: "武隆县"
                    },
                    counties3352: {
                        region_id: "3352",
                        region_name: "丰都县"
                    },
                    counties3353: {
                        region_id: "3353",
                        region_name: "城口县"
                    },
                    counties3354: {
                        region_id: "3354",
                        region_name: "梁平县"
                    },
                    counties3355: {
                        region_id: "3355",
                        region_name: "开县"
                    },
                    counties3356: {
                        region_id: "3356",
                        region_name: "巫溪县"
                    },
                    counties3357: {
                        region_id: "3357",
                        region_name: "巫山县"
                    },
                    counties3358: {
                        region_id: "3358",
                        region_name: "奉节县"
                    },
                    counties3359: {
                        region_id: "3359",
                        region_name: "云阳县"
                    },
                    counties3360: {
                        region_id: "3360",
                        region_name: "忠县"
                    },
                    counties3361: {
                        region_id: "3361",
                        region_name: "石柱"
                    },
                    counties3362: {
                        region_id: "3362",
                        region_name: "彭水"
                    },
                    counties3363: {
                        region_id: "3363",
                        region_name: "酉阳"
                    },
                    counties3364: {
                        region_id: "3364",
                        region_name: "秀山"
                    }
                }
            }
        }
    },
    provinces33: {
        region_id: "33",
        region_name: "香港",
        city: {
            cities395: {
                region_id: "395",
                region_name: "香港",
                county: {
                    counties3365: {
                        region_id: "3365",
                        region_name: "沙田区"
                    },
                    counties3366: {
                        region_id: "3366",
                        region_name: "东区"
                    },
                    counties3367: {
                        region_id: "3367",
                        region_name: "观塘区"
                    },
                    counties3368: {
                        region_id: "3368",
                        region_name: "黄大仙区"
                    },
                    counties3369: {
                        region_id: "3369",
                        region_name: "九龙城区"
                    },
                    counties3370: {
                        region_id: "3370",
                        region_name: "屯门区"
                    },
                    counties3371: {
                        region_id: "3371",
                        region_name: "葵青区"
                    },
                    counties3372: {
                        region_id: "3372",
                        region_name: "元朗区"
                    },
                    counties3373: {
                        region_id: "3373",
                        region_name: "深水埗区"
                    },
                    counties3374: {
                        region_id: "3374",
                        region_name: "西贡区"
                    },
                    counties3375: {
                        region_id: "3375",
                        region_name: "大埔区"
                    },
                    counties3376: {
                        region_id: "3376",
                        region_name: "湾仔区"
                    },
                    counties3377: {
                        region_id: "3377",
                        region_name: "油尖旺区"
                    },
                    counties3378: {
                        region_id: "3378",
                        region_name: "北区"
                    },
                    counties3379: {
                        region_id: "3379",
                        region_name: "南区"
                    },
                    counties3380: {
                        region_id: "3380",
                        region_name: "荃湾区"
                    },
                    counties3381: {
                        region_id: "3381",
                        region_name: "中西区"
                    },
                    counties3382: {
                        region_id: "3382",
                        region_name: "离岛区"
                    }
                }
            }
        }
    },
    provinces34: {
        region_id: "34",
        region_name: "澳门",
        city: {
            cities396: {
                region_id: "396",
                region_name: "澳门",
                county: {
                    counties3383: {
                        region_id: "3383",
                        region_name: "澳门"
                    }
                }
            }
        }
    },
    provinces35: {
        region_id: "35",
        region_name: "台湾",
        city: {
            cities397: {
                region_id: "397",
                region_name: "台湾",
                county: {
                    counties3384: {
                        region_id: "3384",
                        region_name: "台北"
                    },
                    counties3385: {
                        region_id: "3385",
                        region_name: "高雄"
                    },
                    counties3386: {
                        region_id: "3386",
                        region_name: "基隆"
                    },
                    counties3387: {
                        region_id: "3387",
                        region_name: "台中"
                    },
                    counties3388: {
                        region_id: "3388",
                        region_name: "台南"
                    },
                    counties3389: {
                        region_id: "3389",
                        region_name: "新竹"
                    },
                    counties3390: {
                        region_id: "3390",
                        region_name: "嘉义"
                    },
                    counties3391: {
                        region_id: "3391",
                        region_name: "宜兰县"
                    },
                    counties3392: {
                        region_id: "3392",
                        region_name: "桃园县"
                    },
                    counties3393: {
                        region_id: "3393",
                        region_name: "苗栗县"
                    },
                    counties3394: {
                        region_id: "3394",
                        region_name: "彰化县"
                    },
                    counties3395: {
                        region_id: "3395",
                        region_name: "南投县"
                    },
                    counties3396: {
                        region_id: "3396",
                        region_name: "云林县"
                    },
                    counties3397: {
                        region_id: "3397",
                        region_name: "屏东县"
                    },
                    counties3398: {
                        region_id: "3398",
                        region_name: "台东县"
                    },
                    counties3399: {
                        region_id: "3399",
                        region_name: "花莲县"
                    },
                    counties3400: {
                        region_id: "3400",
                        region_name: "澎湖县"
                    }
                }
            }
        }
    }
};
    function province(){
        var arr=[];
        angular.forEach(region,function(e,i){
            var oitem = {id: e.region_id,name: e.region_name,key : i};
            arr.push(oitem);
        });
        return arr;
    }
    function city(key){
        var arr = [],
            cities = region[key].city;
        angular.forEach(cities,function(e,i){
           arr.push({name : e.region_name,id: e.region_id,keyProvince:key,key:i});
        });
        return arr;
    }
    function county(pkey,key){
        var arr = [],
            counties = region[pkey].city[key].county;
        angular.forEach(counties,function(e,i){
            arr.push({name : e.region_name,id: e.region_id,keyProvince:pkey,keyCity:key,key:i});
        });
        return arr;
    }
    return {
        restrict : 'AECM',
        template : '<div><select ng-options="p.key as p.name for p in province " ng-model="Province" ng-change="changeProvince()"><option value="">请选择省份</option></select>&nbsp;&nbsp;'
                    +'<select ng-options="c.key as c.name for c in city " ng-model="City" ng-change="changeCity()"><option value="">请选择城市</option></select>&nbsp;&nbsp;'
                    +'<select ng-options="n.key as n.name for n in county " ng-model="County" ng-change="changeCounty()"><option value="">请选择区/县</option></select></div>',
        replace : true,
        scope : {
            areaName : '=region'
        },
        controller : function($scope, $element, $attrs){
            $scope.province = province();
            $scope.city = [{name:'',key:''}],
            $scope.county = [{name:'',key:''}],
            $scope.area = {
                "p" : "",
                "c" :"",
                "n" : "",
                "pn" : "",
                "cn" : "",
                "nn" : ""
            };
            $scope.changeProvince = function(){
                var p = $scope.Province;
                $scope.area = {
                    "p" : "",
                    "c" :"",
                    "n" : "",
                    "pn" : "",
                    "cn" : "",
                    "nn" : ""
                };
                $scope.city = city(p),
                $scope.county = [{id:'',name:'',key:''}];
            }
            $scope.changeCity = function(){
                var p = $scope.Province,
                    c = $scope.City;
                $scope.county = county(p,c);
            }
            $scope.changeCounty = function(){
                $scope.area.county = $scope.County;
                var p = $scope.Province,
                c = $scope.City,
                n = $scope.County,
                pn = region[p].region_name,
                cn = region[p].city[c].region_name,
                nn = region[p].city[c].county[n].region_name;
                $scope.area = {
                    "p" : p,
                    "c" :c,
                    "n" : n,
                    "pn" : pn,
                    "cn" : cn,
                    "nn" : nn
                };
                $scope.areaName = JSON.stringify($scope.area);
            }
            if($scope.areaName){
                var areaJson = JSON.parse($scope.areaName);
                if(typeof areaJson === 'object'){
                    $scope.Province = areaJson.p;
                    $scope.City = areaJson.c;
                    $scope.County = areaJson.n;
                    $scope.changeProvince();
                    $scope.changeCity();
                    $scope.changeCounty();
                }
            }
        }
    }
});
